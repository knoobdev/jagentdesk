...
Use this guide to update plugins written for JAgentDesk 0.7 or earlier to the current client/server entry format, introduced in 0.8.
...
7. Keep `jagentdesk-plugin.json`, `package.json`, and `tsconfig.json` at the root.
8. Delete the old root entry. JAgentDesk does not load it.
...
| `import { defineRpc, defineAttachmentSource } from "@jagentdesk/plugin/server"` in shared files | `import { defineRpc, defineAttachmentSource } from "@jagentdesk/plugin"`                                     |
| `ZodOutput<typeof contract.input>` handler parameter types                                      | `RpcInput<typeof contract>` from `@jagentdesk/plugin`; `RpcOutput` for return types                          |

Import `PluginClientContext` from `@jagentdesk/plugin/client` and `PluginServerContext` from
`@jagentdesk/plugin/server`. Remove imports of the old context type. Client registrations return idempotent removal functions, except header buttons and composer pills, which return `{ update, remove }` handles. Preserve any remover the plugin calls before teardown; JAgentDesk removes outstanding
registrations after the entry cleanup runs.

### Composer pills

Update the plugin project's `@jagentdesk/plugin` dependency, then run `npm run typecheck`.
...
Move hooks (`useJAgentDesk`, `useRpc`, `useSettings`, `useAgent`, `useWorkspace`) and client contribution
types from `@jagentdesk/plugin` to `@jagentdesk/plugin/client`. Move `Icon` to
`@jagentdesk/plugin/client/react-native`. Import server contexts and lifecycle contracts from
`@jagentdesk/plugin/server`. Shared helpers (`defineRpc`, `defineSettings`, `defineAttachmentSource`),
...
| Old entry                         | 0.8 entry                                |
| --------------------------------- | ---------------------------------------- |
| `@jagentdesk/plugin/react-native` | `@jagentdesk/plugin/client/react-native` |
| `@jagentdesk/plugin/ui`           | `@jagentdesk/plugin/client/ui`           |
| `@jagentdesk/plugin/provider`     | `@jagentdesk/plugin/server/provider`     |
| `@jagentdesk/plugin/acp`          | `@jagentdesk/plugin/server/acp`          |

The old entries and the pre-0.8 `@jagentdesk/plugin` scope are removed. `/client/host` is private to
JAgentDesk's app integration and is never a plugin-author import.
...
| -------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------- |
| `This plugin has no requirements.jagentdesk`                                                                               | Complete the migration and declare the range in step 7.                                                                   |
| `This plugin was made for an older version of JAgentDesk`                                                                  | The directory still has only the old root entry. Create a runtime entry, move registrations, then delete the old file.    |
...
```ts
// index.ts
import type { PluginContext } from "@jagentdesk/plugin";
...
## 7. Declare the JAgentDesk requirement

After migrating the entries and imports, add the minimum runtime version to `jagentdesk-plugin.json`:

```json
{
  "id": "my-plugin",
  "requirements": { "jagentdesk": ">=0.8.0" }
}
```

Keep your existing ID and build commands. Missing `requirements.jagentdesk` means `<0.8.0`, so JAgentDesk 0.8 and later
reject the plugin even if its files have been moved. Adding the field alone does not migrate the
code. Update the local `@jagentdesk/plugin` development dependency to the version you target and
...
```bash
npm run typecheck
jagentdesk plugin reload <plugin-id>
