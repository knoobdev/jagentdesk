---
title: MCP reference
description: Reference for the JAgentDesk tools agents use to manage agents, workspaces, scripts, terminals, and schedules.
nav: MCP reference
order: 33
...
JAgentDesk can inject these tools into every new agent it launches. Open **Settings → your host → Agents** and turn on **Enable JAgentDesk tools**, or set `daemon.mcp.injectIntoAgents` to `true`.

Depending on the provider, JAgentDesk delivers the catalog through its native tool interface or MCP. The capabilities are the same either way.
...
These tools manage scripts configured in a workspace's `jagentdesk.json`. Each requires an explicit `workspaceId`; start and stop also require the configured `scriptName`.

| Tool                     | Function                                                                                |
| ------------------------ | --------------------------------------------------------------------------------------- |
| `list_workspace_scripts` | List configured scripts with lifecycle, terminal, port, proxy URL, and health metadata. |
| `start_workspace_script` | Start a configured script through JAgentDesk's managed launcher.                             |
| `stop_workspace_script`  | Stop a running script through its supervised terminal.                                  |

See [Git worktrees](/docs/worktrees#scripts-and-services) for `jagentdesk.json` configuration.
