---
title: Orchestration
description: Give any coding agent control of JAgentDesk so it can launch and coordinate agents from other providers.
nav: Overview
order: 30
...
JAgentDesk orchestration gives a coding agent control of the JAgentDesk daemon. The agent can discover every provider and model you have configured, create workspaces, launch other agents, send them follow-ups, and create heartbeats or schedules. The same work stays visible in the JAgentDesk app.

## Native subagents vs JAgentDesk subagents

The most important difference from native subagents is that **JAgentDesk subagents can cross provider boundaries**.
...
JAgentDesk subagents are full agents managed by the JAgentDesk daemon. The orchestrator can choose any configured provider and model, keep the worker in the current workspace, or place it in another workspace created for the task. Use them when you want one model to plan, another to implement, and another to review.

|                      | Native subagent                           | JAgentDesk subagent                                     |
| -------------------- | ----------------------------------------- | -------------------------------------------------- |
| Provider             | Same provider as its parent               | Any provider configured in JAgentDesk                   |
| Working directory    | Managed by the parent provider            | Current or explicitly selected workspace           |
| Lifecycle            | Owned by the parent provider              | Managed by JAgentDesk; can receive follow-ups           |
...
Open **Settings → your host → Agents**, then turn on **Enable JAgentDesk tools**. Start a new agent, or reload an existing one so it receives the tools.

Then ask naturally:

```text
Stay as the orchestrator. Use JAgentDesk to find my available Codex models, then
...
- **JAgentDesk subagents** open as full agent sessions. You can talk to them directly, change their settings, or archive them.
- **Native provider subagents** open as read-only timelines. You can inspect their work, but their provider owns their lifecycle.

A cross-workspace subagent still belongs to its parent's Subagents track. JAgentDesk also opens its workspace so the work is not hidden in an otherwise empty workspace. If you want to turn any subagent into a top-level agent, detach it manually in the app or with `jagentdesk agent detach`; detachment is not an agent-creation mode.

If an agent says background work is running but the track is empty, update JAgentDesk. Provider-created subagent timelines require JAgentDesk 0.1.107 or newer.
...
```text
Use JAgentDesk to create a heartbeat every 10 minutes. Keep checking this PR, fix any
new CI failures, and stop when all checks pass or after two hours.
```

The base [`/jagentdesk` orchestration skill](/docs/skills) teaches agents how to create heartbeats, so you only need to ask. A heartbeat continues the current conversation; a [schedule](/docs/schedules) is better for standalone cron-style jobs such as daily triage.
