...
description: Configure custom providers, alternative endpoints, profiles, custom binaries, and ACP agents in ~/.jagentdesk/config.json.
...
Everything beyond the [supported providers](/docs/supported-providers) lives under `agents.providers` in `~/.jagentdesk/config.json`. You can:
...
The examples below are a quick tour. The full, up-to-date reference is the local
[`docs/custom-providers.md`](../docs/custom-providers.md) document in this workspace.
...
Any agent that speaks [ACP](https://agentclientprotocol.com) over stdio can be added with `extends: "acp"` and a `command`. JAgentDesk spawns the process, sends an `initialize` JSON-RPC request, and the agent reports its capabilities, modes, and models at runtime.
...
For the complete field reference (`extends`, `label`, `command`, `env`, `models`, `additionalModels`, `disallowedTools`, `enabled`, `order`), model and thinking-option schemas, and deeper examples for each plan, see the local `docs/custom-providers.md` file.
