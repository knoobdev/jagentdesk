...
description: Copyable prompts for delegating, parallelizing, reviewing, and continuing agent work with JAgentDesk.
...
```text
Stay as the orchestrator. Use JAgentDesk to find the available Codex 5.6 model, then
...
```text
Create three JAgentDesk subagents in this workspace. Have one trace the request path,
...
```text
Split these two issues between two JAgentDesk subagents. Create a separate workspace
...
```text
Use JAgentDesk to create a heartbeat every 10 minutes. Continue this migration in
