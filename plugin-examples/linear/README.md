...
```bash
jagentdesk plugin add /absolute/path/to/jagentdesk/plugin-examples/linear
jagentdesk plugin ls linear
```

Use `jagentdesk plugin reload linear` after source edits. Changing the API key requires restarting the
