...
- Inject `JAGENTDESK_HOOK_CREATE_EXAMPLE=created` on creation and `JAGENTDESK_HOOK_OPEN_EXAMPLE=opened` on each session opening.
...
```bash
jagentdesk plugin install /absolute/path/to/plugin-examples/lifecycle-actions
jagentdesk plugin install /absolute/path/to/plugin-examples/lifecycle-logger
jagentdesk plugin logs lifecycle-logger
```

Outside the JAgentDesk repository, `server/inspect.ts` needs `@jagentdesk/protocol` installed for its
type imports. Add it to your plugin development dependencies at the same version as
`@jagentdesk/plugin` before installing this example.
