...
async function main() {
  ...
  try {
    ...
    await page.screenshot({ path: "/tmp/deploy-1-clusters.png" });
    ...
    // Select the target kind in the nav rail.
    await page.getByText(KIND, { exact: true }).first().click();
    await page.waitForTimeout(3000);
    await page.screenshot({ path: "/tmp/deploy-3-list.png" });
    const listBody = await page.evaluate(() => document.body.innerText);
    console.log(`[shot] 3 ${KIND} list. NAMESPACE hdr:`, /NAMESPACE|NAME/.test(listBody));

    // Open the first resource row → detail tab. Rows render the resource name;
    // click the first data cell under the header.
    const envRow = process.env.SHOT_ROW ?? "";
    const rowName =
      envRow ||
      (await page.evaluate((ctx: string) => {
    ...
    console.log("[shot] first row candidate:", JSON.stringify(rowName));
    if (rowName) {
      await page.getByText(rowName, { exact: true }).first().click();
      await page.waitForTimeout(3500);
      await page.screenshot({ path: "/tmp/deploy-4-detail.png" });
      const dBody = await page.evaluate(() => document.body.innerText);
      console.log(
        "[shot] 4 detail. Has DETAILS:",
        /DETAILS/i.test(dBody),
        "Has EVENTS:",
        /EVENTS/i.test(dBody),
        "Has Replicas:",
        /Replicas/i.test(dBody),
      );
      // Scroll the detail to reveal events, screenshot again.
      await page.mouse.wheel(0, 600);
      await page.waitForTimeout(800);
      await page.screenshot({ path: "/tmp/deploy-5-detail-scrolled.png" });
    }
    await browser.close();
    fin(true, `SHOT OK: clusters -> connect ${CTX} -> ${KIND} list -> detail`);
  } catch (e: unknown) {
