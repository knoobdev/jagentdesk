...
const appProductName = "JAgentDeskDebug";
const appScheme = "jagentdesk";
const preferredSimulatorType = process.env.JAGENTDESK_IOS_DEVICE_TYPE || "iPhone 16 Pro";
const jagentdeskPort = requiredEnv("JAGENTDESK_PORT");
const worktreePath = process.env.JAGENTDESK_WORKTREE_PATH || rootDir;
const worktreeName = process.env.JAGENTDESK_BRANCH_NAME || basename(worktreePath);
const worktreeHash = createHash("sha1").update(worktreePath).digest("hex").slice(0, 8);
const simulatorName =
  process.env.JAGENTDESK_IOS_SIMULATOR_NAME || `JAgentDesk ${worktreeName} ${worktreeHash}`;
const daemonEndpoint =
  process.env.JAGENTDESK_DEV_DAEMON_ENDPOINT ||
  `localhost:${process.env.JAGENTDESK_SERVICE_DAEMON_PORT || "6768"}`;
...
async function main() {
  ...
  metro = startMetro();
  await waitForUrl(`http://127.0.0.1:${jagentdeskPort}/.sim`);
  console.log(`iOS preview: ${process.env.JAGENTDESK_URL || `http://127.0.0.1:${jagentdeskPort}`}/.sim`);
...
function launchApp() {
  const metroUrl = encodeURIComponent(`http://127.0.0.1:${jagentdeskPort}`);
  run(
    "xcrun",
...
function startMetro() {
  const child = spawn("npx", ["expo", "start", "--port", jagentdeskPort, "--localhost"], {
    cwd: appDir,
    env: {
      ...env,
      JAGENTDESK_SERVE_SIM_PREVIEW: "1",
      JAGENTDESK_SERVE_SIM_DEVICE_UDID: simulatorUdid,
      BROWSER: "none",
    },
    stdio: "inherit",
  });
...
function requiredEnv(name) {
  ...
  if (!value) throw new Error(`${name} is required; run this as a JAgentDesk service.`);
  return value;
}
