...
// one interpreter every JAgentDesk checkout already depends on.

const sourceRoot = process.env.JAGENTDESK_SOURCE_CHECKOUT_PATH;
const targetRoot = process.env.JAGENTDESK_WORKTREE_PATH || process.cwd();
...
function seedJAgentDeskHome() {
  const source = process.env.JAGENTDESK_DEV_SEED_HOME || join(sourceRoot, ".dev/jagentdesk-home");
  const target = join(targetRoot, ".dev/jagentdesk-home");
  ...
  if (process.env.JAGENTDESK_DEV_RESET_HOME === "1") {
