 ...
 *
 * Boots its OWN isolated daemon subprocess (fresh mkdtemp JAGENTDESK_HOME, random
 ...
 *
 * Output: pretty table to stdout + JSON to /tmp/jagentdesk-terminal-bench/<ts>.json
 *
...
async function bootDaemon(): Promise<BootedDaemon> {
  ...
  const jagentdeskHome = await mkdtemp(path.join(os.tmpdir(), "jagentdesk-bench-home-"));
  ...
  const child = spawn(tsxBin, ["scripts/supervisor-entrypoint.ts", "--dev"], {
    ...
    env: {
      ...
      JAGENTDESK_SERVER_ID: "srv_terminal_bench",
  ...
  return { child, port, jagentdeskHome, pid: child.pid ?? -1 };
}
...
async function main(): Promise<void> {
  ...
  console.log("Booting isolated daemon (random port, fresh JAGENTDESK_HOME)...");
  ...
  try {
    ...
    console.log(`Daemon ready: pid=${daemon.pid} port=${daemon.port} home=${daemon.jagentdeskHome}`);

    const ctor = await loadDaemonClientCtor();
    client = await connectClient(ctor, daemon.port);

    workspaceDir = await mkdtemp(path.join(os.tmpdir(), "jagentdesk-bench-ws-"));
    ...
    const outDir = "/tmp/jagentdesk-terminal-bench";
    ...
    if (daemon) {
      ...
      await rm(daemon.jagentdeskHome, { recursive: true, force: true }).catch(() => undefined);
    }
    if (workspaceDir) {
