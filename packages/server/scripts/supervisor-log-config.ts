...
export function resolveSupervisorLogFile(
  ...
  const envRotateSize = env.JAGENTDESK_LOG_ROTATE_SIZE?.trim();
  const envRotateMaxFiles = parseOptionalPositiveInteger(env.JAGENTDESK_LOG_ROTATE_COUNT);
  let logPath = path.join(jagentdeskHome, DEFAULT_DAEMON_LOG_FILENAME);
  if (configuredPath) {
    logPath = path.isAbsolute(configuredPath)
      ? configuredPath
      : path.resolve(jagentdeskHome, configuredPath);
  }
