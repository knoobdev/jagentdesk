...
process.title = "JAgentDesk Supervisor";
...
function resolveWorkerExecArgv(workerEntry: string, devMode: boolean): string[] {
  ...
  const devArgs = [
    ...
    "--report-directory=/tmp/jagentdesk-reports",
  ];
  const inspectArg = process.env.JAGENTDESK_NODE_INSPECT ?? "--inspect";
...
function resolvePackagedNodeEntrypointRunnerPath(currentScriptPath: string): string | null {
  const packageMarker = `${path.sep}node_modules${path.sep}@jagentdesk${path.sep}server${path.sep}`;
...
async function main(): Promise<void> {
  ...
  const jagentdeskHome = resolveJAgentDeskHome(workerEnv);
  const persistedConfig = loadPersistedConfig(jagentdeskHome);
  const supervisorLogFile = resolveSupervisorLogFile(jagentdeskHome, persistedConfig, workerEnv);
  ...
  const stopLockHeartbeat = startPidLockHeartbeat(jagentdeskHome, {
    ownerPid: process.pid,
    onError: (error) => {
  ...
  const releaseLock = async (): Promise<void> => {
    ...
    lockReleased = true;
    stopLockHeartbeat();
    await releasePidLock(jagentdeskHome, {
      ownerPid: process.pid,
    });
  };
  ...
  const supervisor = runSupervisor({
    ...
    onWorkerReady: async ({ listen }) => {
      await updatePidLock(jagentdeskHome, { listen }, { ownerPid: process.pid });
    },
