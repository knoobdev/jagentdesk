...
async function verifyInjectedMcpForProvider(
  ...
  try {
    const prompt = [
      ...
      "If you have a tool called list_agents or create_agent from a jagentdesk MCP server, call list_agents once.",
      "After checking, reply with exactly JAGENTDESK_MCP_FOUND.",
      "If you do not have those tools, reply with exactly JAGENTDESK_MCP_NOT_FOUND.",
    ...
    if (!assistantText.includes("JAGENTDESK_MCP_FOUND")) {
      throw new Error(
        `Expected assistant to confirm JAgentDesk MCP availability. Assistant text:\n${assistantText}`,
      );
    }
    ...
    const listAgentsCalls = toolCalls.filter(
      (call) =>
        ...
        call.name === "jagentdesk.list_agents" ||
...
async function main(): Promise<void> {
  ...
  const rootCwd = await mkdtemp(path.join(os.tmpdir(), "jagentdesk-mcp-inject-real-"));
