...
const MEASUREMENT_CWD = "/tmp/jagentdesk-agent-tools-measurement";
...
Measures the MCP tools/list payload for JAgentDesk agent tools.
...
async function measureCatalog(params: {
  ...
  const client = new Client({ name: "jagentdesk-agent-tools-measurement", version: "0.0.0" });
...
function formatMarkdown(measurements: CatalogMeasurement[], top: number): string {
  ...
  const delta =
    withoutBrowser && withBrowser
      ...
      : null;
  const lines: string[] = [];
  lines.push("# JAgentDesk Agent Tool Catalog Context");
  lines.push("");
