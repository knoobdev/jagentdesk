...
import { createTestJAgentDeskDaemon } from "../src/server/test-utils/jagentdesk-daemon.js";
...
function parseArgs(): { sourceHome: string; frozenHomeRoot: string | null; scenario: Scenario } {
  let sourceHome = process.env.JAGENTDESK_BENCHMARK_SOURCE_HOME ?? path.join(os.homedir(), ".jagentdesk");
  let frozenHomeRoot = process.env.JAGENTDESK_BENCHMARK_FROZEN_HOME_ROOT ?? null;
  let scenario = (process.env.JAGENTDESK_BENCHMARK_SCENARIO ?? "snapshotOnly") as Scenario;
...
async function freezeHome(sourceHome: string, requestedRoot: string | null): Promise<string> {
  const frozenHomeRoot = requestedRoot ?? mkdtempSync(path.join(os.tmpdir(), "jagentdesk-real-home-"));
  if (process.env.JAGENTDESK_BENCHMARK_REUSE_FROZEN_HOME === "1") {
    return frozenHomeRoot;
  }
  const frozenHome = path.join(frozenHomeRoot, ".jagentdesk");
...
async function main(): Promise<void> {
  ...
  const daemon = await createTestJAgentDeskDaemon({ jagentdeskHomeRoot: frozenHomeRoot, cleanup: false });
