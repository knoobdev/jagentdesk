...
async function main(): Promise<void> {
  const tempDir = mkdtempSync(path.join(tmpdir(), "jagentdesk-git-observation-measurement-"));
  ...
  runGit(repoDir, ["config", "user.email", "measurement@jagentdesk.local"]);
  runGit(repoDir, ["config", "user.name", "JAgentDesk Measurement"]);
  ...
  try {
    healthy = createMeasuredService({ repoDir, jagentdeskHome });
    ...
    degraded = createMeasuredService({ repoDir, jagentdeskHome, failWorktreeWatch: true });
    startGitCommandMetrics();
    ...
    const outputPath = process.env.JAGENTDESK_GIT_OBSERVATION_REPORT?.trim();
    if (outputPath) {
