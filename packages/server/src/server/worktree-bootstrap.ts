...
import type { JAgentDeskServicePortAllocation } from "@jagentdesk/protocol/jagentdesk-config-schema";
...
function buildTerminalTimelineItem(input: {
  ...
  if (input.status === "running") {
    return {
      type: "tool_call",
      name: "jagentdesk_worktree_terminals",
      callId: input.callId,
      status: "running",
      detail: {
        type: "unknown",
...
export interface SpawnWorkspaceScriptOptions {
  ...
  globalServicePorts?: JAgentDeskServicePortAllocation;
  logger?: Logger;
...
async function setupServiceScriptRoute(params: {
  ...
  serviceProxy: ServiceProxySubsystem;
  servicePortAllocation: JAgentDeskServicePortAllocation | undefined;
...
export async function spawnWorkspaceScript(
  ...
  const configResult = readJAgentDeskConfig(repoRoot);
  if (!configResult.ok) {
    throw jagentdeskConfigParseError(configResult);
  }
  const scriptConfigs = getScriptConfigs(configResult.config);
  const config = scriptConfigs.get(scriptName);
  if (!config) {
    throw new Error(`Script '${scriptName}' is not configured in jagentdesk.json`);
  }
