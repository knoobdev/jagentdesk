...
import type { JAgentDeskOpenAIConfig, JAgentDeskSpeechConfig } from "../bootstrap.js";
...
function buildFeatureProviderInputs(params: {
  ...
  const voiceModeEnabled = resolveOptionalBooleanFlag(
    firstSpeechDefinedValue<string | boolean>([
      params.env.JAGENTDESK_VOICE_MODE_ENABLED,
  ...
  return {
    dictationStt: {
      configuredValue: firstSpeechDefinedValue<string>([
        params.env.JAGENTDESK_DICTATION_STT_PROVIDER,
        params.persisted.features?.dictation?.stt?.provider,
      ]),
      enabled: resolveOptionalBooleanFlag(
        firstSpeechDefinedValue<string | boolean>([
          params.env.JAGENTDESK_DICTATION_ENABLED,
    ...
    voiceTurnDetection: {
      configuredValue: firstSpeechDefinedValue<string>([
        params.env.JAGENTDESK_VOICE_TURN_DETECTION_PROVIDER,
    ...
    voiceStt: {
      configuredValue: firstSpeechDefinedValue<string>([
        params.env.JAGENTDESK_VOICE_STT_PROVIDER,
    ...
    voiceTts: {
      configuredValue: firstSpeechDefinedValue<string>([
        params.env.JAGENTDESK_VOICE_TTS_PROVIDER,
...
export function resolveSpeechConfig(params: {
  ...
  openai: JAgentDeskOpenAIConfig | undefined;
  ...
  const local = resolveLocalSpeechConfig({
    jagentdeskHome: params.jagentdeskHome,
    env: params.env,
    persisted: params.persisted,
    providers,
  });
