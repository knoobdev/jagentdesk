...
const RUN = process.env.JAGENTDESK_SPEECH_E2E_DOWNLOAD === "1";
...
downloadTest(
  ...
  async () => {
    ...
    const jagentdeskHomeRoot = mkdtempSync(path.join(tmpdir(), "jagentdesk-speech-download-"));
    const modelsDir = path.join(jagentdeskHomeRoot, ".jagentdesk", "models", "local-speech");
