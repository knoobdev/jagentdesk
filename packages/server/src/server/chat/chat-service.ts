...
import {
  ...
  type ChatRoom,
  type ChatRoomDetail,
} from "@jagentdesk/protocol/chat/types";
...
export class FileBackedChatService {
  ...
  constructor(options: { jagentdeskHome: string; logger: pino.Logger }) {
    this.filePath = path.join(options.jagentdeskHome, "chat", "rooms.json");
