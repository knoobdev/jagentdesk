...
function getPidFilePath(jagentdeskHome: string): string {
  return join(jagentdeskHome, "jagentdesk.pid");
}
...
function createLockHeldError(lock: PidLockInfo): PidLockError {
  return new PidLockError(
    `Another JAgentDesk daemon is already running (PID ${lock.pid}, started ${lock.startedAt})`,
    lock,
  );
}
  ...
  try {
    ...
    if (raceLock) {
      throw new PidLockError(
        `Another JAgentDesk daemon is already running (PID ${raceLock.pid})`,
        raceLock,
      );
    }
...
export async function acquirePidLock(
  ...
  const pidPath = getPidFilePath(jagentdeskHome);

  // Ensure jagentdeskHome directory exists
  ...
  const lockInfo: PidLockInfo = {
    ...
    listen,
    heartbeat: true,
    ...(process.env.JAGENTDESK_DESKTOP_MANAGED === "1" ? { desktopManaged: true } : {}),
  };
...
export function startPidLockHeartbeat(
  ...
  const timer = setInterval(() => {
    ...
    refreshing = true;
    refreshPidLock(jagentdeskHome, { ownerPid: options?.ownerPid })
      .catch((error) => {
        if (options?.onError) {
...
export async function getPidLockInfo(jagentdeskHome: string): Promise<PidLockInfo | null> {
  const pidPath = getPidFilePath(jagentdeskHome);
...
export async function isLocked(
  ...
  const info = await getPidLockInfo(jagentdeskHome);
  if (!info) {
    return { locked: false };
  }
