...
async function createSnapshotStormDaemon(clients: SnapshotStormProviderClient[]) {
  return createTestJAgentDeskDaemon({
    mcpEnabled: false,
    isDev: true,
    agentClients: {
...
function seedSameCwdWorkspaces(): { jagentdeskHomeRoot: string; cwd: string } {
  const jagentdeskHomeRoot = mkdtempSync(path.join(tmpdir(), "jagentdesk-same-cwd-home-"));
  const cwd = mkdtempSync(path.join(tmpdir(), "jagentdesk-same-cwd-dir-"));
  const projectsDir = path.join(jagentdeskHomeRoot, ".jagentdesk", "projects");
  ...
  return { jagentdeskHomeRoot, cwd };
}

async function seedWorkspaceWithLegacyAgent(): Promise<{ jagentdeskHomeRoot: string; cwd: string }> {
  const jagentdeskHomeRoot = mkdtempSync(path.join(tmpdir(), "jagentdesk-legacy-agent-home-"));
  const cwd = mkdtempSync(path.join(tmpdir(), "jagentdesk-legacy-agent-dir-"));
  const jagentdeskHome = path.join(jagentdeskHomeRoot, ".jagentdesk");
  const projectsDir = path.join(jagentdeskHome, "projects");
  mkdirSync(projectsDir, { recursive: true });
  ...
  const agentStorage = new AgentStorage(path.join(jagentdeskHome, "agents"), createTestLogger());
...
test("daemon bootstrap migrates cwd-only legacy agents before same-cwd workspaces are added", async () => {
  const { jagentdeskHomeRoot, cwd } = await seedWorkspaceWithLegacyAgent();
  const daemon = await createTestJAgentDeskDaemon({ jagentdeskHomeRoot });
...
test("workspace.create directory source with firstAgentContext generates a daemon-visible workspace title", async () => {
  const cwd = mkdtempSync(path.join(tmpdir(), "jagentdesk-named-local-dir-"));
...
test("local workspace auto-title does not broadcast provider snapshot warm-up to clients", async () => {
  const cwd = mkdtempSync(path.join(tmpdir(), "jagentdesk-title-snapshot-storm-"));
...
test("create_agent_request with workspaceId does not retitle an existing workspace", async () => {
  const cwd = mkdtempSync(path.join(tmpdir(), "jagentdesk-agent-submit-title-"));
  const daemon = await createTestJAgentDeskDaemon({
    agentClients: { mock: new MockLoadTestAgentClient() },
  });
  const client = new DaemonClient({
...
test("creating another same-cwd local workspace keeps running status on the owning workspace only", async () => {
  const cwd = mkdtempSync(path.join(tmpdir(), "jagentdesk-running-same-cwd-create-"));
  const daemon = await createTestJAgentDeskDaemon({
    agentClients: { mock: new MockLoadTestAgentClient() },
  });
  const client = new DaemonClient({
...
test("two workspaces sharing one cwd compute agent status per workspaceId", async () => {
  const { jagentdeskHomeRoot, cwd } = seedSameCwdWorkspaces();
  const daemon = await createTestJAgentDeskDaemon({ jagentdeskHomeRoot });
  const client = new DaemonClient({
    url: `ws://127.0.0.1:${daemon.port}/ws`,
    appVersion: "0.1.82",
