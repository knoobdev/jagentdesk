...
describe("daemon bearer auth", () => {
  beforeEach(() => {
    vi.useRealTimers();
    vi.restoreAllMocks();
    process.env = { ...originalEnv, JAGENTDESK_SUPERVISED: "0" };
  });
  ...
  test("closes WebSocket connections with readable auth failures when password is configured", async () => {
    ...
    try {
      ...
      await expectWebSocketCloses({
        port: daemonHandle.port,
        protocol: "jagentdesk.bearer.wrong-password",
        code: 4401,
      ...
      const { ws, protocol } = await connectWebSocket({
        port: daemonHandle.port,
        protocol: "jagentdesk.bearer.correct-password",
      });
      expect(protocol).toBe("jagentdesk.bearer.correct-password");
      ws.close();
    } finally {
      await daemonHandle.close();
    }
