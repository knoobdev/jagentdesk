...
import {
  ...
  handleJAgentDeskWorktreeArchiveRequest,
  handleJAgentDeskWorktreeListRequest,
  resolveGitCreateBaseBranch,
  runWorktreeSetupInBackground,
  handleCreateJAgentDeskWorktreeRequest,
...
import {
  ...
  type CreateJAgentDeskWorktreeFn,
...
function createWorkflowForRequestTest(options: {
  jagentdeskHome: string;
  createJAgentDeskWorktree?: CreateJAgentDeskWorktreeFn;
...
function createJAgentDeskWorktreeForTest(options: {
  jagentdeskHome: string;
  events?: string[];
}): CreateJAgentDeskWorktreeFn {
  ...
  return (input, serviceOptions) => {
    return createJAgentDeskWorktreeService(input, {
...
describe("handleJAgentDeskWorktreeListRequest", () => {
  test("lists worktrees through the workspace git service", async () => {
    const emitted: SessionOutboundMessage[] = [];
    const workspaceGitService = {
      listWorktrees: vi.fn().mockResolvedValue([
        {
          path: "/tmp/jagentdesk-home/worktrees/repo/feature",
    ...
    await handleJAgentDeskWorktreeListRequest(
      {
        emit: (message) => emitted.push(message),
        jagentdeskHome: "/tmp/jagentdesk-home",
    ...
    expect(emitted).toContainEqual({
      type: "jagentdesk_worktree_list_response",
      payload: {
        worktrees: [
          {
            worktreePath: "/tmp/jagentdesk-home/worktrees/repo/feature",
            createdAt: "2026-04-12T00:00:00.000Z",
            branchName: "feature",
            head: "abc123",
          },
  ...
  test("agent setup continuation starts setup for the created agent timeline", async () => {
    ...
    try {
      const result = await createJAgentDeskWorktreeWorkflow(
        {
          jagentdeskHome,
          createJAgentDeskWorktree: createJAgentDeskWorktreeForTest({ jagentdeskHome }),
      ...
      await vi.waitFor(() => {
        expect(appendedItems).toContainEqual({
          name: "jagentdesk_worktree_setup",
          status: "completed",
        });
      });
...
function createGitRepo(options?: { jagentdeskConfig?: Record<string, unknown> }) {
...
describe("runWorktreeSetupInBackground", () => {
  ...
  test("runs setup from an exact workspace subdirectory", async () => {
    ...
    writeFileSync(
      path.join(sourceWorkspaceCwd, "jagentdesk.json"),
      JSON.stringify({
        worktree: {
  ...
  test("archives the pending workspace and emits a failed snapshot when setup cannot start", async () => {
    ...
    writeFileSync(path.join(repoDir, "jagentdesk.json"), "{ invalid json\n");
    execFileSync("git", ["add", "jagentdesk.json"], { cwd: repoDir, stdio: "pipe" });
    ...
    expect(progressMessages[1]?.payload.error).toMatch(
      /Failed to parse jagentdesk\.json at .*jagentdesk\.json/,
    );
    expect(progressMessages[1]?.payload.detail.commands).toEqual([]);
    expect(snapshots.get(workspaceId)).toMatchObject({
      status: "failed",
      error: expect.stringMatching(/Failed to parse jagentdesk\.json at .*jagentdesk\.json/),
    });
    ...
    const snapshots = new Map([
      [
        "ws-feature-a",
        {
          ...
          detail: {
            type: "worktree_setup",
            worktreePath: "/repo/.jagentdesk/worktrees/feature-a",
...
describe("handleCreateJAgentDeskWorktreeRequest", () => {
  const cleanupPaths: string[] = [];
  ...
  test("checks out the GitHub PR branch when githubPrNumber is supplied", async () => {
    ...
    await handleCreateJAgentDeskWorktreeRequest(
      {
        jagentdeskHome,
        ...
        emit: (message) => emitted.push(message),
        sessionLogger: logger,
        createJAgentDeskWorktreeWorkflow: createWorkflowForRequestTest({ jagentdeskHome }),
      },
      {
  ...
  test("buildAgentSessionConfig passes prompt and attachment context into worktree creation", async () => {
    const createJAgentDeskWorktree = vi.fn(async () => ({
      worktree: {
  ...
  test("createJAgentDeskWorktreeForTest forwards the default branch resolver for branch-off intents", async () => {
    const { tempDir, repoDir } = createGitRepo();
    cleanupPaths.push(tempDir);
    const jagentdeskHome = path.join(tempDir, ".jagentdesk");
    const resolveDefaultBranch = vi.fn(async () => "main");

    const result = await createJAgentDeskWorktreeForTest({ jagentdeskHome })(
      {
        cwd: repoDir,
      ...
      await handleCreateJAgentDeskWorktreeRequest(
        {
          ...
          sessionLogger: createLogger(),
          emit: (message) => emitted.push(message),
          createJAgentDeskWorktreeWorkflow: createWorkflowForRequestTest({
            jagentdeskHome,
            createJAgentDeskWorktree: createJAgentDeskWorktreeForTest({ jagentdeskHome, events }),
          }),
  ...
  test("creates the worktree before emitting the response", async () => {
    ...
    try {
      await handleCreateJAgentDeskWorktreeRequest(
        {
          ...
          createJAgentDeskWorktreeWorkflow: createWorkflowForRequestTest({
            jagentdeskHome,
            createJAgentDeskWorktree: async (input) => {
              const result = await createJAgentDeskWorktreeForTest({ jagentdeskHome })(input);
      ...
      expect(emitted.map((message) => message.type).slice(0, 2)).toEqual([
        "create_jagentdesk_worktree_response",
        "workspace_update",
      ]);
...
describe("handleJAgentDeskWorktreeArchiveRequest worktree scope", () => {
  const cleanupPaths: string[] = [];
  ...
  test("archives every active workspace on the directory and removes it", async () => {
    ...
    await handleJAgentDeskWorktreeArchiveRequest(
      {
        jagentdeskHome,
        github: createGitHubServiceStub(),
        workspaceGitService: {
    ...
    expect(
      emitted.find((message) => message.type === "jagentdesk_worktree_archive_response"),
    ).toMatchObject({
      payload: {
        success: true,
        error: null,
  ...
  test("ignores deleteWorktreeFromDisk:true and derives directory removal from remaining references", async () => {
    ...
    await handleJAgentDeskWorktreeArchiveRequest(deps, {
      type: "jagentdesk_worktree_archive_request",
    ...
    expect(existsSync(sharedCwd)).toBe(false);
    expect(
      emitted.filter((message) => message.type === "jagentdesk_worktree_archive_response"),
    ).toHaveLength(2);
  });
});
