...
function createTempHome(): string {
  return mkdtempSync(path.join(tmpdir(), "jagentdesk-config-"));
}
...
describe("PersistedConfigSchema worktrees config", () => {
  test("accepts optional worktree root", () => {
    ...
    expect(parsed.worktrees?.root).toBe("/mnt/fast/jagentdesk-worktrees");
  });
...
describe("loadPersistedConfig", () => {
  test("initializes with the current daemon defaults", () => {
    const home = createTempHome();
    try {
      const config = loadPersistedConfig(home);
      expect(config.daemon?.listen).toBe("127.0.0.1:6768");
    } finally {
