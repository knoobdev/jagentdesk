...
import { z } from "zod";
import { PluginIdSchema, type PluginSourceStatusItem } from "@jagentdesk/protocol/messages";
...
export class ManagedPluginSources {
  ...
  constructor(jagentdeskHome: string) {
    this.root = path.join(jagentdeskHome, "plugins");
...
async function clone(remote: string, checkoutRoot: string): Promise<void> {
  ...
  const cloneRemote = publicRemote === remote ? remote : "https://jagentdesk.invalid/plugin.git";
  const envOverlay =
    cloneRemote === remote
      ? GIT_ENV
      : {
