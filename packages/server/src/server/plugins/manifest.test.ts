...
describe("plugin manifest", () => {
  ...
  it("reads and validates requirements before any plugin code runs", async () => {
    const directory = await mkdtemp(path.join(tmpdir(), "jagentdesk-plugin-manifest-"));
    directories.push(directory);
    const manifest = path.join(directory, "jagentdesk-plugin.json");
    await writeFile(
      manifest,
      JSON.stringify({ id: "example", requirements: { jagentdesk: "^0.8.0" } }),
    );
