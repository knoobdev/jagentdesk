...
async function writePlugin(root: string, jagentdesk?: string, build?: string[][]) {
  await writeFile(
    path.join(root, "jagentdesk-plugin.json"),
    JSON.stringify({
      id: "example",
      requirements: jagentdesk === undefined ? undefined : { jagentdesk },
      build,
    }),
  );
  await writeFile(
...
it("rejects incompatible installs without persisting them, then accepts the corrected manifest", async () => {
  ...
  expect(service.catalog()).toEqual([
    { id: "example", requirements: { jagentdesk: "^0.8.0" }, clientBundle: expect.any(String) },
  ]);
});

it("runs plugins without requirements and enforces a declared range on reload", async () => {
  const root = await directory();
  ...
  await writePlugin(root, ">=0.9.0");
  await expect(service.reloadPlugin("example")).rejects.toThrow("requires JAgentDesk >=0.9.0");
  expect(service.catalog()).toEqual([]);
});

it("runs a single-entry plugin with a compatible requirement as a client entry", async () => {
  // Upstream rejects pre-split plugins; JAgentDesk keeps loading them (index.ts → client entry).
  const root = await directory();
  await writePlugin(root, ">=0.8.0");
  await rm(path.join(root, "index.client.ts"));
  await writeFile(path.join(root, "index.ts"), "export default () => () => {};");
  const { service } = await host();
  const installed = await service.installDirectory({ path: root });
  ...
  await expect(service.applyUpdates([preview!.proposal!])).resolves.toMatchObject([
    {
      id: "example",
      outcome: "error",
      error: expect.stringContaining("requires JAgentDesk >=0.9.0"),
    },
  ]);
