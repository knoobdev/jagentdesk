...
function createRuntimeBoundaryPlugin(target: PluginBuildTarget): Plugin {
  return {
    name: `jagentdesk-plugin-${target}-runtime-boundary`,
    setup(buildContext) {
...
function createUnusedPlatformModulePlugin(target: PluginBuildTarget): Plugin {
  const filter =
    target === "server"
      ...
      : /^node:/;
  return {
    name: `jagentdesk-plugin-${target}-unused-platform-modules`,
    setup(buildContext) {
      buildContext.onResolve({ filter }, (args) => ({
        path: args.path,
        namespace: "jagentdesk-unused-platform-module",
        sideEffects: false,
      }));
      buildContext.onLoad({ filter: /.*/, namespace: "jagentdesk-unused-platform-module" }, () => ({
