...
async function createPlugin(id: string, source: string): Promise<string> {
  const directory = await mkdtemp(path.join(tmpdir(), "jagentdesk-plugin-service-"));
  roots.push(directory);
  await writeFile(path.join(directory, "jagentdesk-plugin.json"), JSON.stringify({ id }));
...
function bindTestSessionHost(service: PluginService): PluginService {
  service.bindJAgentDeskSessionHost({
...
function createPausedRuntime() {
  ...
  const runtime: TestPluginRuntime = {
    ...
    subscribe: () => () => undefined,
    bindJAgentDeskSessionHost: () => undefined,
  };
...
describe("PluginService", () => {
  it("resolves a provider icon path to sanitized inline SVG", async () => {
    const home = await mkdtemp(path.join(tmpdir(), "jagentdesk-plugin-home-"));
    roots.push(home);
  ...
  it("lists manifest descriptions for running and disabled plugins without hiding malformed entries", async () => {
    ...
    await writeFile(
      path.join(running, "jagentdesk-plugin.json"),
      JSON.stringify({ id: "running", description: "Runs checks" }),
    );
    await writeFile(
      path.join(disabled, "jagentdesk-plugin.json"),
      JSON.stringify({ id: "disabled", description: "Waits until enabled" }),
    );
    await writeFile(path.join(malformed, "jagentdesk-plugin.json"), "{");
    const service = createService(home, {
  ...
  it("prefers an existing directory and installs its selected plugin subdirectory", async () => {
    ...
    await writeFile(
      path.join(pluginDirectory, "jagentdesk-plugin.json"),
  ...
  it.each([true, false])(
    ...
    async (enabled) => {
      const home = await mkdtemp(path.join(tmpdir(), "jagentdesk-legacy-layout-"));
      roots.push(home);
      const repository = await mkdtemp(path.join(tmpdir(), "jagentdesk-legacy-remote-"));
      roots.push(repository);
      const id = "legacy-review";
      const pluginPath = "plugins/review";
      await mkdir(path.join(repository, pluginPath), { recursive: true });
      await writeFile(
        path.join(repository, pluginPath, "jagentdesk-plugin.json"),
        JSON.stringify({ id, requirements: { jagentdesk: ">=0.8.0" } }),
      );
      await writeFile(
        path.join(repository, pluginPath, "index.server.ts"),
        `import { defineSettings } from "@jagentdesk/plugin";
import { z } from "zod";
...
export default function contribute(server: PluginServerContext) {
  ...
  it("keeps the running commit when a Git update build command fails", async () => {
    const home = await mkdtemp(path.join(tmpdir(), "jagentdesk-plugin-home-"));
    roots.push(home);
    const repository = await mkdtemp(path.join(tmpdir(), "jagentdesk-plugin-repository-"));
    roots.push(repository);
    await runGitCommand(["init", "-b", "main"], { cwd: repository });
    await runGitCommand(["config", "user.name", "JAgentDesk Tests"], { cwd: repository });
    await runGitCommand(["config", "user.email", "jagentdesk@example.test"], { cwd: repository });
    await writeFile(
      path.join(repository, "jagentdesk-plugin.json"),
  ...
  it.each([false, true])(
    ...
    async (failRestore) => {
      const home = await mkdtemp(path.join(tmpdir(), "jagentdesk-plugin-recovery-"));
      roots.push(home);
...
export default function contribute(plugin: unknown) {
  ...
  it("reports invalid manifests, missing entries, and startup failures", async () => {
    ...
    await writeFile(path.join(invalid, "jagentdesk-plugin.json"), JSON.stringify({}));
    ...
    // JAgentDesk keeps loading pre-split single-entry plugins (index.ts runs as the client entry).
    await expect(service.installDirectory({ path: legacy })).resolves.toMatchObject({
      id: "legacy-plugin",
      status: "running",
    });
    ...
    expect(await service.listPlugins()).toMatchObject([
      expect.objectContaining({ id: "legacy-plugin", status: "running" }),
      expect.objectContaining({
        id: "missing-entry",
        status: "failed",
...
describe("npm plugin installation", () => {
  it("preserves dependencies and metadata, and reloads without npm", async () => {
    ...
    const failures: Array<[string, Record<string, string>]> = [
      ["bad-manifest", { "jagentdesk-plugin.json": "{}" }],
      [
        "bad-build",
        {
          "jagentdesk-plugin.json": JSON.stringify({
            id: "bad-build",
            requirements: { jagentdesk: ">=0.4.0" },
      ...
      [
        "bad-requirements",
        {
          "jagentdesk-plugin.json": JSON.stringify({
            id: "bad-requirements",
            requirements: { jagentdesk: ">=999.0.0" },
          }),
        },
      ],
    ...
    const registry = await startNpmRegistry([
      ...packages,
      {
        name: "jagentdesk-prepared-plugin",
        version: "1.0.0",
        dependencies: { "jagentdesk-fixture-dependency": "1.0.0" },
        scripts: { postinstall: "node -e \"throw new Error('npm lifecycle must not run')\"" },
        files: {
          "nested/jagentdesk-plugin.json": JSON.stringify({
            id: "prepared",
            requirements: { jagentdesk: ">=0.4.0" },
            build: [
              [
...
import dependency from "jagentdesk-fixture-dependency";
import { z } from "zod";
export default function contribute() {
  ...
  return () => {};
    ...
    const home = await mkdtemp(path.join(tmpdir(), "jagentdesk-npm-service-"));
    roots.push(home);
    ...
    try {
      ...
      const installed = await service.installSource({
        source: "npm:jagentdesk-fixture-plugin@^1.0.0",
      });
      expect(installed).toMatchObject({
        id: "npm-review",
        status: "running",
        installation: {
          identity: { kind: "npm", packageName: "jagentdesk-fixture-plugin", pluginPath: "." },
      ...
      expect(
        JSON.parse(await readFile(path.join(installRoot, "package-lock.json"), "utf8")).packages[
          "node_modules/jagentdesk-fixture-dependency"
        ].version,
      ).toBe("1.0.0");
      for (const [source, id, version] of [
        ["npm:jagentdesk-fixture-plugin@1.0.0", "exact", "1.0.0"],
        ["jagentdesk-fixture-plugin@stable", "tag", "1.0.0"],
        ["npm:@jagentdesk-fixture/review", "scoped", "2.0.0"],
      ]) {
      ...
      registry.setTag("jagentdesk-fixture-plugin", "latest", "2.0.0");
      ...
      registry.setTag("jagentdesk-fixture-plugin", "latest", "1.1.0");
      ...
      const prepared = await service.installSource({
        source: "npm:jagentdesk-prepared-plugin@1.0.0:nested",
      });
      ...
      expect(registry.requests).not.toContain("/@jagentdesk/plugin");
      await expect(service.installSource({ source: "npm:missing-plugin" })).rejects.toThrow();
      await expect(
        service.installSource({ source: "npm:jagentdesk-fixture-plugin", ref: "main" }),
      ).rejects.toThrow("--ref is only valid for Git");
      await expect(
        service.installSource({ source: "npm:jagentdesk-fixture-plugin" }),
      ...
      await expect(
        service.installSource({ source: "npm:jagentdesk-fixture-plugin", id: "missing-npm" }),
