...
import type { ProviderEvent } from "@jagentdesk/plugin/server/provider";
...
async function createPlugin(id: string, source: string): Promise<string> {
  const directory = await mkdtemp(path.join(tmpdir(), "jagentdesk-plugin-"));
  temporaryDirectories.push(directory);
  await writeFile(path.join(directory, "jagentdesk-plugin.json"), JSON.stringify({ id }), "utf8");
...
describe("PluginRuntime", () => {
  it.each([
    { specifier: "@jagentdesk/plugin", moduleDirectory: "shared" },
    { specifier: "@jagentdesk/plugin", moduleDirectory: "server" },
  ])(
...
import type { ProviderEvent, ProviderRegistration } from "@jagentdesk/plugin/server/provider";
...
export default function contribute(server: PluginServerContext) {
  ...
  return () => {};
    ...
    // The daemon's provider registry does not read plugin providers yet; the plugin log says so.
    expect(pluginMessages(runtime, "provider-round-trip")).toContain(
      "[jagentdesk] Not supported by this daemon yet, so it will not run: agent provider direct-example",
    );
...
export default function contribute(server: any) { server.registerProvider(provider); return () => undefined; }`,
  ...
  it("records host-owned plugin lifecycle events", async () => {
    ...
    expect(
      ...
      { stream: "stdout", message: "[jagentdesk] Loading plugin" },
      { stream: "stdout", message: "[jagentdesk] Plugin ready" },
      { stream: "stdout", message: "[jagentdesk] Stopping plugin" },
      { stream: "stdout", message: "[jagentdesk] Plugin stopped" },
    ]);
  });
  ...
  it("frames stdout and stderr, normalizes CRLF, and flushes final fragments once", async () => {
    ...
    expect(
      logs
        .filter((entry) => !entry.message.startsWith("[jagentdesk]"))
  ...
  it("retains compilation failures in the plugin stderr tail", async () => {
    ...
    expect(
      ...
      {
        stream: "stdout",
        message: "[jagentdesk] Loading plugin",
      },
      {
        stream: "stderr",
  ...
  it("waits for asynchronous plugin cleanup before stopping", async () => {
    const cleanupFile = path.join(tmpdir(), `jagentdesk-plugin-cleanup-${Date.now()}`);
    const directory = await createPlugin(
      "async-cleanup",
...
export default function contribute(plugin: unknown) {
  ...
  it("rejects provider input while a connection is closing", async () => {
    const releaseDirectory = await mkdtemp(path.join(tmpdir(), "jagentdesk-provider-close-"));
...
export default function contribute(server: { registerProvider(provider: ProviderRegistration): void }) {
  ...
  it("closes a provider connection that resolves during plugin shutdown", async () => {
    ...
    const startedFile = path.join(tmpdir(), `jagentdesk-provider-connect-started-${suffix}`);
    const closedFile = path.join(tmpdir(), `jagentdesk-provider-connect-closed-${suffix}`);
    const directory = await createPlugin(
      "shutdown-connect",
      `import { writeFile } from "node:fs/promises";
import type { ProviderRegistration } from "@jagentdesk/plugin/server/provider";
...
export default function contribute(server: { registerProvider(provider: ProviderRegistration): void }) {
  ...
  it("runs a pre-split index.ts plugin as a client entry", async () => {
    const directory = await mkdtemp(path.join(tmpdir(), "jagentdesk-plugin-"));
    temporaryDirectories.push(directory);
    await Promise.all([
      writeFile(path.join(directory, "jagentdesk-plugin.json"), JSON.stringify({ id: "legacy" })),
      writeFile(path.join(directory, "index.ts"), "export default function contribute() {}"),
    ]);
    const runtime = createTestRuntime();

    // Upstream rejects pre-split plugins; JAgentDesk keeps them working as client-only plugins.
    await runtime.startPlugin("legacy", directory);
    expect(runtime.catalog().map((plugin) => plugin.id)).toContain("legacy");
    await runtime.stopAll();
  });

  it("uses a renamed index.tsx as the client entry beside index.server.ts", async () => {
    // Installs made by JAgentDesk <= v0.9.40 renamed index.client.tsx to index.tsx.
    const directory = await mkdtemp(path.join(tmpdir(), "jagentdesk-plugin-"));
    temporaryDirectories.push(directory);
    await Promise.all([
      writeFile(path.join(directory, "jagentdesk-plugin.json"), JSON.stringify({ id: "renamed" })),
      writeFile(
      ...
      writeFile(
        ...
        'export default function contribute() { const label = "renamed client"; void label; return () => {}; }',
      ),
    ]);
    const runtime = createTestRuntime();

    await runtime.startPlugin("renamed", directory);
    const entry = runtime.catalog().find((plugin) => plugin.id === "renamed");
    expect(entry?.clientBundle).toContain("renamed client");
    await runtime.stopAll();
  });
  ...
  it("loads a client-only plugin without spawning a subprocess", async () => {
    ...
    temporaryDirectories.push(directory);
    await Promise.all([
      writeFile(path.join(directory, "jagentdesk-plugin.json"), JSON.stringify({ id: "theme" })),
      writeFile(
  ...
  it("loads separate entries, exposes the client bundle, and invokes the server RPC", async () => {
    ...
    await writeFile(
      ...
      JSON.stringify({ id: "hello" }),
    );
    await writeFile(
      path.join(directory, "index.client.tsx"),
      `import React from "react";
import { Text } from "react-native";
import { defineAttachmentSource } from "@jagentdesk/plugin";
...
export default function contribute(server: any) {
  ...
  return () => undefined;
    ...
    await expect(runtime.invoke("hello", "greet", { name: "JAgentDesk" })).resolves.toMatchObject({
      message: "Hello, JAgentDesk",
    });
...
export default function contribute(plugin: any) {
  ...
  it("attaches no replacement session when the plugin is stopped", async () => {
    ...
    expect(runtime.getLogs("stopping").map((entry) => entry.message)).not.toContain(
      "[jagentdesk] Re-attached plugin session",
    );
    expect(sessions.active.size).toBe(0);
  });
