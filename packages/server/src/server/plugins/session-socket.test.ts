...
describe("PluginSessionSocket", () => {
  it("preserves text and binary frame order across IPC", () => {
    ...
    expect(sent).toEqual([
      { type: "jagentdesk_frame", data: "first", isBinary: false },
      { type: "jagentdesk_frame", data: new Uint8Array([1, 2, 3]), isBinary: true },
      { type: "jagentdesk_frame", data: "last", isBinary: false },
    ]);
  });
