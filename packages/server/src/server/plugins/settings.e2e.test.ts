...
import { z } from "zod";
import { defineRpc, settingsRpc } from "@jagentdesk/plugin";
...
export default function(server) {
  ...
  const first = await createTestJAgentDeskDaemon(options);
  ...
  const restarted = await createTestJAgentDeskDaemon(options);
...
test("two clients share settings, observe changes, and preserve values through plugin lifecycle", async () => {
  ...
  try {
    ...
    await writeFile(
      path.join(directory, "index.server.ts"),
      `import { defineRpc, defineSettings } from "@jagentdesk/plugin";
import { z } from "zod";
...
export default function(server) {
  ...
  return () => {};
    ...
    // JAgentDesk broadcasts status messages to every session; there is no per-event opt-in.
