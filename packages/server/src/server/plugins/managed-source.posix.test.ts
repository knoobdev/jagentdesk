...
async function createRepository(): Promise<string> {
  const repository = await mkdtemp(path.join(tmpdir(), "jagentdesk-plugin-git-repository-"));
...
describe("managed Git plugin sources", () => {
  it("does not expose Git URL credentials when cloning fails", async () => {
    const home = await mkdtemp(path.join(tmpdir(), "jagentdesk-plugin-git-home-"));
    roots.push(home);
  ...
  it("normalizes explicit GitHub identifiers and shorthand to the same Git source", async () => {
    ...
    const home = await mkdtemp(path.join(tmpdir(), "jagentdesk-plugin-github-home-"));
    roots.push(home);
    const overlay = {
      GIT_CONFIG_COUNT: "1",
  ...
  it("resolves branch, tag and commit installs against changed default HEAD and preserves the reviewed commit", async () => {
    const repository = await createRepository();
    const home = await mkdtemp(path.join(tmpdir(), "jagentdesk-plugin-review-"));
    roots.push(home);
    const remote = pathToFileURL(repository).href;
  ...
  it("uses retained acquisition remote when credential rewriting changes origin, preserving nested identity", async () => {
    ...
    for (const name of ["jagentdesk-plugin.json", "index.server.ts"])
    ...
    const home = await mkdtemp(path.join(tmpdir(), "jagentdesk-plugin-credential-"));
    roots.push(home);
    try {
      const sources = new ManagedPluginSources(home);
      const candidate = await sources.place(
      ...
      ).toBe("https://jagentdesk.invalid/plugin.git");
