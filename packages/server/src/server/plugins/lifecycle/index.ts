...
import { z } from "zod";
import { CreateAgentRequestMessageSchema } from "@jagentdesk/protocol/messages";
import type {
  PluginHookAgent,
  PluginHookContext,
...
import { WorkspaceCreateRequestSchema } from "@jagentdesk/protocol/messages";
...
export class PluginHookHandlers implements PluginLifecycleRegistration {
  ...
  async invoke(
    ...
    name: string,
    input: unknown,
    jagentdesk: PluginHookContext["jagentdesk"],
  ): Promise<unknown> {
    ...
    try {
      if (kind === "before") {
        if (
          !beforeHookNames.includes(
            name as keyof import("@jagentdesk/plugin/server").PluginBeforeRequests,
          )
        ) {
          throw new Error(`Unknown before hook: ${name}`);
        }
        const hookName = name as keyof import("@jagentdesk/plugin/server").PluginBeforeRequests;
        let request = validateBeforeRequest(hookName, input);
        for (const handler of this.transforms.get(name) ?? []) {
          controller.signal.throwIfAborted();
          const result = await handler(
            { request: structuredClone(request) },
            { jagentdesk, signal: controller.signal },
          );
          if (result !== undefined) {
      ...
      for (const handler of this.events.get(name) ?? []) {
        controller.signal.throwIfAborted();
        try {
          await handler(structuredClone(input), { jagentdesk, signal: controller.signal });
        } catch (error) {
