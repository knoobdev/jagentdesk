import type { DaemonTransportFactory } from "@jagentdesk/client/internal/daemon-client-transport-types";
...
export function createPluginDaemonTransportFactory(
  ...
  return () => {
    ...
    const unsubscribe = port.onMessage((message) => {
      ...
      if (message.type === "jagentdesk_close") closeTransport();
    });
    ...
    return {
      send(data) {
        ...
        port.send({ type: "jagentdesk_frame", data: frame, isBinary: typeof frame !== "string" });
      },
      close() {
        if (!isOpen) return;
        port.send({ type: "jagentdesk_close" });
        closeTransport();
      },
      onMessage(handler) {
