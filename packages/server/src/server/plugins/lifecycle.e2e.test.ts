// Upstream e2e coverage kept for the next selective port. Skipped: plugin lifecycle hooks and plugin agent providers are not wired into the fork's workspace/agent managers yet.
...
test.skip("a plugin transforms workspace creation once across receipt replays and observes its committed lifecycle", async () => {
  const directory = await mkdtemp(path.join(tmpdir(), "jagentdesk-lifecycle-"));
  ...
  try {
    await writeFile(
      ...
      JSON.stringify({ id: "lifecycle", requirements: { jagentdesk: ">=0.8.0" } }),
    );
    await writeFile(
...
test.skip("plugins observe turns, answer permissions, and observe archive without blocking the agent", async () => {
  const directory = await mkdtemp(path.join(tmpdir(), "jagentdesk-turn-hooks-"));
  const daemon = await createTestJAgentDeskDaemon({ daemonVersion: "0.8.0" });
  const client = new DaemonClient({ url: `ws://127.0.0.1:${daemon.port}/ws`, appVersion: "0.8.0" });
  try {
    await writeFile(
      path.join(directory, "jagentdesk-plugin.json"),
      JSON.stringify({ id: "turn-hooks", requirements: { jagentdesk: ">=0.8.0" } }),
    );
    await writeFile(
      path.join(directory, "index.server.ts"),
      `
export default function contribute(server) {
  ...
  server.on("agent.permission_requested", async (event, context) => {
    ...
    await context.jagentdesk.agents.ref(event.agent.id).respondToPermission({
...
test.skip("agent creation hooks change the provider and environment before the session opens", async () => {
  const directory = await mkdtemp(path.join(tmpdir(), "jagentdesk-agent-hooks-"));
  const daemon = await createTestJAgentDeskDaemon({ daemonVersion: "0.8.0" });
  const client = new DaemonClient({ url: `ws://127.0.0.1:${daemon.port}/ws`, appVersion: "0.8.0" });
  try {
    await writeFile(
      path.join(directory, "jagentdesk-plugin.json"),
      JSON.stringify({ id: "agent-hooks", requirements: { jagentdesk: ">=0.8.0" } }),
    );
    await writeFile(
      path.join(directory, "index.server.ts"),
      `
...
test.skip("invalid output from an untyped plugin rejects creation before later callbacks run", async () => {
  const directory = await mkdtemp(path.join(tmpdir(), "jagentdesk-invalid-hook-"));
  const daemon = await createTestJAgentDeskDaemon({ daemonVersion: "0.8.0" });
  const client = new DaemonClient({ url: `ws://127.0.0.1:${daemon.port}/ws`, appVersion: "0.8.0" });
  try {
    await writeFile(
      path.join(directory, "jagentdesk-plugin.json"),
      JSON.stringify({ id: "invalid-hook", requirements: { jagentdesk: ">=0.8.0" } }),
    );
    await writeFile(
      path.join(directory, "index.server.ts"),
      `
...
test.skip("a clean daemon shutdown leaves a completed plugin-provider agent without an error", async () => {
  ...
  const daemon = await createTestJAgentDeskDaemon({
    jagentdeskHomeRoot: path.join(root, "daemon"),
...
test.skip("a provider that never acknowledges session.close does not hold the daemon open", async () => {
...
async function createSlowClosingProviderPlugin() {
  const directory = await mkdtemp(path.join(tmpdir(), "jagentdesk-slow-close-plugin-"));
...
test.skip("reloading a plugin does not send on a closed IPC channel", async () => {
  const {
  ...
  const cwd = await mkdtemp(path.join(tmpdir(), "jagentdesk-reload-teardown-"));
  const daemon = await createTestJAgentDeskDaemon();
