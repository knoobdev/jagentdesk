// Upstream e2e coverage kept for the next selective port. Skipped: needs timeline.append, DaemonClient.observeEvents and the upstream subscription API, which the fork does not have.
...
test.skip("plugin handlers create workspaces and agents through their JAgentDesk API", async () => {
  const pluginDirectory = await mkdtemp(path.join(tmpdir(), "jagentdesk-api-plugin-"));
  const workspaceDirectory = await mkdtemp(path.join(tmpdir(), "jagentdesk-api-workspace-"));
  roots.push(pluginDirectory, workspaceDirectory);
  await writeFile(
    path.join(pluginDirectory, "jagentdesk-plugin.json"),
    JSON.stringify({
      id: "jagentdesk-api",
      requirements: { jagentdesk: `>=${resolveDaemonVersion(import.meta.url)}` },
    }),
  );
  await writeFile(
...
export default function contribute(server: PluginServerContext) {
  server.handle(create, async ({ path }, { jagentdesk }) => {
    const workspace = await jagentdesk.workspaces.create({
  ...
  server.handle(list, async (_input, { jagentdesk }) => {
    const result = await jagentdesk.agents.list({ page: { limit: 100 } });
    return { agentIds: result.entries.map((entry) => entry.agent.id) };
  });
  server.handle(append, ({ agentId, status }, { jagentdesk }) =>
    jagentdesk.agents.ref(agentId).timeline.append({
      type: "plugin",
      id: "review-1",
      kind: "review",
      version: 1,
  ...
  try {
    ...
    const created = await client.invokePluginRpc("jagentdesk-api", "create", {
    ...
    const listed = await client.invokePluginRpc("jagentdesk-api", "list", {});
    expect(listed).toEqual({
      agentIds: expect.arrayContaining([Reflect.get(created, "agentId")]),
    });
    const agentId = Reflect.get(created, "agentId");
    await expect(
      client.invokePluginRpc("jagentdesk-api", "append", { agentId, status: "running" }),
    ).resolves.toEqual({ seq: expect.any(Number), epoch: expect.any(String) });
    await client.invokePluginRpc("jagentdesk-api", "append", { agentId, status: "complete" });
    ...
    expect(timeline.entries.filter((entry) => entry.item.type === "plugin")).toEqual([
      expect.objectContaining({
        item: expect.objectContaining({
          type: "plugin",
          id: "review-1",
          pluginId: "jagentdesk-api",
          data: { status: "complete" },
        }),
      }),
    ]);
    await client.removePlugin("jagentdesk-api");
...
test.skip("daemon config reload enables and disables configured plugins without restarting", async () => {
  const pluginDirectory = await mkdtemp(path.join(tmpdir(), "jagentdesk-reload-plugin-"));
  const jagentdeskHomeRoot = await mkdtemp(path.join(tmpdir(), "jagentdesk-reload-home-"));
  const jagentdeskHome = path.join(jagentdeskHomeRoot, ".jagentdesk");
  roots.push(pluginDirectory, jagentdeskHomeRoot);
  await writeFile(
    path.join(pluginDirectory, "jagentdesk-plugin.json"),
    JSON.stringify({
  ...
  const configPath = path.join(daemon.jagentdeskHome, "config.json");
...
test.skip("plugin host APIs and observations recover after repeated daemon-side socket closes", async () => {
  const pluginDirectory = await mkdtemp(path.join(tmpdir(), "jagentdesk-reconnecting-plugin-"));
  const workspaceDirectory = await mkdtemp(
    path.join(tmpdir(), "jagentdesk-reconnecting-workspace-"),
  );
  roots.push(pluginDirectory, workspaceDirectory);
  await writeFile(
    path.join(pluginDirectory, "jagentdesk-plugin.json"),
    JSON.stringify({ id: "reconnecting", requirements: { jagentdesk: ">=0.8.0" } }),
  );
  await writeFile(
    path.join(pluginDirectory, "index.server.ts"),
    `
...
export default function contribute(server: PluginServerContext) {
  ...
  server.handle(defineRpc({ name: "observe", input: z.object({}), output: z.string() }), async (_, { jagentdesk }) => {
    const { subscription } = await jagentdesk.workspaces.list({ subscribe: {} });
  ...
  server.handle(defineRpc({ name: "probe", input: z.object({}), output: z.object({ pid: z.number(), projectIds: z.array(z.string()) }) }), async (_, { jagentdesk }) => {
    await reconnected;
    return { pid: process.pid, projectIds: (await jagentdesk.projects.list()).projects.map((project) => project.projectId) };
  });
  ...
  server.handle(defineRpc({ name: "disconnect", input: z.object({}), output: z.null() }), () => new Promise((resolve) => {
    ...
    function closed(message: { type: string }) {
      if (message.type !== "jagentdesk_close") return;
    ...
    process.send!({ type: "jagentdesk_frame", isBinary: false, data: JSON.stringify({
