...
function resolveFilePath(jagentdeskHome: string, configuredPath: string | undefined): string {
  const fallback = path.join(jagentdeskHome, DEFAULT_DAEMON_LOG_FILENAME);
  if (!configuredPath) {
    return fallback;
  }
  ...
  return path.resolve(jagentdeskHome, configuredPath);
}
...
function resolveConfiguredJAgentDeskHome(options: ResolveLogConfigOptions | undefined): string {
  if (options?.jagentdeskHome) {
    return options.jagentdeskHome;
  }
  return resolveJAgentDeskHome();
}
...
export function resolveLogConfig(
  ...
  const jagentdeskHome = resolveConfiguredJAgentDeskHome(options);
  ...
  const file =
    options?.file !== false && persistedLog?.file
      ? {
          ...
          path: resolveFilePath(jagentdeskHome, persistedLog.file.path),
        }
      : undefined;
