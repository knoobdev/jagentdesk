...
// The daemon-level workspace contract that `jagentdesk run` depends on: each
...
// precedence (--workspace > $JAGENTDESK_WORKSPACE_ID > --worktree > bare) is covered
...
test("daemon resolves human and managed CLI workspace ownership", async () => {
  ...
  const cwd = mkdtempSync(path.join(tmpdir(), "jagentdesk-cli-run-cwd-"));
  const otherCwd = mkdtempSync(path.join(tmpdir(), "jagentdesk-cli-run-other-cwd-"));
  ...
  try {
    ...
    // $JAGENTDESK_WORKSPACE_ID land a run) creates no new workspace record: the
