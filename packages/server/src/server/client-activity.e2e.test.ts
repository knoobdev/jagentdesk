...
import type { AgentStreamEventPayload } from "@jagentdesk/protocol/messages";
...
describe("client activity tracking", () => {
  ...
  const TEST_CWD = "/tmp";
  let daemon: TestJAgentDeskDaemon;
  ...
  beforeEach(async () => {
    ...
    daemon = await createTestJAgentDeskDaemon({ pushNotificationSender: pushNotifications });
  });
