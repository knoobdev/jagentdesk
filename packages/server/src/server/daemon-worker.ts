...
import { createJAgentDeskDaemon } from "./bootstrap.js";
...
process.title = "JAgentDesk Daemon";

type SupervisorLifecycleMessage =
  | {
      type: "jagentdesk:shutdown";
      reason: string;
    }
  | {
      type: "jagentdesk:ready";
      listen: string;
    }
  | {
      type: "jagentdesk:restart";
      reason?: string;
    };
...
function bootstrapFromEnvironment(): BootstrapResult {
  try {
    ...
    const logger = createRootLogger({ log: config.log }, { jagentdeskHome, file: false });
    return { jagentdeskHome, logger, config };
  } catch (err) {
...
async function main() {
  const { jagentdeskHome, logger, config } = bootstrapFromEnvironment();
  let daemon: Awaited<ReturnType<typeof createJAgentDeskDaemon>> | null = null;
  ...
  const handleLifecycleIntent = (intent: DaemonLifecycleIntent) => {
    if (intent.type === "shutdown") {
      ...
      if (sendSupervisorLifecycleMessage({ type: "jagentdesk:shutdown", reason: intent.reason })) {
        return;
      }
  ...
  const installSupervisorLivenessGuard = () => {
    ...
    const exitAfterSupervisorLoss = (reason: string) => {
      ...
      writeWorkerLifecycleLog(jagentdeskHome, "Supervisor liveness lost; worker exiting", {
        reason,
  ...
  try {
    daemon = await createJAgentDeskDaemon(
      {
        ...config,
  ...
  try {
    ...
    sendSupervisorLifecycleMessage({ type: "jagentdesk:ready", listen });
  } catch (err) {
