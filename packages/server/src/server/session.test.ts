...
import type { WorkspaceDescriptorPayload } from "@jagentdesk/protocol/messages";
import {
...
interface SessionHandlerInternals {
  ...
  createJAgentDeskWorktree(params: unknown): Promise<unknown>;
...
const jagentdeskWorktreeServiceMocks = vi.hoisted(() => ({
  createJAgentDeskWorktree: vi.fn(),
}));
...
vi.mock("./jagentdesk-worktree-service.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("./jagentdesk-worktree-service.js")>();
  return {
    ...actual,
    createJAgentDeskWorktree: jagentdeskWorktreeServiceMocks.createJAgentDeskWorktree,
  };
});
    ...
    const session = createSessionForTest({
      ...
      workspaceGitService: {
        getCheckout: vi.fn(async (cwd: string) => ({
          ...
          remoteUrl: null,
          worktreeRoot: null,
          isJAgentDeskOwnedWorktree: false as const,
          mainRepoRoot: null,
        })),
      },
    });
...
describe("workspace file access (behavior preservation)", () => {
  ...
  test("file upload round-trips bytes through binary frames", async () => {
    const jagentdeskHome = makeDir("file-access-upload-");
    const messages: SessionOutboundMessage[] = [];
    const session = createSessionForTest({ messages, jagentdeskHome });
...
`;
  ...
  function writeConfig(repoRoot: string, config: unknown): void {
    writeFileSync(join(repoRoot, "jagentdesk.json"), `${JSON.stringify(config)}\n`);
  }

  async function generateCommitPromptWithConfig(config: unknown): Promise<string> {
    const repoRoot = makeRoot();
    if (typeof config === "string") {
      writeFileSync(join(repoRoot, "jagentdesk.json"), config);
  ...
  test.each([
    ["jagentdesk.json missing", undefined],
    ["jagentdesk.json exists but invalid JSON", "{ nope"],
    ["jagentdesk.json valid but missing metadataGeneration", {}],
  ...
  test("keeps the PR fallback when structured generation fails", async () => {
    ...
    expect(checkoutGitMocks.createPullRequest).toHaveBeenCalledWith(
      "/tmp/request-worktree",
      {
        ...
        body: "Automated PR generated by JAgentDesk.",
        base: "main",
      },
      expect.anything(),
    );
...
describe("session branch validation", () => {
  ...
  test("does not validate tags as branches", async () => {
    const tempDir = mkdtempSync(join(tmpdir(), "jagentdesk-session-branch-validation-"));
...
describe("session stash list handling", () => {
  test("lists stashes through the workspace git service", async () => {
    ...
    const entries = [
      {
        index: 0,
        message: "jagentdesk-auto-stash: feature",
        branch: "feature",
        isJAgentDesk: true,
      },
    ];
...
describe("session stash mutation handling", () => {
  test("forces a workspace git snapshot refresh after pushing a stash", async () => {
    ...
    expect(gitCommandMocks.runGitCommand).toHaveBeenCalledWith(
      ["stash", "push", "--include-untracked", "-m", "jagentdesk-auto-stash: feature"],
...
describe("session jagentdesk worktree creation handling", () => {
  test("forces workspace git refreshes for the source repo and created worktree", async () => {
    const workspaceGitService = { getSnapshot: vi.fn().mockResolvedValue({}) };
    const session = createSessionForTest({ workspaceGitService });
    jagentdeskWorktreeServiceMocks.createJAgentDeskWorktree.mockResolvedValue({
      repoRoot: "/tmp/repo",
      worktree: {
        branchName: "feature/new-worktree",
        worktreePath: "/tmp/jagentdesk/worktrees/new-worktree",
      },
      workspace: {
        workspaceId: "workspace-new-worktree",
        projectId: "project-repo",
        cwd: "/tmp/jagentdesk/worktrees/new-worktree",
        kind: "worktree",
    ...
    await asSessionInternals(session).createJAgentDeskWorktree({
      cwd: "/tmp/repo",
    ...
    expect(workspaceGitService.getSnapshot).toHaveBeenCalledWith(
      "/tmp/jagentdesk/worktrees/new-worktree",
      {
        force: true,
        reason: "create-worktree",
      },
...
describe("session workspace script handling", () => {
  test("passes the project slug and cached branch into workspace script spawning", async () => {
    ...
    const workspaceGitService = {
      ...
      getProjectSlug: vi.fn().mockResolvedValue("jagentdesk"),
    };
