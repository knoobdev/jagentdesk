...
async function createJAgentDeskHome(config: unknown): Promise<string> {
  const root = await mkdtemp(path.join(os.tmpdir(), "jagentdesk-config-web-ui-"));
  roots.push(root);
...
describe("daemon web UI config", () => {
  ...
  test("JAGENTDESK_WEB_UI_ENABLED overrides persisted setting", async () => {
    const home = await createJAgentDeskHome({
      version: 1,
      features: { webUi: { enabled: true } },
    });

    const config = loadConfig(home, { env: { JAGENTDESK_WEB_UI_ENABLED: "false" } });

    expect(config.webUi.enabled).toBe(false);
  });

  test("JAGENTDESK_WEB_UI_ENABLED=true enables web UI", async () => {
    const home = await createJAgentDeskHome({ version: 1 });

    const config = loadConfig(home, { env: { JAGENTDESK_WEB_UI_ENABLED: "true" } });
  ...
  test("CLI web UI enable override wins over env and persisted config", async () => {
    ...
    const config = loadConfig(home, {
      env: { JAGENTDESK_WEB_UI_ENABLED: "false" },
  ...
  test("CLI web UI disable override wins over env and persisted config", async () => {
    ...
    const config = loadConfig(home, {
      env: { JAGENTDESK_WEB_UI_ENABLED: "true" },
  ...
  test("resolves JAGENTDESK_WEB_UI_DIST_DIR as absolute path", async () => {
    const home = await createJAgentDeskHome({ version: 1 });
    const distDir = path.join(os.tmpdir(), "jagentdesk-web-ui-dist");

    const config = loadConfig(home, { env: { JAGENTDESK_WEB_UI_DIST_DIR: distDir } });

    expect(config.webUi.distDir).toBe(path.resolve(distDir));
  });

  test("resolves relative persisted distDir against JAGENTDESK_HOME", async () => {
    const home = await createJAgentDeskHome({
      version: 1,
  ...
  test("JAGENTDESK_WEB_UI_DIST_DIR overrides persisted distDir", async () => {
    const home = await createJAgentDeskHome({
      version: 1,
    ...
    const config = loadConfig(home, { env: { JAGENTDESK_WEB_UI_DIST_DIR: envDir } });
