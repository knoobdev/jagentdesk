...
async function createJAgentDeskHome(config: unknown): Promise<string> {
  const root = await mkdtemp(path.join(os.tmpdir(), "jagentdesk-config-auth-"));
  roots.push(root);
...
describe("daemon auth config", () => {
  ...
  test("loads optional auth password hash from config.json", async () => {
    const jagentdeskHome = await createJAgentDeskHome({
      version: 1,
      daemon: {
  ...
  test("lets JAGENTDESK_PASSWORD override config.json auth password hash", async () => {
    const jagentdeskHome = await createJAgentDeskHome({
      version: 1,
      daemon: {
        auth: { password: CONFIG_PASSWORD_HASH },
    ...
    const config = loadConfig(jagentdeskHome, {
      env: { JAGENTDESK_PASSWORD: "from-env" },
    });
