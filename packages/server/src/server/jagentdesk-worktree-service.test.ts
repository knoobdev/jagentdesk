...
import {
  ...
  type CreateJAgentDeskWorktreeDeps,
} from "./jagentdesk-worktree-service.js";
import { readJAgentDeskWorktreeMetadata } from "../utils/worktree-metadata.js";
import { createWorktree, getJAgentDeskWorktreesRoot } from "../utils/worktree.js";
...
test("creates a worktree and registers it in the source workspace project without git snapshot lookup", async () => {
  ...
  const result = await createJAgentDeskWorktree(
    {
      cwd: repoDir,
...
test("seeds an uncommitted exact-project config into the mapped worktree directory", async () => {
  ...
  writeFileSync(path.join(sourceDir, "jagentdesk.json"), config);
  ...
  expect(readFileSync(path.join(result.workspace.cwd, "jagentdesk.json"), "utf8")).toBe(config);
  expect(existsSync(path.join(result.worktree.worktreePath, "jagentdesk.json"))).toBe(false);
});
...
test("removes a new worktree when its ref does not contain the selected project directory", async () => {
  ...
  const worktreePath = path.join(
    await getJAgentDeskWorktreesRoot(repoDir, jagentdeskHome),
    "missing-subproject",
  );
...
test("maps a nested cwd from an existing JAgentDesk worktree into the next worktree", async () => {
  const { repoDir, tempDir } = createGitRepo();
  cleanupPaths.push(tempDir);
  const jagentdeskHome = path.join(tempDir, ".jagentdesk");
  ...
  commitAll(repoDir, "add subproject");
  const deps = createDeps();
  const source = await createJAgentDeskWorktree(
    {
      cwd: repoDir,
  ...
  const created = await createJAgentDeskWorktree(
    {
      cwd: sourceCwd,
...
test("rejects source checkout planning before creating a worktree", async () => {
  ...
  expect(existsSync(path.join(jagentdeskHome, "worktrees"))).toBe(false);
...
test.skipIf(isPlatform("win32"))(
  ...
  async () => {
    ...
    const first = await createJAgentDeskWorktree(
      {
        cwd: repoDir,
    ...
    const second = await createJAgentDeskWorktree(
      {
        cwd: repoDir,
        worktreeSlug: "reuse-me",
        runSetup: false,
...
test("renames an eligible unnamed branch-off worktree once on first agent context", async () => {
  ...
  expect(readJAgentDeskWorktreeMetadata(created.worktree.worktreePath)).toMatchObject({
    version: 2,
...
test("does not mutate registries or broadcast when core worktree creation fails", async () => {
  const tempDir = mkdtempSync(path.join(tmpdir(), "jagentdesk-worktree-service-"));
  cleanupPaths.push(tempDir);
  const deps = createDeps();
...
// Worktree restore (Unit 3): recreate a deleted JAgentDesk-owned worktree from its
...
// repoRoot=null, so deleteJAgentDeskWorktree's `git worktree remove`/`prune` is
...
interface TestDeps extends CreateJAgentDeskWorktreeDeps {
...
function createWorkspaceGitSnapshot(cwd: string): WorkspaceGitRuntimeSnapshot {
  ...
  return {
    ...
    git: {
      ...
      currentBranch,
      remoteUrl: null,
      isJAgentDeskOwnedWorktree: repoRoot !== mainRepoRoot,
      isDirty: false,
      baseRef: "main",
      aheadBehind: null,
      aheadOfOrigin: null,
