...
function createCheckoutFacts(
  ...
  return {
    ...
    jagentdeskWorktree: { isJAgentDeskOwnedWorktree: false },
    storedBaseRef: null,
...
interface CreateServiceOptions {
  ...
  listJAgentDeskWorktrees?: ReturnType<typeof vi.fn>;
  github?: ForgeService;
...
function buildDefaultServiceDeps() {
  return {
    ...
    listJAgentDeskWorktrees: vi.fn(async () => []),
  ...
  test("attributes Git commands submitted by a workspace refresh", async () => {
    ...
    execFileSync(
      ...
      [
        ...
        "-c",
        "user.name=JAgentDesk Test",
        "-c",
        "user.email=jagentdesk@example.test",
        "commit",
        "-m",
        "initial",
      ],
  ...
  test("GitHub self-heal polling uses the fork PR head branch instead of the owner-prefixed local branch", async () => {
    ...
    const getCheckoutSnapshotFacts = vi.fn(async (cwd: string) =>
      createCheckoutFacts(cwd, {
        ...
        branchRemoteName: "jagentdesk-pr-1285",
...
describe("WorkspaceGitServiceImpl D2 read methods", () => {
  ...
  test("listStashes cold-loads, warms, forces, and coalesces per cwd", async () => {
    let nowMs = 0;
    const stashOutput = "stash@{0}\u0000jagentdesk-auto-stash: feature\n";
    ...
    const first = service.listStashes(REPO_CWD, { jagentdeskOnly: true });
    const second = service.listStashes(join(REPO_CWD, "."), { jagentdeskOnly: true });
    await flushPromises();
    ...
    await expect(Promise.all([first, second])).resolves.toEqual([
      [{ index: 0, message: "jagentdesk-auto-stash: feature", branch: "feature", isJAgentDesk: true }],
      [{ index: 0, message: "jagentdesk-auto-stash: feature", branch: "feature", isJAgentDesk: true }],
    ]);

    nowMs = 1_000;
    await service.listStashes(REPO_CWD, { jagentdeskOnly: true });
    expect(runGitCommand).toHaveBeenCalledTimes(1);

    await service.listStashes(REPO_CWD, { jagentdeskOnly: true }, { force: true, reason: "test" });
    expect(runGitCommand).toHaveBeenCalledTimes(2);
  ...
  test("listWorktrees cold-loads, warms, forces, and coalesces per repo root", async () => {
    ...
    const listJAgentDeskWorktrees = vi.fn().mockResolvedValue(worktrees);
    const service = createService({
    ...
    expect(listJAgentDeskWorktrees).toHaveBeenCalledTimes(1);
    ...
    expect(listJAgentDeskWorktrees).toHaveBeenCalledTimes(2);
  ...
  test("listWorktrees shares one repo-root scoped read across sibling workspace cwds", async () => {
    ...
    const worktrees = [
      {
        path: join(tempDir, "jagentdesk-home", "worktrees", "repo", "feature"),
        createdAt: "2026-04-12T00:00:00.000Z",
        branchName: "feature",
      },
    ];
    const listJAgentDeskWorktrees = vi.fn(async () => worktrees);
    const service = createService({
      getCheckoutSnapshotFacts: getCheckoutSnapshotFactsUncached as never,
      getCheckoutStatus: getCheckoutStatusUncached as never,
      listJAgentDeskWorktrees,
    ...
    try {
      ...
      expect(listJAgentDeskWorktrees).toHaveBeenCalledTimes(1);
      expect(listJAgentDeskWorktrees).toHaveBeenCalledWith({
  ...
  test("getProjectSlug derives the slug from the snapshot cache", async () => {
    ...
    await expect(service.getProjectSlug(REPO_CWD)).resolves.toBe("jagentdesk");
