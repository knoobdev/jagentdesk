...
import { listJAgentDeskWorktrees, type JAgentDeskWorktreeInfo } from "../utils/worktree.js";
...
export interface WorkspaceGitRuntimeSnapshot {
  ...
  git: {
    ...
    isJAgentDeskOwnedWorktree: boolean;
...
export type WorkspaceGitWorktreeInfo = JAgentDeskWorktreeInfo;
...
interface WorkspaceGitServiceDependencies {
  ...
  listJAgentDeskWorktrees: typeof listJAgentDeskWorktrees;
  /**
...
export class WorkspaceGitServiceImpl implements WorkspaceGitService {
  ...
  async getCheckout(cwd: string): Promise<ProjectCheckoutLitePayload> {
    ...
    return checkoutLiteFromGitSnapshot(normalizedCwd, {
      ...
      isJAgentDeskOwnedWorktree: status.isJAgentDeskOwnedWorktree,
  ...
  listStashes(
    ...
    const normalizedCwd = resolve(cwd);
    const jagentdeskOnly = options?.jagentdeskOnly !== false;
    const key = JSON.stringify(["stashes", normalizedCwd, jagentdeskOnly]);
    return this.readAuxiliaryCache(this.stashListCache, key, readOptions, async () => {
      ...
      return parseWorkspaceGitStashList(stdout, { jagentdeskOnly });
    });
  }
  ...
  async listWorktrees(
    ...
    return this.readAuxiliaryCache(this.worktreeListCache, key, options, () =>
      this.deps.listJAgentDeskWorktrees({
        cwd: repoRoot,
        jagentdeskHome: this.jagentdeskHome,
        worktreesRoot: this.worktreesRoot,
      }),
  ...
  async resolveRepoRoot(cwd: string, options?: WorkspaceGitReadOptions): Promise<string> {
    ...
    return snapshot.git.isJAgentDeskOwnedWorktree
  ...
  private async refreshGitSnapshot(
    ...
    target.latestGit = {
      ...
      isJAgentDeskOwnedWorktree: checkoutStatus.isJAgentDeskOwnedWorktree,
...
function parseWorkspaceGitStashList(
  ...
  for (const line of lines) {
    ...
    const prefix = "jagentdesk-auto-stash:";
    const prefixIdx = subject.indexOf(prefix);
    const isJAgentDesk = prefixIdx >= 0;
    const branch = isJAgentDesk ? subject.slice(prefixIdx + prefix.length).trim() || null : null;

    if (options.jagentdeskOnly && !isJAgentDesk) {
      continue;
    }

    entries.push({ index, message: subject, branch, isJAgentDesk });
  }
