...
const CWD = "/tmp/jagentdesk/worktrees/repo/branch";
const JAGENTDESK_HOME = "/tmp/jagentdesk";
const WORKTREES_ROOT = "/tmp/jagentdesk/worktrees/repo";
...
function createHarness(overrides?: {
  ...
  isJAgentDeskOwnedWorktreeCwd?: ArchiveIfSafeDependencies["isJAgentDeskOwnedWorktreeCwd"];
  ...
  const options: AutoArchiveArchiveOptions = {
    jagentdeskHome: JAGENTDESK_HOME,
    daemonConfigStore: {
      get: getConfig,
  ...
  const isJAgentDeskOwnedWorktreeCwd = vi.fn(
    overrides?.isJAgentDeskOwnedWorktreeCwd ??
      (async () => ({
        allowed: true,
        repoRoot: "/tmp/repo",
  ...
  ) as unknown as ArchiveIfSafeDependencies["isJAgentDeskOwnedWorktreeCwd"];
...
describe("archiveIfSafe", () => {
  ...
  test("does nothing when there is no snapshot", async () => {
    ...
    expect(harness.deps.isJAgentDeskOwnedWorktreeCwd).not.toHaveBeenCalled();
    expect(harness.deps.archiveByScope).not.toHaveBeenCalled();
  });
  ...
  test("does nothing when the cwd is not a JAgentDesk-owned worktree", async () => {
    const harness = createHarness({
      isJAgentDeskOwnedWorktreeCwd: async () => ({ allowed: false, worktreePath: CWD }),
    });

    await runArchiveIfSafe(harness);

    expect(harness.deps.isJAgentDeskOwnedWorktreeCwd).toHaveBeenCalledWith(CWD, {
      jagentdeskHome: JAGENTDESK_HOME,
    });
    expect(harness.deps.archiveByScope).not.toHaveBeenCalled();
  });
  ...
  test("archives a clean JAgentDesk-owned worktree after merge", async () => {
    const harness = createHarness();
  ...
  test("real outcome: keeps sibling workspace and directory on last reference", async () => {
    ...
    const worktree = await createJAgentDeskOwnedWorktree(repoDir, jagentdeskHome, "merged-with-sibling");
  ...
  test("real outcome: removes directory when no sibling workspace remains", async () => {
    const { tempDir, repoDir } = createGitRepo();
    const jagentdeskHome = path.join(tempDir, ".jagentdesk");
    const worktree = await createJAgentDeskOwnedWorktree(repoDir, jagentdeskHome, "merged-last-ref");
