...
describe("daemon web UI route module", () => {
  ...
  beforeEach(async () => {
    tempRoot = await mkdtemp(path.join(os.tmpdir(), "jagentdesk-web-ui-"));
  ...
  test("serves index.html with injected initial connection hint", async () => {
    ...
    expect(res.body).toContain("window.__JAGENTDESK_INITIAL_DAEMON_CONNECTION__");
  ...
  test("injects hint before closing head tag", async () => {
    ...
    expect(res.body).toMatch(/window\.__JAGENTDESK_INITIAL_DAEMON_CONNECTION__.*<\/head>/);
  });
