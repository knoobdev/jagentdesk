...
import { handleCreateJAgentDeskWorktreeRequest } from "./worktree-session.js";
...
describe("wire compatibility", () => {
  ...
  test("legacy worktree request shape normalizes to the same internal input as the new shape", async () => {
    ...
    const dependencies = {
      ...
      createJAgentDeskWorktreeWorkflow: workflow.create.bind(workflow),
    };
      ...
      attachments: [
        {
          ...
          url: "https://github.com/jagentdesk/jagentdesk/issues/55",
        },
      ],
    });
    ...
    if (legacyRequest.type !== "create_jagentdesk_worktree_request") {
      throw new Error("Expected legacy worktree request");
    }
    if (newRequest.type !== "create_jagentdesk_worktree_request") {
      throw new Error("Expected new worktree request");
    }

    await handleCreateJAgentDeskWorktreeRequest(dependencies, legacyRequest);
    await handleCreateJAgentDeskWorktreeRequest(dependencies, newRequest);
