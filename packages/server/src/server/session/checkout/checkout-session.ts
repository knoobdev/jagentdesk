...
export class CheckoutSession {
  private static readonly JAGENTDESK_STASH_PREFIX = "jagentdesk-auto-stash:";
  ...
  async handleStashSaveRequest(
    ...
    try {
      ...
      const message = branchLabel
        ? `${CheckoutSession.JAGENTDESK_STASH_PREFIX} ${branchLabel}`
        : `${CheckoutSession.JAGENTDESK_STASH_PREFIX} unnamed`;
  ...
  async handleStashListRequest(
    ...
    const { cwd, requestId } = msg;
    const jagentdeskOnly = msg.jagentdeskOnly !== false;
    try {
      const entries = await this.workspaceGitService.listStashes(cwd, { jagentdeskOnly });
  ...
  async handleCheckoutMergeRequest(
    ...
    try {
      ...
      const mutatedCwd = await mergeToBase(
        ...
        { jagentdeskHome: this.jagentdeskHome, worktreesRoot: this.worktreesRoot },
      );
      await Promise.all([
