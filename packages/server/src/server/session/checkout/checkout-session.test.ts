...
describe("CheckoutSession", () => {
  ...
  describe("stash list", () => {
    it("returns stash entries scoped to jagentdesk stashes by default", async () => {
      const listStashesCalls: Array<{ cwd: string; jagentdeskOnly: boolean | undefined }> = [];
      const { checkout, emitted } = makeCheckoutSession({
        git: {
          listStashes: async (cwd, opts) => {
            listStashesCalls.push({ cwd, jagentdeskOnly: opts?.jagentdeskOnly });
            return [];
          },
        },
      });
      ...
      expect(listStashesCalls).toEqual([{ cwd: "/repo", jagentdeskOnly: true }]);
      expect(emitted).toEqual([
        {
