...
export async function collectDaemonDiagnostics(options: DaemonDiagnosticsOptions): Promise<string> {
  const sections: string[] = [
    formatSection("JAgentDesk diagnostics", [
...
function collectProcessEntries(options: DaemonDiagnosticsOptions): DiagnosticEntry[] {
  ...
  return [
    ...
    { label: "JAgentDesk home", value: options.jagentdeskHome },
...
function collectRuntimeConfigEntries(options: DaemonDiagnosticsOptions): DiagnosticEntry[] {
  const tailnetAddress = options.daemonRuntimeConfig?.getTailnetAddress() ?? null;
  return [
    { label: "Listen", value: formatListenKind(options.daemonRuntimeConfig?.listen ?? null) },
    {
      label: "Tailnet address",
      value: tailnetAddress ? `${tailnetAddress.host}:${tailnetAddress.port}` : "not configured",
    },
    {
      label: "Tailnet TLS",
      value: tailnetAddress ? String(tailnetAddress.useTls ?? false) : "n/a",
    },
  ];
}
...
async function collectDiskEntries(options: DaemonDiagnosticsOptions): Promise<DiagnosticEntry[]> {
  const stats = await statfs(options.jagentdeskHome);
  const freeBytes = stats.bavail * stats.bsize;
  const totalBytes = stats.blocks * stats.bsize;
  return [
    { label: "Path", value: options.jagentdeskHome },
...
async function safeLogTailSection(options: DaemonDiagnosticsOptions): Promise<string> {
  const logPath = path.join(options.jagentdeskHome, "daemon.log");
  try {
...
export function redactDiagnostic(
  ...
  let redacted = value;
  const tailnetAddress = options?.daemonRuntimeConfig?.getTailnetAddress();
  const sensitiveValues = [
    options?.daemonRuntimeConfig?.listen,
    tailnetAddress ? `${tailnetAddress.host}:${tailnetAddress.port}` : undefined,
  ...
  return redacted
    .replace(/jagentdesk:\/\/\S+/gi, "jagentdesk://[redacted]")
    .replace(
      /([?&](?:password|token|secret|key|publicKey|daemonPublicKeyB64)=)[^&\s"']+/gi,
      "$1[redacted]",
    )
    .replace(
      /((?:password|token|secret|authorization|api[_-]?key|daemonPublicKeyB64)\s*[:=]\s*)("[^"]+"|'[^']+'|[^\s,}]+)/gi,
      "$1[redacted]",
    );
}
