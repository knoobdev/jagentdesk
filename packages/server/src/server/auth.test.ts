...
describe("daemon bearer validator", () => {
  ...
  test("extracts WebSocket JAgentDesk bearer subprotocol tokens", () => {
    const protocol = extractWsBearerProtocol("chat, jagentdesk.bearer.secret.with.dots");

    expect(protocol).toBe("jagentdesk.bearer.secret.with.dots");
    expect(extractWsBearerToken(protocol)).toBe("secret.with.dots");
    expect(extractWsBearerToken("jagentdesk.other.secret")).toBeNull();
  });
