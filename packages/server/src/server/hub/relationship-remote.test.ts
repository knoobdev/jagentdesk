...
const openJAgentDeskHomes: string[] = [];
...
afterEach(async () => {
  ...
  await Promise.all(openJAgentDeskHomes.splice(0).map((home) => rm(home, { recursive: true })));
});
...
async function connectController(
  ...
  const jagentdeskHome = await mkdtemp(path.join(tmpdir(), "jagentdesk-hub-socket-"));
  openJAgentDeskHomes.push(jagentdeskHome);
