...
export class DaemonExecutions implements HubExecutionAgents {
  ...
  private async createOrResolve(
    ...
    let createdWorktree: CreateJAgentDeskWorktreeWorkflowResult | null = null;
  ...
  private async controlOwnedExecution(
    ...
    if (workspace?.isJAgentDeskOwnedWorktree) {
      this.requireAuthority(authorityGeneration, "execution control");
...
function requireHubMcpNamespace(mcpServers: Record<string, McpServerConfig> | undefined): void {
  if (mcpServers && Object.hasOwn(mcpServers, "jagentdesk")) {
    throw new Error('Hub execution MCP server name "jagentdesk" is reserved by the daemon');
  }
}

function ownedCreatedWorktree(
  worktree: CreateJAgentDeskWorktreeWorkflowResult | null,
): CreateJAgentDeskWorktreeWorkflowResult | null {
