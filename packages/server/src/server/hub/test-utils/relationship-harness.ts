...
import { createJAgentDeskDaemon, type JAgentDeskDaemon, type JAgentDeskDaemonConfig } from "../../bootstrap.js";
...
export class HubRelationshipHarness {
  ...
  private daemon: JAgentDeskDaemon | null = null;
  private config!: JAgentDeskDaemonConfig;
  private root = "";
  private jagentdeskHome = "";
  private host = "";
  ...
  async relationshipStateBecomes(expected: string | null): Promise<void> {
    ...
    const watcher = watch(this.jagentdeskHome, () => {
  ...
  async durableOwnedAgentIdsOnDisk(): Promise<string[]> {
    const storage = new AgentStorage(
      path.join(this.jagentdeskHome, "agents"),
      pino({ level: "silent" }),
    );
  ...
  async hubExecutionIntentFiles(): Promise<string[]> {
    const directory = path.join(this.jagentdeskHome, "hub-executions");
  ...
  private workspaceArchivedAt(workspaceId: string): string | null {
    const records = JSON.parse(
      readFileSync(path.join(this.jagentdeskHome, "projects", "workspaces.json"), "utf8"),
  ...
  relationshipFile(): PersistedRelationship | null {
    const file = path.join(this.jagentdeskHome, "hub-relationship.json");
    if (!existsSync(file)) return null;
    return JSON.parse(readFileSync(file, "utf8")) as PersistedRelationship;
  }

  relationshipFileMode(): number {
    return statSync(path.join(this.jagentdeskHome, "hub-relationship.json")).mode & 0o777;
  }

  async corruptRelationshipFile(contents = "{not-json"): Promise<void> {
    await this.stopDaemon();
    await writeFile(path.join(this.jagentdeskHome, "hub-relationship.json"), contents, "utf8");
  }

  async quarantinedRelationshipFiles(): Promise<string[]> {
    return (await readdir(this.jagentdeskHome)).filter((file) =>
  ...
  private async createHome(): Promise<void> {
    this.root = await mkdtemp(path.join(tmpdir(), "jagentdesk-hub-relationship-"));
    this.jagentdeskHome = path.join(this.root, ".jagentdesk");
    const staticDir = path.join(this.root, "static");
    await Promise.all([mkdir(this.jagentdeskHome, { recursive: true }), mkdir(staticDir)]);
  ...
  private async startDaemon(): Promise<void> {
    ...
    this.daemon = await createJAgentDeskDaemon(this.config, pino({ level: "trace" }, destination), {
