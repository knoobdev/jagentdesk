...
test("Hub MCP configuration reaches the provider alongside JAgentDesk MCP without entering snapshots", async () => {
...
test("new Hub executions cannot override the daemon-owned JAgentDesk MCP server", async () => {
  const hub = await launchRelationship();
  hub.beginOwnedCreate("reserved-mcp-create", "reserved-mcp-execution", {
    mcpServers: {
      jagentdesk: { type: "http", url: "https://hub.test/replace-jagentdesk" },
    },
  });
...
test("reserved JAgentDesk MCP input does not invalidate replay of an owned execution", async () => {
  const hub = await launchRelationship();
