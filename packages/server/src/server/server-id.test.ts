...
function tmpHome(): string {
  return mkdtempSync(path.join(tmpdir(), "jagentdesk-server-id-"));
}
...
describe("getOrCreateServerId", () => {
  ...
  beforeEach(() => {
    ...
    delete process.env.JAGENTDESK_SERVER_ID;
    home = tmpHome();
  });
  ...
  it("creates and persists a stable id per JAGENTDESK_HOME", () => {
  ...
  it("respects and persists JAGENTDESK_SERVER_ID override", () => {
    process.env.JAGENTDESK_SERVER_ID = "test-daemon-id";
