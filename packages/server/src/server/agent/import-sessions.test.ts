...
import type { FetchRecentProviderSessionsRequestMessage } from "@jagentdesk/protocol/messages";
...
test("listImportableProviderSessions includes a provider session after its JAgentDesk agent is archived", async () => {
  const cwd = "/tmp/project";
...
test("listImportableProviderSessions keeps realpath-equivalent cwd matches", async () => {
  const root = mkdtempSync(path.join(tmpdir(), "jagentdesk-import-cwd-"));
