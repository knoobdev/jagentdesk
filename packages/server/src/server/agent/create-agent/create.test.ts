...
import type { CreateJAgentDeskWorktreeWorkflowResult } from "../../worktree-session.js";
...
function fakeWorktreeCreator(args: { repoRoot: string; createdWorkspaceId: string }) {
  ...
  return async (): Promise<CreateJAgentDeskWorktreeWorkflowResult> =>
    ({
    ...
    }) as unknown as CreateJAgentDeskWorktreeWorkflowResult;
}
...
test("session create forwards clientMessageId to the initial prompt run options", async () => {
  const snapshot = {
    id: "agent-1",
    provider: "codex",
    cwd: "/tmp/jagentdesk-create-test",
    runtimeInfo: null,
  } as ManagedAgent;
  ...
  await createAgentCommand(dependencies, {
    kind: "session",
    config: { provider: "codex", cwd: "/tmp/jagentdesk-create-test" },
...
test("session create validates the requested mode against the provider's modes", async () => {
  ...
  await expect(
    createAgentCommand(dependencies, {
      kind: "session",
      config: { provider: "opencode", cwd: "/tmp/jagentdesk-create-test", modeId: "plan" },
      workspaceId: "ws-create-test",
      labels: {},
      provisionalTitle: null,
      firstAgentContext: { attachments: [] },
...
test("session create applies the resolved mode from the provider create config", async () => {
  ...
  await createAgentCommand(dependencies, {
    kind: "session",
    config: { provider: "opencode", cwd: "/tmp/jagentdesk-create-test", modeId: "build" },
    workspaceId: "ws-create-test",
    labels: {},
    provisionalTitle: null,
    firstAgentContext: { attachments: [] },
    ...
    const { snapshot: child } = await createAgentCommand(
      {
        ...
        logger,
        providerSnapshotManager,
        createJAgentDeskWorktree: fakeWorktreeCreator({
          repoRoot: workdir,
          createdWorkspaceId: "ws-new-worktree",
        }),
      },
    ...
    await createAgentCommand(
      {
        ...
        createJAgentDeskWorktree: async () => createdWorktree,
      },
      {
        kind: "mcp",
        provider: "codex",
