...
import { getOpenAgentTabLabel, PARENT_AGENT_ID_LABEL } from "@jagentdesk/protocol/agent-labels";
  ...
  override async createSession(
    ...
    const script = `
      process.stdout.write(JSON.stringify({
        ...
        agentId: process.env.JAGENTDESK_AGENT_ID ?? null
      }));
    `;
...
test("createAgent passes daemon launch env through the provider launch context", async () => {
  ...
  expect(client.lastLaunchContext).toEqual({
    agentId: snapshot.id,
    env: {
      JAGENTDESK_AGENT_ID: snapshot.id,
      JAGENTDESK_AGENT_CWD: workdir,
    },
  });
});
...
test("createAgent injects jagentdesk MCP server only into provider launch config", async () => {
  const workdir = mkdtempSync(join(tmpdir(), "agent-manager-test-"));
  const storagePath = join(workdir, "agents");
  const storage = new AgentStorage(storagePath, logger);
...
test("createAgent passes native JAgentDesk tools through launch context without internal MCP", async () => {
  const workdir = mkdtempSync(join(tmpdir(), "agent-manager-test-"));
  const storagePath = join(workdir, "agents");
  const storage = new AgentStorage(storagePath, logger);

  const jagentdeskTools: JAgentDeskToolCatalog = {
    tools: new Map(),
    getTool: () => undefined,
    executeTool: async () => {
  ...
  class NativeToolsClient extends TestAgentClient {
    override readonly capabilities = {
      ...TEST_CAPABILITIES,
      supportsMcpServers: true,
      supportsNativeJAgentDeskTools: true,
    };
    lastConfig: AgentSessionConfig | null = null;
    lastLaunchContext: AgentLaunchContext | undefined;
  ...
  const manager = new AgentManager({
    ...
    logger,
    mcpBaseUrl: "http://127.0.0.1:6767/mcp/agents",
    jagentdeskToolCatalogFactory: () => jagentdeskTools,
  ...
  expect(client.lastLaunchContext?.jagentdeskTools).toBe(jagentdeskTools);
  expect(client.lastConfig?.mcpServers).toEqual({
    custom: {
      type: "stdio",
      command: "custom-mcp",
...
test("createAgent allows best-effort internal MCP when the provider session reports no support", async () => {
  ...
  expect(client.lastConfig?.mcpServers?.jagentdesk).toEqual({
    type: "http",
    url: `http://127.0.0.1:6767/mcp/agents?callerAgentId=${snapshot.id}`,
...
test("resumeAgentFromPersistence replaces stored internal jagentdesk MCP with current runtime URL", async () => {
  const workdir = mkdtempSync(join(tmpdir(), "agent-manager-test-"));
  const storagePath = join(workdir, "agents");
  const storage = new AgentStorage(storagePath, logger);
...
test("resumeAgentFromPersistence drops stored internal jagentdesk MCP when runtime injection is disabled", async () => {
  const workdir = mkdtempSync(join(tmpdir(), "agent-manager-test-"));
  const storagePath = join(workdir, "agents");
  const storage = new AgentStorage(storagePath, logger);
  const client = new TestAgentClient();
...
test("createAgent preserves a user-provided jagentdesk MCP config", async () => {
  const workdir = mkdtempSync(join(tmpdir(), "agent-manager-test-"));
  const storagePath = join(workdir, "agents");
  const storage = new AgentStorage(storagePath, logger);
  ...
  const snapshot = await manager.createAgent(
    {
      ...
      mcpServers: {
        jagentdesk: {
          type: "http",
          url: "https://example.com/custom-jagentdesk",
        },
      },
    },
    undefined,
...
test("resumeAgentFromPersistence keeps metadata config, applies overrides, and passes launch env", async () => {
  ...
  const resumed = await manager.resumeAgentFromPersistence(handle, {
    ...
    mcpServers: {
      jagentdesk: {
        type: "stdio",
        command: "node",
        args: ["/tmp/mcp-bridge.mjs", "--socket", "/tmp/jagentdesk.sock"],
      },
    },
  });
  ...
  expect(client.lastResumeLaunchContext).toEqual({
    agentId: resumed.id,
    env: {
      JAGENTDESK_AGENT_ID: resumed.id,
      JAGENTDESK_AGENT_CWD: workdir,
    },
  });
});
...
test("importProviderSession imports the selected session without listing and publishes ready state", async () => {
  ...
  expect(client.importLaunchContext).toEqual({
    agentId: imported.id,
    env: {
      JAGENTDESK_AGENT_ID: imported.id,
      JAGENTDESK_AGENT_CWD: workdir,
    },
  });
...
test("user_message events wrapping a jagentdesk-system envelope are not added to the timeline", async () => {
...
test("user_message events wrapping a jagentdesk-system envelope are not restored during history replay", async () => {
