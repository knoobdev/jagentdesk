...
describe("codex tool-call mapper", () => {
  ...
  it("normalizes codex jagentdesk speak mcp calls and extracts spoken text", () => {
    const item = expectMapped(
      mapCodexToolCallFromThreadItem({
        type: "mcpToolCall",
  ...
  it("normalizes codex jagentdesk_voice.speak mcp calls and extracts spoken text", () => {
    const item = expectMapped(
      mapCodexToolCallFromThreadItem({
        type: "mcpToolCall",
        id: "codex-speak-thread-2",
        status: "completed",
        server: "jagentdesk_voice",
        tool: "speak",
        arguments: { text: "Voice response from Codex via jagentdesk_voice." },
        result: { ok: true },
      }),
    );
    ...
    expect(item.detail).toEqual({
      type: "unknown",
      input: "Voice response from Codex via jagentdesk_voice.",
      output: null,
    });
  });

  it("normalizes codex jagentdesk speak tool-call names and extracts spoken text", () => {
    const item = expectMapped(
      mapCodexToolCallEnvelope({
        callId: "codex-speak-tool-call-1",
        name: "jagentdesk.speak",
