...
async function startPublicSteeringSession(
...
): Promise<{ session: AgentSession; jagentdeskTurnId: string }> {
  const session = new CodexAppServerAgentSession(
  ...
  return { session, jagentdeskTurnId: started.turnId };
}
...
describe("Codex active-turn steering admission", () => {
  test("a steer without the clearing contract leaves permissions open", async () => {
    ...
    const { session, jagentdeskTurnId } = await startPublicSteeringSession(appServer);
    castInternals<{ emitSyntheticPlanApprovalRequest: (planText: string) => void }>(
      session,
    ).emitSyntheticPlanApprovalRequest("Ship the thing");

    await expect(
      session.steerActiveTurn!("background notification", { expectedTurnId: jagentdeskTurnId }),
  ...
  test("does not steer B when A completes while command resolution is pending", async () => {
    ...
    expect(startedB.turnId).not.toBe(jagentdeskTurnId);
...
const capturePath = process.env.JAGENTDESK_FAKE_CODEX_CAPTURE;
let buffer = "";
    ...
    providerOverrides: {
      [providerId]: {
        ...
        env: {
          ...
          JAGENTDESK_FAKE_CODEX_CAPTURE: capturedRequestsPath,
        },
      },
    },
  });
...
describe("Codex app-server provider", () => {
  ...
  test("initializes Codex app-server without making JAgentDesk the request originator", async () => {
  ...
  test("unarchives Codex when an active JAgentDesk agent resumes an archived thread", async () => {
    const threadRequests: string[] = [];
          ...
          data: [
            {
              ...
              skills: [
                {
                  name: "jagentdesk-implement",
                  description: "Execute an existing JAgentDesk plan.",
                  path: "/tmp/skills/jagentdesk-implement/SKILL.md",
                },
              ],
              errors: [],
            },
    ...
    expect(turnStartCall?.[1]).toEqual(
      expect.objectContaining({
        input: [
          ...
          {
            type: "text",
            text: "$jagentdesk-implement in a worktree, remember to use Claude for the UI",
            text_elements: [],
          },
        ],
      }),
    ...
    const commands = await listCommandsFromFakeCodex([
      {
        ...
        path: "/Users/test/.agents/skills/jagentdesk/SKILL.md",
      },
      {
        name: "jagentdesk",
        description: "Shared orchestration skill.",
        path: "/Users/test/.codex/skills/jagentdesk/SKILL.md",
      },
    ]);

    expect(commands.filter((command) => command.name === "jagentdesk")).toEqual([
      {
        name: "jagentdesk",
        description: "Shared orchestration skill.",
        argumentHint: "",
    ...
    const input = await codexAppServerTurnInputFromPrompt(
      [
        { type: "text", text: "Please review this" },
        {
          ...
          url: "https://github.com/jagentdesk/jagentdesk/issues/456",
        },
      ],
      logger,
    );
  ...
  test("builds app-server env from launch-context env overrides", () => {
    const launchContext: AgentLaunchContext = {
      env: {
        JAGENTDESK_AGENT_ID: "00000000-0000-4000-8000-000000000301",
        JAGENTDESK_TEST_FLAG: "codex-launch-value",
      },
    };
    const env = buildCodexAppServerEnv(
      {
        env: {
          JAGENTDESK_AGENT_ID: "runtime-value",
          JAGENTDESK_TEST_FLAG: "runtime-test-value",
        },
      },
      launchContext.env,
    );

    expect(env.JAGENTDESK_AGENT_ID).toBe(launchContext.env?.JAGENTDESK_AGENT_ID);
    expect(env.JAGENTDESK_TEST_FLAG).toBe(launchContext.env?.JAGENTDESK_TEST_FLAG);
  });
    ...
    expect(events).toEqual([
      {
        ...
        item: {
          type: "assistant_message",
          text: "![Image](file:///tmp/jagentdesk%20image.png)",
        },
      },
    ]);
  });
  ...
  test("materializes imageGeneration base64 results before rendering markdown", () => {
    ...
    expect(source).toMatch(/jagentdesk-attachments(?:-[^\\/]+)?[\\/].+\.png$/);
    expect(existsSync(source)).toBe(true);
    rmSync(source, { force: true });
  });
      ...
      expect(timelineEvents).toEqual([
        {
          ...
          item: {
            ...
            name: "jagentdesk.browser_screenshot",
            status: "completed",
            error: null,
            detail: {
              type: "unknown",
