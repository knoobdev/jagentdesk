...
const COMMAND_NAME = "jagentdesk-issue-903-big-pickle";
const COMMAND_FILE_NAME = `${COMMAND_NAME}.md`;
const OPENCODE_REAL_TEST_MODEL = getRealProviderConfig("opencode").model;
const EXPECTED_RESPONSE = "JAGENTDESK_ISSUE_903_BIG_PICKLE_OK";
...
describe("opencode custom command Big Pickle E2E (real)", () => {
  ...
  test("executes a global custom command through JAgentDesk using Big Pickle", async () => {
    ...
    const projectDir = await mkdtemp(path.join(tmpdir(), "jagentdesk-opencode-big-pickle-"));
    ...
    try {
      ...
      await writeFile(
        ...
        [
          "---",
          "description: JAgentDesk issue 903 Big Pickle custom command",
          "agent: build",
          "---",
          "",
