...
export default async function jagentdeskPlugin(input, options) {
  const request = async (pathname, init) => {
    ...
    if (!response.ok) {
      const error = new Error(payload.error ?? `JAgentDesk OpenCode bridge failed: ${response.status}`);
  ...
  for (const definition of manifest.tools ?? []) {
    tools[`jagentdesk_${definition.name}`] = {
      ...
      execute: async (args, context) => {
        ...
        return {
          ...
          metadata: { jagentdeskTool: definition.name },
        };
      },
    };
  }
...
function logPluginError(stage, context, error) {
  console.error(`[jagentdesk-opencode-plugin] ${stage} failed`, {
    ...context,
