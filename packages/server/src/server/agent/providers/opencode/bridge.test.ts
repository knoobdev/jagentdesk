...
function createCatalog(): JAgentDeskToolCatalog {
  const tool = {
    name: "echo_context",
    title: "Echo context",
...
describe("OpenCodeBridge", () => {
  test("loads packaged bundle bytes without invoking source compilation", async () => {
    const root = await mkdtemp(path.join(tmpdir(), "jagentdesk-opencode-artifact-"));
  ...
  test("serves authenticated session context and caller-scoped tools", async () => {
    const jagentdeskHome = await mkdtemp(path.join(tmpdir(), "jagentdesk-opencode-bridge-"));
    temporaryDirectories.push(jagentdeskHome);
    const catalog = createCatalog();
    const bridge = new OpenCodeBridge({ jagentdeskHome, logger: createTestLogger() });
    await bridge.start();
    bridge.setManifestCatalog(catalog);
    const release = bridge.bindSession({
      sessionId: "ses_one",
      env: {
        JAGENTDESK_AGENT_ID: "agent-one",
        JAGENTDESK_AGENT_CWD: "/workspace/one",
        CUSTOM_VALUE: "one",
      },
      tools: catalog,
    });
    ...
    try {
      ...
      await expect(
        hooks.tool.jagentdesk_echo_context.execute(
      ...
      expect(pluginError).toHaveBeenCalledWith(
        "[jagentdesk-opencode-plugin] shell.env failed",
  ...
  test("preserves user OpenCode config while installing one content-addressed plugin", async () => {
    const jagentdeskHome = await mkdtemp(path.join(tmpdir(), "jagentdesk-opencode-bridge-config-"));
    temporaryDirectories.push(jagentdeskHome);
    const bridge = new OpenCodeBridge({ jagentdeskHome, logger: createTestLogger() });
    await bridge.start();
    ...
    try {
      ...
      expect(config.plugin[1]?.[0]).toMatch(/jagentdesk-[a-f0-9]{64}\.mjs$/);
    } finally {
      await bridge.close();
    }
  });
