...
import type { JAgentDeskToolCatalog, JAgentDeskToolDefinition, JAgentDeskToolResult } from "../../tools/types.js";
import {
...
function createCatalog(tools: JAgentDeskToolDefinition[]): JAgentDeskToolCatalog {
...
class OmpHostToolHarness {
  ...
  private resolveControlledResult: ((result: JAgentDeskToolResult) => void) | null = null;
  ...
  static async withTools(tools: JAgentDeskToolDefinition[]): Promise<OmpHostToolHarness> {
  ...
  static async cancellable(): Promise<OmpHostToolHarness> {
    ...
    const tool: JAgentDeskToolDefinition = {
      name: "wait_for_agent",
      description: "Wait for a JAgentDesk agent.",
      handler: async (_input, context) => {
        harness.controlledSignal = context.signal ?? null;
        harness.resolveControlledStart?.();
        return await new Promise<JAgentDeskToolResult>((resolve) => {
  ...
  completeControlledCall(result: JAgentDeskToolResult): void {
  ...
  private routerInput() {
    return { runtimeSession: this.runtimeSession, jagentdeskTools: this.catalog, logger: this.logger };
  }
}

describe("OMP host tools", () => {
  test("marks every caller-scoped JAgentDesk tool essential for direct invocation", () => {
    const catalog = createCatalog([
      {
        name: "create_agent",
        title: "Create agent",
        description: "Create a JAgentDesk agent.",
