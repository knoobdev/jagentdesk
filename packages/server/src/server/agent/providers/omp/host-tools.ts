...
import {
  ...
  serializeJAgentDeskToolInputParameters,
} from "../../tools/jagentdesk-tool-serialization.js";
import type { JAgentDeskToolCatalog, JAgentDeskToolResult } from "../../tools/types.js";
...
interface OmpHostToolRouterInput {
  ...
  catalog: JAgentDeskToolCatalog;
  logger: Logger;
}

const routersByRuntimeSession = new WeakMap<OmpRuntimeSession, OmpHostToolRouter>();

export function serializeOmpHostTools(catalog: JAgentDeskToolCatalog): OmpRpcHostToolDefinition[] {
  return [...catalog.tools.values()].map((tool) => {
    const definition: OmpRpcHostToolDefinition = {
      name: tool.name,
      description: tool.description,
      loadMode: "essential",
      parameters: serializeJAgentDeskToolInputParameters(tool),
    };
    if (tool.title) {
...
export function handleOmpHostToolRuntimeEvent(
  ...
  input: {
    runtimeSession: OmpRuntimeSession;
    jagentdeskTools?: JAgentDeskToolCatalog;
    logger: Logger;
  },
): boolean {
...
function sendMissingCatalogResult(
  ...
  runtimeSession.sendHostToolResult(
    toOmpHostToolErrorResult(
      request.id,
      `Host tool "${request.toolName}" was called before JAgentDesk tools were registered`,
    ),
  );
}
...
class OmpHostToolRouter {
  ...
  private readonly catalog: JAgentDeskToolCatalog;
  ...
  private sendUpdate(callId: string, result: JAgentDeskToolResult): void {
...
function toOmpHostToolResult(id: string, result: JAgentDeskToolResult): OmpRpcHostToolResult {
...
function toOmpAgentToolResult(result: JAgentDeskToolResult): OmpAgentToolResult {
