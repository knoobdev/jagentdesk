...
function createToolCatalog(): JAgentDeskToolCatalog {
  return {
    tools: new Map([
      [
        "create_agent",
...
describe("OMP agent client and session", () => {
  test("owns launch configuration and registers native host tools", async () => {
    ...
    expect(omp.launchConfiguration()).toEqual({
      cwd: "/tmp/jagentdesk-omp-agent-test",
      protocolMode: "rpc-ui",
      modeId: "ask",
  ...
  test("resumes an OMP session and replays its history", async () => {
    ...
    expect(omp.launchConfiguration()).toEqual({
      ...
      protocolMode: "rpc-ui",
      modeId: "ask",
      session: expect.stringMatching(/[\\/]jagentdesk-omp-resume-.*[\\/]session\.jsonl$/),
      argv: [
        "omp",
        "--mode",
        "rpc-ui",
        ...
        "high",
        "--session",
        expect.stringMatching(/[\\/]jagentdesk-omp-resume-.*[\\/]session\.jsonl$/),
      ],
    });
