...
export class OmpAgentSession implements AgentSession {
  ...
  constructor(options: OmpAgentSessionOptions) {
    ...
    this.jagentdeskTools = options.jagentdeskTools;
  ...
  private readonly jagentdeskTools?: JAgentDeskToolCatalog;
  ...
  private handleExtraRuntimeEvent(event: OmpRuntimeEvent): boolean {
    if (
      handleOmpHostToolRuntimeEvent(event, {
        runtimeSession: this.runtimeSession,
        jagentdeskTools: this.jagentdeskTools,
        logger: this.logger,
      })
    ) {
      return true;
  ...
  private handleSessionEvent(event: OmpAgentSessionEvent): void {
    ...
    switch (event.type) {
      ...
      case "agent_end": {
        ...
        // so do not advertise JAgentDesk idle until it reports that transition.
...
export class OmpAgentClient implements AgentClient {
  ...
  private async configureNativeJAgentDeskTools(
    runtimeSession: OmpRuntimeSession,
    catalog: JAgentDeskToolCatalog | undefined,
  ): Promise<void> {
    if (!catalog) {
      return;
    }
  ...
  async createSession(
    ...
    try {
      await this.configureNativeJAgentDeskTools(runtimeSession, launchContext?.jagentdeskTools);
      return new OmpAgentSession({
        ...
        jagentdeskTools: launchContext?.jagentdeskTools,
      });
    } catch (error) {
