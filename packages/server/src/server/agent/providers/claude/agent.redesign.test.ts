...
test("logs redacted query summary and never leaks sentinel secrets", async () => {
  const envSecret = "JAGENTDESK_ENV_SENTINEL_SECRET";
  const runtimeSecret = "JAGENTDESK_RUNTIME_SENTINEL_SECRET";
  const systemSecret = "JAGENTDESK_SYSTEM_PROMPT_SENTINEL_SECRET";
  const previousEnv = process.env.JAGENTDESK_TEST_SENTINEL_SECRET;
  process.env.JAGENTDESK_TEST_SENTINEL_SECRET = envSecret;
  ...
  const client = new ClaudeAgentClient({
    ...
    runtimeSettings: {
      env: {
        JAGENTDESK_RUNTIME_SENTINEL_SECRET: runtimeSecret,
      },
    },
    resolveBinary: async () => "/test/claude/bin",
  });
  ...
  try {
    ...
    if (previousEnv === undefined) {
      delete process.env.JAGENTDESK_TEST_SENTINEL_SECRET;
    } else {
      process.env.JAGENTDESK_TEST_SENTINEL_SECRET = previousEnv;
    }
  }
});
