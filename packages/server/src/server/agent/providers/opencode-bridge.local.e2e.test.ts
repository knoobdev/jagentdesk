...
test("real OpenCode server shares one process while shell.env stays session-scoped", async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), "jagentdesk-opencode-real-"));
  const firstCwd = path.join(root, "first");
  const secondCwd = path.join(root, "second");
  const logger = createTestLogger();
  const bridge = new OpenCodeBridge({ jagentdeskHome: root, logger });
  await bridge.start();
  ...
  try {
    ...
    first = await client.createSession(
      { provider: "opencode", cwd: firstCwd, model: "opencode/big-pickle", modeId: "build" },
      {
        ...
        env: { JAGENTDESK_AGENT_ID: "real-agent-one", JAGENTDESK_AGENT_CWD: firstCwd },
        jagentdeskTools: firstTools,
      },
    ...
    second = await client.createSession(
      { provider: "opencode", cwd: secondCwd },
      {
        ...
        env: { JAGENTDESK_AGENT_ID: "real-agent-two", JAGENTDESK_AGENT_CWD: secondCwd },
        jagentdeskTools: secondTools,
      },
      { persistSession: false },
    );
    ...
    const [firstShell, secondShell] = await Promise.all([
      sdk.session.shell({
        ...
        directory: firstCwd,
        agent: "build",
        command: 'printf "%s|%s" "$JAGENTDESK_AGENT_ID" "$JAGENTDESK_AGENT_CWD"',
      }),
      sdk.session.shell({
    ...
    const agentResult = await first.run(
      [
        "Use the bash tool to run: env | grep -E '^(JAGENTDESK_AGENT_ID|JAGENTDESK_AGENT_CWD)='",
    ...
    const callerResult = await first.run(
      [
        "Use the jagentdesk_report_caller_agent_id tool to read your JAgentDesk caller agent ID.",
    ...
    expect(callerResult.timeline).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          type: "tool_call",
          name: "jagentdesk_report_caller_agent_id",
          status: "completed",
        }),
      ]),
    );
...
function createCallerCatalog(callerAgentId: string): JAgentDeskToolCatalog {
  const tool = {
    name: "report_caller_agent_id",
    title: "Report JAgentDesk caller agent ID",
    description: "Returns the caller agent ID assigned by JAgentDesk.",
    inputSchema: {},
    async handler() {
