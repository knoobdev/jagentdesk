...
const JAGENTDESK_PI_TREE_EXTENSION_COMMAND = "jagentdesk_tree";
const JAGENTDESK_PI_CAPTURE_EXTENSION_COMMAND = "jagentdesk_capture_entries";
const JAGENTDESK_PI_ENTRY_CAPTURE_MARKER = "JAGENTDESK_ENTRY_CAPTURE";
const JAGENTDESK_PI_SUBMITTED_USER_ENTRY_MARKER = "JAGENTDESK_SUBMITTED_USER_ENTRY";
const JAGENTDESK_PI_COMMAND_RESULT_MARKER = "JAGENTDESK_COMMAND_RESULT";
...
function buildResumeStartInput(input: {
  ...
  jagentdeskExtension: PiTempFile | null;
}): PiStartSessionInput {
  return {
    ...
    extensionPaths: input.jagentdeskExtension ? [input.jagentdeskExtension.path] : undefined,
  };
}
...
function createPiMcpConfigFile(
  ...
  const dir = mkdtempSync(join(tmpdir(), "jagentdesk-pi-mcp-"));
...
function createPiJAgentDeskExtensionFile(systemPrompt?: string): PiTempFile {
  const dir = mkdtempSync(join(tmpdir(), "jagentdesk-pi-extension-"));
  const filePath = join(dir, "jagentdesk-integration.mjs");
  writeFileSync(
    filePath,
    `
	...
	function emitEntryCapture(ctx, reason, requestId) {
	  ctx.ui.notify(
	    "${JAGENTDESK_PI_ENTRY_CAPTURE_MARKER} " +
	...
	function emitCommandResult(ctx, requestId, result) {
	  ctx.ui.notify(
	    "${JAGENTDESK_PI_COMMAND_RESULT_MARKER} " + JSON.stringify({ requestId, ...result }),
	    result.ok ? "info" : "error",
	  );
	}

	export default function jagentdeskIntegration(pi) {
	  ...
	  function emitSubmittedUserEntries(ctx) {
	    ...
	    for (let index = 0; index < submittedUserMessages.length; index += 1) {
	      ...
	      index -= 1;
	      ctx.ui.notify(
	        "${JAGENTDESK_PI_SUBMITTED_USER_ENTRY_MARKER} " +
	  ...
	  pi.registerCommand("${JAGENTDESK_PI_CAPTURE_EXTENSION_COMMAND}", {
	    description: "Internal JAgentDesk entry capture bridge",
	  ...
	  pi.registerCommand("${JAGENTDESK_PI_TREE_EXTENSION_COMMAND}", {
	    description: "Internal JAgentDesk tree navigation bridge",
	    handler: async (args, ctx) => {
	      const payload = decodePayload(args.trim());
	      try {
...
export class PiRpcAgentSession implements AgentSession {
  ...
  private async runPiTreeExtensionCommand(targetId: string): Promise<unknown> {
    ...
    await this.runtimeSession.prompt(`/${JAGENTDESK_PI_TREE_EXTENSION_COMMAND} ${payload}`);
  ...
  private async requestEntryCapture(reason: string): Promise<void> {
    ...
    await this.runtimeSession.prompt(`/${JAGENTDESK_PI_CAPTURE_EXTENSION_COMMAND} ${payload}`);
    await resultPromise;
  }
  ...
  private handleSubmittedUserEntryMarker(message: string): boolean {
    const payload = parseExtensionMarkerPayload(message, JAGENTDESK_PI_SUBMITTED_USER_ENTRY_MARKER);
    if (!payload) {
      return false;
    }
  ...
  private handleEntryCaptureMarker(message: string): boolean {
    const payload = parseExtensionMarkerPayload(message, JAGENTDESK_PI_ENTRY_CAPTURE_MARKER);
    if (!payload) {
      return false;
    }
  ...
  private handleCommandResultMarker(message: string): boolean {
    const payload = parseExtensionMarkerPayload(message, JAGENTDESK_PI_COMMAND_RESULT_MARKER);
    if (!payload) {
      return false;
    }
    if (typeof payload.requestId !== "string") {
...
export class PiRpcAgentClient implements AgentClient {
  ...
  async createSession(
    ...
    const jagentdeskExtension = createPiJAgentDeskExtensionFile(
    ...
    try {
      runtimeSession = await this.runtime.startSession({
        ...
        extensionPaths: jagentdeskExtension ? [jagentdeskExtension.path] : undefined,
      });
    } catch (error) {
      mcpConfig?.cleanup();
      jagentdeskExtension?.cleanup();
      throw error;
    }
    try {
      return new PiRpcAgentSession({
        ...
        cleanup: combineCleanup([mcpConfig?.cleanup, jagentdeskExtension?.cleanup]),
