...
describe("OpenCode bridge adapter", () => {
  ...
  test("shares the server and binds exact managed env per OpenCode session", async () => {
    const jagentdeskHome = await mkdtemp(path.join(os.tmpdir(), "jagentdesk-opencode-adapter-"));
    ...
    const first = await client.createSession(
      ...
      {
        agentId: "agent-one",
        env: { JAGENTDESK_AGENT_ID: "agent-one", JAGENTDESK_AGENT_CWD: "/workspace/one" },
      },
    );
    const second = await client.createSession(
      ...
      {
        agentId: "agent-two",
        env: { JAGENTDESK_AGENT_ID: "agent-two", JAGENTDESK_AGENT_CWD: "/workspace/two" },
      },
    );

    expect(client.capabilities.supportsNativeJAgentDeskTools).toBe(true);
  ...
  test("keeps process-scoped env and directory-scoped MCP on dedicated servers", async () => {
    ...
    const customEnv = await client.createSession(
      { provider: "opencode", cwd: "/workspace/one" },
      { env: { JAGENTDESK_AGENT_ID: "one", CUSTOM_TOKEN: "secret" } },
    );
