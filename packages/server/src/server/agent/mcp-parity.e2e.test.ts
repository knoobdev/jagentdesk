...
let tempRoot: string;
let daemonHandle: TestJAgentDeskDaemon;
...
beforeAll(async () => {
  ...
  daemonHandle = await createTestJAgentDeskDaemon({ agentClients: createRecordingAgentClients() });
...
describe("Suite A: Core Fixes", () => {
  ...
  test("agentManager.createAgent injects jagentdesk MCP using the daemon listen target", async () => {
    let agentId: string | null = null;
    try {
      ...
      expect(snapshot.config.mcpServers?.jagentdesk).toBeUndefined();

      const liveAgent = daemonHandle.daemon.agentManager.getAgent(agentId);
      expect(liveAgent?.config.mcpServers?.jagentdesk).toBeUndefined();
    } finally {
      await archiveAgentIfPresent(agentId);
    }
  });
