...
import { isJAgentDeskOwnedWorktreeCwd } from "../../utils/worktree.js";
...
interface CreateAgentLifecycleDispatchDependencies {
  ...
  createJAgentDeskWorktreeWorkflow: CreateJAgentDeskWorktreeWorkflowFn;
...
type AutoArchiveTarget =
  | { kind: "agent-only" }
  | { kind: "created-worktree"; result: CreateJAgentDeskWorktreeWorkflowResult };
...
export class CreateAgentLifecycleDispatch {
  ...
  }): Promise<CreateJAgentDeskWorktreeWorkflowResult | null> {
  ...
  private async createWorktreeForTarget(
    ...
    switch (target.mode) {
      case "branch-off":
        return this.dependencies.createJAgentDeskWorktreeWorkflow(
          {
            ...baseInput,
  ...
  private async archiveAutoCreatedWorktree(options: {
    ...
    const ownership = await isJAgentDeskOwnedWorktreeCwd(worktreePath, {
      jagentdeskHome: this.dependencies.jagentdeskHome,
      worktreesRoot: this.dependencies.worktreesRoot,
    });
    if (!ownership.allowed) {
      throw new Error("Auto-created worktree is not a JAgentDesk-owned worktree");
    }

    await archiveByScope(
      {
        jagentdeskHome: this.dependencies.jagentdeskHome,
        jagentdeskWorktreesBaseRoot: this.dependencies.worktreesRoot,
