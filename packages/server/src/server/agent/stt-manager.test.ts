...
describe("STTManager", () => {
  ...
  it("uses JAGENTDESK_VOICE_LANGUAGE over JAGENTDESK_DICTATION_LANGUAGE", async () => {
    const fakeStt = await transcribeWithResolvedVoiceLanguage({
      env: {
        JAGENTDESK_VOICE_LANGUAGE: "pt",
        JAGENTDESK_DICTATION_LANGUAGE: "es",
  ...
  it("uses JAGENTDESK_DICTATION_LANGUAGE when JAGENTDESK_VOICE_LANGUAGE is unset", async () => {
    const fakeStt = await transcribeWithResolvedVoiceLanguage({
      env: {
  ...
  it("uses streaming segmentation for batch transcription and concatenates segment finals", async () => {
    const original = process.env.JAGENTDESK_STT_BATCH_COMMIT_EVERY_SECONDS;
    process.env.JAGENTDESK_STT_BATCH_COMMIT_EVERY_SECONDS = "1";
    ...
    try {
      ...
      if (original === undefined) {
        delete process.env.JAGENTDESK_STT_BATCH_COMMIT_EVERY_SECONDS;
      } else {
        process.env.JAGENTDESK_STT_BATCH_COMMIT_EVERY_SECONDS = original;
      }
    }
  });
});
