...
class RecordingAgentClient implements AgentClient {
  ...
  constructor(
    ...
    this.capabilities = {
      ...
      supportsNativeJAgentDeskTools: false,
    };
  }
...
describe("agent MCP end-to-end (offline)", () => {
  test("create_agent runs initial prompt and affects filesystem", async () => {
    ...
    const agentCwd = await mkdtemp(path.join(os.tmpdir(), "jagentdesk-agent-cwd-"));
    const port = await getAvailablePort();

    const daemonConfig: JAgentDeskDaemonConfig = {
    ...
    const daemon = await createJAgentDeskDaemon(daemonConfig, pino({ level: "silent" }));
    await daemon.start();
  ...
  test("create_agent auto-injects jagentdesk MCP by default and can be disabled", async () => {
    const jagentdeskHome = await mkdtemp(path.join(os.tmpdir(), "jagentdesk-home-"));
    const staticDir = await mkdtemp(path.join(os.tmpdir(), "jagentdesk-static-"));
    const agentCwd = await mkdtemp(path.join(os.tmpdir(), "jagentdesk-agent-cwd-"));
    const port = await getAvailablePort();
    ...
    const disabledDaemonConfig: JAgentDeskDaemonConfig = {
      ...
      jagentdeskHome: disabledJAgentDeskHome,
      corsAllowedOrigins: [],
      hostnames: true,
      mcpEnabled: true,
    ...
    try {
      ...
      expect(injectedAgent?.config.mcpServers?.jagentdesk).toBeUndefined();
      ...
      expect(disabledRecorder.recordedLaunches.at(-1)?.mcpServers?.jagentdesk).toBeUndefined();
      const disabledAgent = disabledDaemon.agentManager.getAgent(disabledAgentId!);
      expect(disabledAgent?.config.mcpServers?.jagentdesk).toBeUndefined();
    } finally {
      if (agentId) {
        await client.callTool({ name: "kill_agent", args: { agentId } });
      }
      ...
      await rm(disabledJAgentDeskHome, { recursive: true, force: true });
  ...
  test("create_agent with worktree is async and boots terminals only after setup success", async () => {
    const jagentdeskHome = await mkdtemp(path.join(os.tmpdir(), "jagentdesk-home-"));
    const staticDir = await mkdtemp(path.join(os.tmpdir(), "jagentdesk-static-"));
    const repoRoot = await mkdtemp(path.join(os.tmpdir(), "jagentdesk-worktree-repo-"));
    const port = await getAvailablePort();
