...
import { everyMsToFiveFieldCron } from "@jagentdesk/protocol/schedule/cadence";
import { expandUserPath, isSameOrDescendantPath, resolvePathFromBase } from "../../path-utils.js";
import type { TerminalManager } from "../../../terminal/terminal-manager.js";
import type { CreateJAgentDeskWorktreeWorkflowFn } from "../../worktree-session.js";
...
import {
  ...
  type CreateJAgentDeskWorktreeCommandInput,
...
export interface JAgentDeskToolHostDependencies {
  ...
  createJAgentDeskWorktree?: CreateJAgentDeskWorktreeWorkflowFn;
...
type WorkspaceWorktreeTarget = Pick<
  CreateJAgentDeskWorktreeCommandInput,
...
export function createJAgentDeskToolCatalog(options: JAgentDeskToolHostDependencies): JAgentDeskToolCatalog {
  ...
  const tools = new Map<string, JAgentDeskToolDefinition>();
  const registerTool = (
    name: string,
    ...
    tools.set(name, {
      ...
      handler: handler as JAgentDeskToolDefinition["handler"],
    });
  };
  const toCatalog = (): JAgentDeskToolCatalog => ({
    tools,
    ...
    async executeTool(
      ...
      if (!tool) {
        throw new Error(`JAgentDesk tool not found: ${name}`);
      }
    ...
    z
      .object({
        ...
        worktreeSlug: z
          ...
          .min(1)
          .optional()
          .describe("Optional worktree slug/path label. Omit to let JAgentDesk generate one."),
        branchName: z
          .string()
          .min(1)
          .optional()
      ...
      .strict()
      .describe("Create a new branch in a new JAgentDesk worktree."),
    z
      .object({
      ...
      .strict()
      .describe("Check out an existing branch in a new JAgentDesk worktree."),
    z
      .object({
      ...
      .strict()
      .describe("Check out a GitHub pull request in a new JAgentDesk worktree."),
  ]);
  ...
  registerTool(
    "create_workspace",
    {
      ...
      description:
        "Create a workspace using an existing local checkout or a new JAgentDesk-managed worktree.",
      inputSchema: {
        isolation: z.enum(["local", "worktree"]),
        path: z
          .string()
    ...
    async ({
      ...
      if (isolation === "local") {
        ...
        const result = await createJAgentDeskWorktreeCommand(
          {
            jagentdeskHome: options.jagentdeskHome,
            worktreesRoot: options.worktreesRoot,
            createJAgentDeskWorktreeWorkflow: options.createJAgentDeskWorktree,
          },
          {
            cwd,
            ...(projectId ? { projectId } : {}),
    ...
    async (args: unknown) => {
      ...
      const {
      ...
      } = await createAgentCommand(
        {
          ...
          terminalManager,
          providerSnapshotManager,
          createJAgentDeskWorktree: options.createJAgentDeskWorktree,
  ...
  registerTool(
    "start_workspace_script",
    {
      ...
      description:
        "Start one configured workspace script through JAgentDesk's managed workspace-script launcher.",
      inputSchema: {
        workspaceId: z.string().describe("Workspace ID containing the configured script."),
        scriptName: z.string().min(1).describe("Configured jagentdesk.json script name to start."),
      },
      outputSchema: {
  ...
  registerTool(
    "stop_workspace_script",
    {
      ...
      inputSchema: {
        ...
        scriptName: z.string().min(1).describe("Configured jagentdesk.json script name to stop."),
      },
      outputSchema: {
        script: WorkspaceScriptPayloadSchema,
      },
...
function archiveWorktreeDependencies(
  ...
  return {
    jagentdeskHome: options.jagentdeskHome,
    jagentdeskWorktreesBaseRoot: options.worktreesRoot,
