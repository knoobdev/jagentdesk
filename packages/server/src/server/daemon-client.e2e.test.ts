...
const localModelsDir =
  process.env.JAGENTDESK_LOCAL_MODELS_DIR ?? path.join(homedir(), ".jagentdesk", "models", "local-speech");
  ...
  try {
    ...
    expect(result).toEqual({
      ...
      file: {
        ...
        mimeType: "text/plain",
        size: 11,
        path: path.join(daemon.jagentdeskHome, "uploads", "upload_req-upload-e2e", "notes.txt"),
      },
      error: null,
    });
...
test("returns home-scoped directory suggestions", async () => {
  const insideHomeDir = mkdtempSync(path.join(homedir(), "jagentdesk-dir-suggestion-"));
  const rootBrowseDir = mkdtempSync(path.join(homedir(), "000-jagentdesk-root-browse-"));
  const outsideHomeDir = mkdtempSync(path.join(tmpdir(), "jagentdesk-dir-suggestion-outside-"));
...
test("returns typed relative suggestions within a requested directory", async () => {
  const cwd = mkdtempSync(path.join(tmpdir(), "jagentdesk-workspace-suggestion-"));
...
test("finds workspace files inside the OpenCode directory", async () => {
  const cwd = mkdtempSync(path.join(tmpdir(), "jagentdesk-opencode-suggestion-"));
...
test("a Desktop-managed daemon does not advertise npm self-update", async () => {
  const daemon = await createTestJAgentDeskDaemon({ desktopManaged: true });
  const client = new DaemonClient({
    url: `ws://127.0.0.1:${daemon.port}/ws`,
