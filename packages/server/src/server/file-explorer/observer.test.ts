...
async function workspace() {
  const root = await mkdtemp(path.join(os.tmpdir(), "jagentdesk-file-observer-"));
  roots.push(root);
...
describe("FileObserver", () => {
  ...
  test("publishes shared watcher updates in each subscriber's path coordinates", async () => {
    const root = await workspace();
    const aliasParent = await mkdtemp(path.join(os.tmpdir(), "jagentdesk-file-observer-alias-"));
