...
describe("server config", () => {
  ...
  test("records when the daemon is managed by JAgentDesk Desktop", async () => {
    const jagentdeskHome = await mkdtemp(path.join(os.tmpdir(), "jagentdesk-config-desktop-managed-"));
    roots.push(jagentdeskHome);

    const desktopConfig = loadConfig(jagentdeskHome, {
      env: { JAGENTDESK_DESKTOP_MANAGED: "1" },
    });
    const standaloneConfig = loadConfig(jagentdeskHome, { env: {} });
  ...
  test("resolves bundled web UI path from globally installed compiled modules", async () => {
    const packageRoot = await mkdtemp(path.join(os.tmpdir(), "jagentdesk-config-compiled-"));
  ...
  test("resolves packaged desktop web UI path from resources app-dist", async () => {
    const packageRoot = await mkdtemp(path.join(os.tmpdir(), "jagentdesk-config-packaged-"));
    roots.push(packageRoot);
