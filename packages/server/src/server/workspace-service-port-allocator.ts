...
export interface AllocateWorkspaceServicePortOptions {
  allocation: JAgentDeskServicePortAllocation | undefined;
  cwd: string;
  scriptName: string;
  workspaceId: string;
  ...
  try {
    result = await execCommand(
      ...
      [options.scriptName, options.workspaceId, options.branchName ?? "", options.cwd],
      {
        cwd: options.cwd,
        envOverlay: {
          JAGENTDESK_SCRIPTNAME: options.scriptName,
          JAGENTDESK_WORKSPACE_ID: options.workspaceId,
          JAGENTDESK_BRANCH_NAME: options.branchName ?? "",
          JAGENTDESK_WORKTREE_PATH: options.cwd,
        },
