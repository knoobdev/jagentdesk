...
export function buildTeamLeadPrompt(topicId: string, originPrompt: string): string {
  return [
    ...
    "- There's also a team CHAT (the 'chém gió' room, a Telegram-style side channel): forum.chat to drop",
    "  a casual line, forum.chat_sticker for a fun sticker, forum.chat_react to react to a line, and",
    "  forum.open_chat_room to start a side room (#bug-safari, #ship-it...). Use it like Slack banter",
    "  WHILE you work — a quick 'nice one', hyping a win, groaning at a bug — ORGANICALLY, only when you",
    "  actually feel like it, never on a schedule. It's off-the-record chatter; the real decisions still",
    "  go in the thread with forum.post_message.",
