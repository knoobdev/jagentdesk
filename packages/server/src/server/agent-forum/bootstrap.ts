...
export function buildTeamLeadPrompt(topicId: string, originPrompt: string): string {
  return [
    ...
    "- If something only the HUMAN can decide (a product call, approval, missing info), ASK THEM with",
    "  your built-in AskUserQuestion tool — that pops a real question in their chat. Wait for their",
    '  answer, then drop it into the thread with forum.post_message (kind "decision", quote what they',
    "  chose) so the team + board have it on record. Only YOU (the lead) ask the human; peers ask you.",
