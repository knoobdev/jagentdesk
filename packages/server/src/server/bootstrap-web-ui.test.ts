...
function readInjectedConnectionHint(html: string): InitialDaemonConnectionHint {
  const match = html.match(
    /window\.__JAGENTDESK_INITIAL_DAEMON_CONNECTION__=(?<json>\{[^<]+})<\/script>/,
  );
...
describe("daemon web UI bootstrap", () => {
  ...
  let daemonHandle: TestJAgentDeskDaemon | null = null;

  async function createWebUiDist(): Promise<string> {
    tempRoot = await mkdtemp(path.join(os.tmpdir(), "jagentdesk-bootstrap-web-ui-"));
  ...
  test("injects a TLS initial connection hint only for HTTPS forwarded by a trusted proxy", async () => {
    ...
    daemonHandle = await createTestJAgentDeskDaemon({
      mcpEnabled: false,
      webUi: {
        enabled: true,
        distDir,
