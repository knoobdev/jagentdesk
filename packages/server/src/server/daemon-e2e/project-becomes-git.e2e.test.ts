...
test("an empty project becomes Git without changing its identity or creating a workspace", async () => {
  const projectRoot = realpathSync(
    mkdtempSync(path.join(os.tmpdir(), "jagentdesk-project-becomes-git-")),
  );
  const jagentdeskHomeRoot = realpathSync(
    mkdtempSync(path.join(os.tmpdir(), "jagentdesk-project-becomes-git-home-")),
  );
  ...
  const persistedProjects = JSON.parse(
    await readFile(path.join(daemon.jagentdeskHome, "projects", "projects.json"), "utf8"),
