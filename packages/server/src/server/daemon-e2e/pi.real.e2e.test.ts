...
import { createTestJAgentDeskDaemon, type TestJAgentDeskDaemon } from "../test-utils/jagentdesk-daemon.js";
import {
  canRunRealProvider,
...
process.env.JAGENTDESK_SUPERVISED = "0";
...
async function withConnectedPiDaemon(
  run: (context: { client: DaemonClient; daemon: TestJAgentDeskDaemon }) => Promise<void>,
): Promise<void> {
...
test(
  "real Pi daemon composes project and JAgentDesk system prompts",
  async () => {
    ...
    try {
      ...
      writeFileSync(
        ...
        [
          "When the user says JAGENTDESK_SYSTEM_PROMPT_PROBE, reply with exactly two tokens:",
          "PROJECT_PROMPT followed by the value of JAGENTDESK_PROMPT_TOKEN from later system instructions.",
          "If no JAGENTDESK_PROMPT_TOKEN exists, use MISSING as the second token.",
        ].join("\n"),
      );
      ...
      await withConnectedPiDaemon(async ({ client }) => {
        const agent = await client.createAgent({
          ...
          systemPrompt:
            "JAGENTDESK_PROMPT_TOKEN is JAGENTDESK_PROMPT. Follow the project instruction for JAGENTDESK_SYSTEM_PROMPT_PROBE.",
        });

        await client.sendMessage(agent.id, "JAGENTDESK_SYSTEM_PROMPT_PROBE");
        ...
        const response = items
          ...
          .join("")
          .trim();
        expect(response).toBe("PROJECT_PROMPT JAGENTDESK_PROMPT");
      });
    } finally {
...
test(
  "real Pi daemon lists JAgentDesk-handled compact slash commands",
  async () => {
...
test(
  ...
  async () => {
    ...
    const firstPrompt = "JAGENTDESK_PI_ENTRY_ID_FIRST. Reply exactly: first-ok";
    const secondPrompt = "JAGENTDESK_PI_ENTRY_ID_SECOND. Reply exactly: second-ok";
