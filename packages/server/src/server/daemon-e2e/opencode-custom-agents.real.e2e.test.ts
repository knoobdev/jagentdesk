...
async function createHarness(): Promise<{
  client: DaemonClient;
  daemon: Awaited<ReturnType<typeof createTestJAgentDeskDaemon>>;
}> {
...
describe("daemon E2E (real opencode) - custom agent discovery", () => {
  ...
  test("custom agents defined in opencode.json appear in availableModes", async () => {
    ...
    writeFileSync(
      ...
      JSON.stringify({
        agent: {
          "jagentdesk-e2e-custom": {
            description: "Custom agent for JAgentDesk daemon E2E test",
            mode: "primary",
          },
        },
      }),
    ...
    try {
      ...
      const custom = snapshot.availableModes.find((m) => m.id === "jagentdesk-e2e-custom");
      expect(custom).toBeDefined();
      expect(custom!.description).toBe("Custom agent for JAgentDesk daemon E2E test");
