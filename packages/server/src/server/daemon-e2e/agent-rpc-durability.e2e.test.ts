...
test("agent fetch RPCs tolerate an agent whose workspace project record is gone", async () => {
  ...
  try {
    daemon = await createTestJAgentDeskDaemon({ jagentdeskHomeRoot: fixture.jagentdeskHomeRoot, cleanup: false });
...
function seedStaleAgentFixture(): StaleAgentFixture {
  const healthyCwd = mkdtempSync(path.join(os.tmpdir(), "jagentdesk-healthy-agent-"));
  const orphanCwd = mkdtempSync(path.join(os.tmpdir(), "jagentdesk-orphan-agent-"));
  const jagentdeskHomeRoot = mkdtempSync(path.join(os.tmpdir(), "jagentdesk-orphan-agent-home-"));
  const jagentdeskHome = path.join(jagentdeskHomeRoot, ".jagentdesk");
  const projectsDir = path.join(jagentdeskHome, "projects");
  const agentsDir = path.join(jagentdeskHome, "agents");
  const healthyProjectId = "proj-healthy-agent-rpc";
  const healthyWorkspaceId = "ws-healthy-agent-rpc";
  const orphanWorkspaceId = "c:\\Users\\jagentdesk\\stale-project";
  ...
  return {
    ...
    orphanAgentId,
    jagentdeskHomeRoot,
    cleanupPaths: [healthyCwd, orphanCwd, jagentdeskHomeRoot],
  };
}
