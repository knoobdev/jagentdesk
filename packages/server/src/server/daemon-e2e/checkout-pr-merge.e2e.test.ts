...
import type { PullRequestMergeable } from "@jagentdesk/protocol/messages";
...
function initGitRepo(repoDir: string): void {
  ...
  execSync("git config user.email 'jagentdesk-test@example.com'", {
    cwd: repoDir,
    stdio: "pipe",
  });
  execSync("git config user.name 'JAgentDesk Test'", {
    cwd: repoDir,
    stdio: "pipe",
  });
    ...
    async () => {
      ...
      try {
        ...
        const worktree = await createWorktreePrimitive({
          ...
          runSetup: true,
          jagentdeskHome: ctx.daemon.jagentdeskHome,
        });
        ...
        const archiveResult = await ctx.client.archiveJAgentDeskWorktree({
          worktreePath: worktree.worktreePath,
        });
        ...
        if (worktreePath) {
          await ctx.client.archiveJAgentDeskWorktree({ worktreePath }).catch(() => undefined);
        }
        if (agentId) {
