...
import { renderTerminalSnapshotToAnsi } from "@jagentdesk/protocol/terminal-snapshot";
import type { TerminalStreamEvent } from "@jagentdesk/client/internal/terminal-stream-router";
import { DaemonClient } from "../test-utils/daemon-client.js";
import { createTestJAgentDeskDaemon } from "../test-utils/jagentdesk-daemon.js";

const BYTE_DONE_MARKER = "__JAGENTDESK_BYTE_PACKAGE_LOCK_DONE__";
...
async function createPackageLockTerminalCwd(): Promise<PackageLockTerminalCwd> {
  const cwd = await mkdtemp(path.join(tmpdir(), "jagentdesk-byte-package-lock-"));
  return {
    path: cwd,
