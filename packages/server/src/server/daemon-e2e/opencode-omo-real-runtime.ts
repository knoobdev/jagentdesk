...
export async function createOpenCodeOmoRealRuntime(): Promise<OpenCodeOmoRealRuntime> {
  ...
  const openCodeVersion =
    process.env.JAGENTDESK_REAL_OPENCODE_VERSION?.trim() || PINNED_OPENCODE_VERSION;
  const omoVersion = process.env.JAGENTDESK_REAL_OMO_VERSION?.trim() || PINNED_OMO_VERSION;
  ...
  if (model === NO_AUTH_MODEL) {
    await runCommand({
      command: openCodeCommand,
      args: ["run", "--model", model, "Reply with exactly: JAGENTDESK_BIG_PICKLE_PROBE_OK"],
      cwd: paths.workspace,
      env: runtimeEnv,
      artifactName: "big-pickle-probe.log",
      artifacts: paths.artifacts,
      secrets,
      requiredOutput: "JAGENTDESK_BIG_PICKLE_PROBE_OK",
    });
  }

  const previousJAgentDeskHome = process.env.JAGENTDESK_HOME;
  ...
  let daemon: TestJAgentDeskDaemon | null = null;
  let client: DaemonClient | null = null;
  try {
    process.env.JAGENTDESK_HOME = path.join(paths.jagentdeskHomeRoot, ".jagentdesk");
    ...
    daemon = await createTestJAgentDeskDaemon({
      agentClients: { opencode: openCodeClient },
      logger,
      jagentdeskHomeRoot: paths.jagentdeskHomeRoot,
    ...
    return {
      ...
      close: async (passed) => {
        try {
          ...
          restoreEnvironment("JAGENTDESK_HOME", previousJAgentDeskHome);
        }
      },
    };
  } catch (error) {
...
function createRuntimePaths(): RuntimePaths {
  const root = mkdtempSync(path.join(tmpdir(), "jagentdesk-real-opencode-omo-"));
  const paths: RuntimePaths = {
    root,
    home: path.join(root, "home"),
    jagentdeskHomeRoot: path.join(root, "jagentdesk-home"),
...
function resolveModel(openRouterApiKey: string | null): string {
  const explicitModel = process.env.JAGENTDESK_REAL_OPENCODE_MODEL?.trim();
  if (explicitModel) {
...
function buildRuntimeEnv(paths: RuntimePaths, openRouterApiKey: string | null): NodeJS.ProcessEnv {
  return {
    ...
    JAGENTDESK_HOME: path.join(paths.jagentdeskHomeRoot, ".jagentdesk"),
