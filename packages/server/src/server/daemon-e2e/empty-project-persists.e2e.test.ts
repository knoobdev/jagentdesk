...
const cleanupDaemons = new Set<TestJAgentDeskDaemon>();
...
test("project.add creates a project without creating a workspace", async () => {
  const previousSupervised = process.env.JAGENTDESK_SUPERVISED;
  process.env.JAGENTDESK_SUPERVISED = "0";
  try {
    const repoRoot = realpathSync(mkdtempSync(path.join(os.tmpdir(), "jagentdesk-add-project-repo-")));
    const jagentdeskHomeRoot = realpathSync(
      mkdtempSync(path.join(os.tmpdir(), "jagentdesk-add-project-home-")),
    );
    cleanupPaths.add(repoRoot);
    cleanupPaths.add(jagentdeskHomeRoot);

    execSync("git init -b main", { cwd: repoRoot, stdio: "pipe" });
    execSync("git config user.email 'test@jagentdesk.dev'", { cwd: repoRoot, stdio: "pipe" });
    execSync("git config user.name 'JAgentDesk Test'", { cwd: repoRoot, stdio: "pipe" });
    ...
    const daemon = await createTestJAgentDeskDaemon({ jagentdeskHomeRoot, cleanup: false });
    ...
    restoreEnv("JAGENTDESK_SUPERVISED", previousSupervised);
  }
}, 30_000);
...
test("archiving the last workspace leaves the project parent with no workspaces", async () => {
  ...
  process.env.JAGENTDESK_SUPERVISED = "0";
  try {
    const repoRoot = realpathSync(mkdtempSync(path.join(os.tmpdir(), "jagentdesk-empty-project-repo-")));
    const jagentdeskHomeRoot = realpathSync(
      mkdtempSync(path.join(os.tmpdir(), "jagentdesk-empty-project-home-")),
    );
    cleanupPaths.add(repoRoot);
    cleanupPaths.add(jagentdeskHomeRoot);
    ...
    const projectsPath = path.join(jagentdeskHome, "projects", "projects.json");
