...
test("openProject preserves a worktree's exact-root project without rehoming it", async () => {
  ...
  try {
    const repoRoot = realpathSync(mkdtempSync(path.join(os.tmpdir(), "jagentdesk-open-project-repo-")));
    const worktreeRoot = realpathSync(
      mkdtempSync(path.join(os.tmpdir(), "jagentdesk-open-project-worktree-")),
    );
    const jagentdeskHomeRoot = realpathSync(
      mkdtempSync(path.join(os.tmpdir(), "jagentdesk-open-project-home-")),
    );
    ...
    const workspacesPath = path.join(jagentdeskHome, "projects", "workspaces.json");
    ...
    process.env.JAGENTDESK_SUPERVISED = previousSupervised;
  }
}, 30_000);
