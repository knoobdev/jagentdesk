...
import { isLikelyExternalToolName } from "@jagentdesk/protocol/tool-name-normalization";
...
class FakeAgentSession implements AgentSession {
  ...
  constructor(options: FakeAgentSessionOptions) {
    ...
    this.historyPath = path.join(
      tmpdir(),
      "jagentdesk-fake-provider-history",
      this.providerName,
      `${this.id}.jsonl`,
    );
  }
  ...
  private async runSlashCommand(
    ...
    if (this.providerName === "codex" && fullName.startsWith("prompts:")) {
      ...
      return {
        text: `JAGENTDESK_OK ${args ?? ""}`.trim(),
        timeline: [{ type: "assistant_message", text: `JAGENTDESK_OK ${promptId}` }],
        usage: { inputTokens: 1, outputTokens: 1 },
      };
    }

    return {
      text: "JAGENTDESK_SKILL_OK",
      timeline: [{ type: "assistant_message", text: "JAGENTDESK_SKILL_OK" }],
      usage: { inputTokens: 1, outputTokens: 1 },
    };
  }
