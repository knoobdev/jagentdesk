...
describe("searchDirectoryEntries", () => {
  ...
  beforeEach(() => {
    ...
    mkdirSync(path.join(searchRoot, "projects", "jagentdesk-desktop"), { recursive: true });
  ...
  it("applies result paths and entry kinds as parameters of one search", async () => {
    ...
    expect({ directories, files }).toEqual({
      directories: [
        {
          path: path.join(searchRoot, "projects", "jagentdesk-desktop"),
          kind: "directory",
        },
      ],
      files: [
  ...
  it("browses an absolute root and an absolute directory ending in a separator", async () => {
    ...
    expect({ rootEntries, projectEntries }).toEqual({
      ...
      projectEntries: [{ path: "projects/jagentdesk-desktop", kind: "directory" }],
    });
  });
  ...
  it("does not spend the scan budget on excluded entry kinds", async () => {
    ...
    const target = path.join(budgetRoot, "z-projects", "jagentdesk-target");
    ...
    await expect(
      searchDirectoryEntries({
        root: budgetRoot,
        query: "jagentdesk-target",
        pathFormat: "relative",
        includeFiles: false,
        includeDirectories: true,
        maxEntriesScanned: 2,
      }),
    ).resolves.toEqual([{ path: "z-projects/jagentdesk-target", kind: "directory" }]);
  });
...
describe("absolute directory-path configuration", () => {
  ...
  beforeEach(() => {
    ...
    mkdirSync(path.join(homeDir, "projects", "jagentdesk"), { recursive: true });
  ...
  it("shares the scan budget fairly between nested sibling branches", async () => {
    ...
    const projectPath = path.join(budgetHome, "work", "client", "team", "jagentdesk-desktop");
  ...
  it.skipIf(isWindows)("does not let a queued symlink hide the direct project branch", async () => {
    ...
    const projectPath = path.join(projectRoot, "jagentdesk-desktop");
  ...
  it.skipIf(isWindows)("follows visible directory symlinks that stay inside home", async () => {
    ...
    const projectPath = path.join(symlinkHome, ".linked", "project-root", "jagentdesk-desktop");
    mkdirSync(projectPath, { recursive: true });
  ...
  it.skipIf(isWindows)("matches the visible name of a directory symlink", async () => {
    ...
    const visibleProjectPath = path.join(projectsPath, "jagentdesk");
  ...
  it("supports home-relative path query syntax", async () => {
    ...
    expect(result.map((entry) => realpathSync.native(entry))).toEqual([
      realpathSync.native(path.join(homeDir, "projects", "jagentdesk")),
...
describe("relative typed-entry configuration", () => {
  ...
  beforeEach(() => {
    ...
    writeFileSync(path.join(workspaceDir, "README.md"), "# jagentdesk\n");
    writeFileSync(
  ...
  it("suffix mode matches whole path segment suffixes without fuzzy matches", async () => {
    ...
    writeFileSync(path.join(workspaceDir, "src", "jagentdesk-config-file.ts"), "");
  ...
  it("suffix mode resolves explicit hidden file paths without broad hidden traversal", async () => {
    const targetPath = path.join(workspaceDir, ".dev", "jagentdesk-home", "daemon.log");
    mkdirSync(path.dirname(targetPath), { recursive: true });
    writeFileSync(targetPath, "daemon log\n");

    const results = await searchRelativeDirectoryEntries({
      cwd: workspaceDir,
      query: ".dev/jagentdesk-home/daemon.log",
      limit: 20,
      includeFiles: true,
      includeDirectories: false,
      matchMode: "suffix",
    ...
    expect(results).toEqual([{ path: ".dev/jagentdesk-home/daemon.log", kind: "file" }]);
  });
