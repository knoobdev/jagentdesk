...
import { base64EncryptedWireByteLength } from "./encrypted-frame-size.js";
import {
...
import {
  getJAgentDeskWorktreeMetadataPath,
  readJAgentDeskWorktreeMetadata,
  writeJAgentDeskWorktreeMetadata,
...
describe("checkout git utilities", () => {
  ...
  it("reuses checkout snapshot facts across status, shortstat, and PR status reads", async () => {
    ...
    __resetPullRequestStatusCacheForTests();
    startGitCommandMetrics();
    const statusWithFacts = await getCheckoutStatus(repoDir, { jagentdeskHome, facts });
...
`;
  ...
  it("returns checkout root metadata for normal repos", async () => {
    ...
    expect(status.isJAgentDeskOwnedWorktree).toBe(false);
  ...
  it("reports a PR worktree as not ahead when its branch is pushed to the configured PR remote", async () => {
    ...
    commitFile(repoDir, "feature.txt", "feature\n", "feature commit");
    execFileSync("git", ["remote", "add", "jagentdesk-pr-1285", prRemoteDir], { cwd: repoDir });
    execFileSync(
      "git",
      ["push", "jagentdesk-pr-1285", "HEAD:refs/heads/open-button-targets-active-file"],
      { cwd: repoDir },
    );
    execFileSync(
      "git",
      ["config", "branch.aaronzhongg/open-button-targets-active-file.remote", "jagentdesk-pr-1285"],
      {
        cwd: repoDir,
      },
    );
  ...
  it("reports a PR worktree as behind when its configured PR remote has newer commits", async () => {
    ...
    execFileSync("git", ["fetch", "jagentdesk-pr-1285"], { cwd: repoDir });
  ...
  it("does not report full history as unpushed for fresh no-track JAgentDesk worktrees", async () => {
    setupRemoteTrackingMain(repoDir, tempDir);
    ...
    const status = await getCheckoutStatus(worktree.worktreePath, { jagentdeskHome });
    expect(status).toMatchObject({
      isGit: true,
      isJAgentDeskOwnedWorktree: true,
      baseRef: "main",
  ...
  it("keeps the structured diff cap below the transport frame limit", () => {
    const transportFrameBytes = 32 * 1024 * 1024;
    ...
    expect(diffWireBytes).toBeLessThan(transportFrameBytes);
    expect(maximumFrameWireBytes).toBe(transportFrameBytes);
  });
  ...
  it("resolves the Git common directory once when reading JAgentDesk worktree facts", async () => {
    ...
    startGitCommandMetrics();
    const facts = await getCheckoutSnapshotFacts(result.worktreePath, { jagentdeskHome });
    const metrics = stopGitCommandMetrics();
  ...
  it("handles status/diff/commit in a .jagentdesk worktree", async () => {
    const result = await createLegacyWorktreeForTest({
      branchName: "main",
      cwd: repoDir,
      baseBranch: "main",
    ...
    const status = await getCheckoutStatus(result.worktreePath, { jagentdeskHome });
    expect(status.isGit).toBe(true);
    ...
    const cleanStatus = await getCheckoutStatus(result.worktreePath, { jagentdeskHome });
    expect(cleanStatus.isDirty).toBe(false);
  ...
  it("returns checkout root metadata for .jagentdesk worktrees", async () => {
    const result = await createLegacyWorktreeForTest({
      branchName: "main",
      cwd: repoDir,
      baseBranch: "main",
  ...
  it("detects plain git worktrees from git alone", async () => {
    ...
    const status = await getCheckoutStatus(worktreeDir, { jagentdeskHome });
    expect(status.isGit).toBe(true);
  ...
  it("merges the current branch into base from a worktree checkout", async () => {
    ...
    await mergeToBase(worktree.worktreePath, { baseRef: "main" }, { jagentdeskHome });
    ...
    const statusAfterMerge = await getCheckoutStatus(worktree.worktreePath, { jagentdeskHome });
  ...
  it("reports the base worktree cwd when merge-to-base mutates a separate checkout", async () => {
    ...
    const mutatedCwd = await mergeToBase(featureWorktree.worktreePath, {}, { jagentdeskHome });
  ...
  it("pushes the current branch to its configured upstream", async () => {
    ...
    execFileSync("git", ["remote", "add", "jagentdesk-pr-526", prRemoteDir], { cwd: repoDir });
    execFileSync("git", ["checkout", "-b", "therainisme/main"], { cwd: repoDir });
    execFileSync("git", ["fetch", "jagentdesk-pr-526", "main"], { cwd: repoDir });
    execFileSync("git", ["config", "branch.therainisme/main.remote", "jagentdesk-pr-526"], {
      cwd: repoDir,
    });
    ...
    expect(upstream).toBe("jagentdesk-pr-526/main");
  });
  ...
  it("pushes the current branch to its configured push remote", async () => {
    ...
    execFileSync("git", ["remote", "add", "jagentdesk-pr-526", prRemoteDir], { cwd: repoDir });
    execFileSync("git", ["checkout", "-b", "therainisme/main"], { cwd: repoDir });
    execFileSync("git", ["config", "branch.therainisme/main.pushRemote", "jagentdesk-pr-526"], {
      cwd: repoDir,
    });
    execFileSync("git", ["config", "remote.jagentdesk-pr-526.push", "HEAD:refs/heads/main"], {
      cwd: repoDir,
    });
    writeFileSync(join(repoDir, "fork-pr.txt"), "fork pr edit\n");
    execFileSync("git", ["add", "fork-pr.txt"], { cwd: repoDir });
    ...
    const trackedPrRemoteHead = execFileSync(
      "git",
      ["rev-parse", "refs/remotes/jagentdesk-pr-526/main"],
      {
        cwd: repoDir,
      },
    )
    ...
    expect(prRemoteMain).toBe(localHead);
    expect(getBranchUpstream(repoDir)).toBe("jagentdesk-pr-526/main");
  ...
  it("refreshes the tracked ref after pushing through a configured push remote", async () => {
    ...
    execFileSync("git", ["checkout", "-b", "feature"], { cwd: repoDir });
    execFileSync("git", ["push", "-u", "origin", "feature"], { cwd: repoDir });
    execFileSync("git", ["remote", "add", "jagentdesk-pr-1790", originDir], { cwd: repoDir });
    execFileSync("git", ["config", "branch.feature.pushRemote", "jagentdesk-pr-1790"], {
      cwd: repoDir,
    });
    execFileSync("git", ["config", "remote.jagentdesk-pr-1790.push", "HEAD:refs/heads/feature"], {
      cwd: repoDir,
    });
  ...
  it("disables GitHub features when gh is unavailable", async () => {
    execFileSync("git", ["remote", "add", "origin", "https://github.com/acme/example.git"], {
      cwd: repoDir,
    });
  ...
  it("uses an origin tracked head when the local branch name differs", async () => {
    ...
    const lookupTarget = await readPullRequestLookupTargetFromFacts(repoDir, jagentdeskHome);
  ...
  it("reconciles a PR worktree lookup from current branch tracking", async () => {
    ...
    const workspaceDir = join(jagentdeskHome, "worktrees", "repo", "pr-worktree");
    mkdirSync(join(jagentdeskHome, "worktrees", "repo"), { recursive: true });
    ...
    writeJAgentDeskWorktreeMetadata(workspaceDir, {
      baseRefName: "main",
    ...
    expect(await readPullRequestLookupTargetFromFacts(workspaceDir, jagentdeskHome)).toMatchObject({
      headRef: "old-change",
      headRepositoryOwner: "contributor",
    });
    ...
    startGitCommandMetrics();
    const switchedTarget = await readPullRequestLookupTargetFromFacts(workspaceDir, jagentdeskHome);
  ...
  it("does not apply ambiguous legacy PR metadata to a suffixed branch", async () => {
    ...
    const workspaceDir = join(jagentdeskHome, "worktrees", "repo", "legacy-pr-worktree");
    mkdirSync(join(jagentdeskHome, "worktrees", "repo"), { recursive: true });
    ...
    const lookupTarget = await readPullRequestLookupTargetFromFacts(workspaceDir, jagentdeskHome);
  ...
  it("does not apply a legacy fork hint to an ownerless branch with the same head", async () => {
    ...
    const workspaceDir = join(jagentdeskHome, "worktrees", "repo", "legacy-fork-worktree");
    mkdirSync(join(jagentdeskHome, "worktrees", "repo"), { recursive: true });
    execFileSync("git", ["worktree", "add", workspaceDir, "contributor/old-change"], {
      cwd: repoDir,
    });
    ...
    const forkFacts = await getCheckoutSnapshotFacts(workspaceDir, { jagentdeskHome });
    await getPullRequestStatus(
      workspaceDir,
      forge,
      { force: true, reason: "legacy-fork-branch" },
      { jagentdeskHome, facts: forkFacts },
    );
    ...
    const ownerlessFacts = await getCheckoutSnapshotFacts(workspaceDir, { jagentdeskHome });
    await getPullRequestStatus(
      workspaceDir,
      forge,
      { force: true, reason: "ownerless-same-head" },
      { jagentdeskHome, facts: ownerlessFacts },
    );
  ...
  it("recognizes a normalized GitHub owner branch from legacy Enterprise metadata", async () => {
    ...
    const workspaceDir = join(jagentdeskHome, "worktrees", "repo", "legacy-enterprise-worktree");
    mkdirSync(join(jagentdeskHome, "worktrees", "repo"), { recursive: true });
    ...
    const facts = await getCheckoutSnapshotFacts(workspaceDir, { jagentdeskHome });
    await getPullRequestStatus(
      workspaceDir,
      forge,
    ...
    const repointedFacts = await getCheckoutSnapshotFacts(workspaceDir, { jagentdeskHome });
    await getPullRequestStatus(
      workspaceDir,
      forge,
      { force: true, reason: "repointed-enterprise-owner" },
      { jagentdeskHome, facts: repointedFacts },
    );
  ...
  it("keeps a ref-only change request bound across rename but not branch switch", async () => {
    ...
    const workspaceDir = join(jagentdeskHome, "worktrees", "repo", "gitlab-mr-worktree");
    mkdirSync(join(jagentdeskHome, "worktrees", "repo"), { recursive: true });
    ...
    expect(readJAgentDeskWorktreeMetadata(workspaceDir)?.changeRequestLookupTarget).toMatchObject({
      localBranchName: "feature/renamed",
    });
    const renamedFacts = await getCheckoutSnapshotFacts(workspaceDir, { jagentdeskHome });
    const renamedStatus = await getPullRequestStatus(
      workspaceDir,
      forge,
      { force: true, reason: "renamed-change-request" },
      { jagentdeskHome, facts: renamedFacts },
    );
    ...
    const switchedFacts = await getCheckoutSnapshotFacts(workspaceDir, { jagentdeskHome });
    await getPullRequestStatus(
      workspaceDir,
      forge,
      { force: true, reason: "switched-after-rename" },
      { jagentdeskHome, facts: switchedFacts },
    );
  ...
  it("does not attach an owner when the tracked remote is the same GitHub repository", async () => {
    ...
    execFileSync("git", ["remote", "add", "origin", "git@github.com:acme/example.git"], {
      cwd: repoDir,
    });
    execFileSync("git", ["remote", "add", "upstream", "https://github.com/acme/example.git"], {
      cwd: repoDir,
    });
  ...
  it("uses the configured push remote for fork PR lookup when upstream is absent", async () => {
    ...
    execFileSync("git", ["config", "remote.jagentdesk-pr-345.push", "HEAD:refs/heads/main"], {
      cwd: repoDir,
    });
    const requestedTargets: RequestedPullRequestTarget[] = [];
    const github = createGitHubServiceRecordingPullRequestTargets({ requestedTargets });

    const factsTarget = await readPullRequestLookupTargetFromFacts(repoDir, jagentdeskHome);
    await getPullRequestStatus(
      repoDir,
      github,
  ...
  it("uses the tracked fork branch for PR worktree status lookup", async () => {
    ...
    const github = createGitHubServiceRecordingPullRequestTargets({
      ...
      statusOverrides: {
        number: 345,
        url: "https://github.com/acme/example/pull/345",
      },
    });
  ...
  it("returns closed-unmerged PR status without marking it as merged", async () => {
    ...
    const status = await getPullRequestStatus(
      ...
      createGitHubServiceForStatus(
        createPullRequestStatus({
          url: "https://github.com/acme/example/pull/999",
  ...
  it("does not reuse a PR status cache entry after HEAD changes on the same branch", async () => {
    ...
    github.getCurrentPullRequestStatus = async (options) => {
      ...
      return createPullRequestStatus({
        url: `https://github.com/acme/example/pull/${requestedShas.length}`,
      });
    };
  ...
  it("expires cached PR status after the TTL", async () => {
    ...
    try {
      ...
      github.getCurrentPullRequestStatus = async () => {
        callCount += 1;
        return createPullRequestStatus({
          url: `https://github.com/acme/example/pull/${callCount}`,
        });
      };
      const first = await getPullRequestStatus(repoDir, github);
      await sleep(80);
  ...
  it("uses stored baseRefName for JAgentDesk worktrees (no heuristics)", async () => {
  ...
  it("excludes dirty working tree changes from JAgentDesk worktree base diffs", async () => {
    const worktree = await createLegacyWorktreeForTest({
      branchName: "feature",
      cwd: repoDir,
      baseBranch: "main",
  ...
  it("merges to stored baseRefName when baseRef is not provided", async () => {
    ...
    // Create a JAgentDesk worktree configured to use develop as base.
    const worktree = await createLegacyWorktreeForTest({
      branchName: "feature",
      cwd: repoDir,
      baseBranch: "develop",
    ...
    await mergeToBase(worktree.worktreePath, {}, { jagentdeskHome });
  ...
  it("falls back to plain git checkout status when JAgentDesk worktree metadata is missing", async () => {
    const worktree = await createLegacyWorktreeForTest({
      branchName: "feature",
      cwd: repoDir,
      baseBranch: "main",
  ...
  describe("parseWorktreeList", () => {
    it("parses porcelain worktree output", () => {
      const output = [
        ...
        "",
        "worktree /home/user/.jagentdesk/worktrees/feature",
      ...
      expect(entries[1]).toEqual({
        path: "/home/user/.jagentdesk/worktrees/feature",
  ...
  describe("isJAgentDeskWorktreePath", () => {
    it("matches Unix .jagentdesk/worktrees/ paths", () => {
      expect(isJAgentDeskWorktreePath("/home/user/.jagentdesk/worktrees/feature")).toBe(true);
    });
    ...
    it("matches worktrees under a custom JAGENTDESK_HOME", () => {
      ...
      const worktreePath =
        process.platform === "win32"
          ...
          : `${customJAgentDeskHome}/worktrees/project/feature`;

      expect(
        isJAgentDeskWorktreePath(worktreePath, {
          jagentdeskHome: customJAgentDeskHome,
        }),
      ).toBe(true);
    });

    it("rejects paths without .jagentdesk/worktrees segment", () => {
      expect(isJAgentDeskWorktreePath("/home/user/repo")).toBe(false);
      expect(isJAgentDeskWorktreePath("C:\\Users\\dev\\repo")).toBe(false);
    });
  });
