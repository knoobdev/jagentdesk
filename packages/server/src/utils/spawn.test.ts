...
import { buildSelfNodeCommand } from "../server/jagentdesk-env.js";
...
describe("execCommand", () => {
  ...
  test("treats env as the replacement base and finalizes external command env", async () => {
    ...
    expect(parsePrintedEnv(result.stdout)).toEqual({
      ...
      JAGENTDESK_DESKTOP_MANAGED: null,
      JAGENTDESK_NODE_ENV: null,
      JAGENTDESK_SUPERVISED: null,
    });
  });

  test("does not inherit process.env when env replacement is supplied", async () => {
    process.env.JAGENTDESK_TEST_SHOULD_NOT_LEAK = "leaked";
    try {
      const result = await execCommand(
        process.execPath,
        [
          "-e",
          "console.log(JSON.stringify({ leaked: process.env.JAGENTDESK_TEST_SHOULD_NOT_LEAK ?? null }))",
        ],
        {
          env: {
            PATH: process.env.PATH,
      ...
      delete process.env.JAGENTDESK_TEST_SHOULD_NOT_LEAK;
    }
  });
  ...
  test("internal env mode preserves JAgentDesk-owned launcher env", async () => {
    const result = await execCommand(process.execPath, ["-e", printEnvScript], {
      envMode: "internal",
      baseEnv: {
