...
import type { ProjectIconSource } from "@jagentdesk/protocol/messages";
import {
...
async function project() {
  const rootPath = await tempDir("jagentdesk-project-root-");
  const jagentdeskHome = await tempDir("jagentdesk-home-");
  ...
  return {
    ...
    set: (source: ProjectIconSource) =>
      setProjectCustomIcon({ jagentdeskHome, projectId: "project-a", source, projects }),
    read: () => readProjectIcon({ jagentdeskHome, project: record }),
    revision: () => record.customIconRevision,
    remove: () => removeProjectCustomIcon({ jagentdeskHome, projectId: "project-a" }),
  };
}
...
describe("project custom icon", () => {
  ...
  it("drops the stored image when the project is removed", async () => {
    ...
    await expect(
      readProjectIcon({ jagentdeskHome: target.jagentdeskHome, project: stored }),
    ).resolves.toBeNull();
  });
