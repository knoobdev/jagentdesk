...
import { isJAgentDeskOwnedWorktreeCwd, resolveJAgentDeskWorktreesBaseRoot } from "./worktree.js";
import {
  branchNameFromRef,
  getJAgentDeskWorktreeChangeRequestHintForBranch,
  type JAgentDeskWorktreeMetadata,
  readJAgentDeskWorktreeMetadata,
  rebindJAgentDeskWorktreeChangeRequestHint,
...
export interface CheckoutStatusGitNonJAgentDesk {
  isGit: true;
  repoRoot: string;
...
export interface CheckoutStatusGitJAgentDesk {
  isGit: true;
  repoRoot: string;
  mainRepoRoot: string;
  currentBranch: string | null;
...
export type CheckoutStatusGit = CheckoutStatusGitNonJAgentDesk | CheckoutStatusGitJAgentDesk;
...
export type CheckoutSnapshotFacts =
  ...
  | {
      ...
      jagentdeskWorktree: JAgentDeskWorktreeForCwd;
...
async function getMainRepoRootFromCommonDir(
  ...
  const nonBareNonJAgentDesk = worktrees.filter(
    (wt) =>
      !wt.isBare &&
      !isJAgentDeskWorktreePath(wt.path, {
        jagentdeskHome: context?.jagentdeskHome,
...
/** Check whether a path is under JAgentDesk's worktree root. */
export function isJAgentDeskWorktreePath(
  p: string,
  options?: { jagentdeskHome?: string; worktreesRoot?: string },
): boolean {
  if (options?.worktreesRoot || options?.jagentdeskHome) {
    return isDescendantPath(p, resolveJAgentDeskWorktreesBaseRoot(options));
  }
  return /[/\\]\.jagentdesk[/\\]worktrees[/\\]/.test(p);
}
...
export async function renameCurrentBranch(
  ...
  if (currentBranch) {
    rebindJAgentDeskWorktreeChangeRequestHint(worktreeRoot, previousBranch, currentBranch);
  }
  return { previousBranch, currentBranch };
}

type JAgentDeskWorktreeForCwd =
  | { isJAgentDeskOwnedWorktree: false }
  | { isJAgentDeskOwnedWorktree: true; worktreeRoot: string };

interface JAgentDeskWorktreeLookupOptions {
  context?: CheckoutContext;
  knownWorktreeRoot?: string | null;
  knownGitCommonDir?: string | null;
}

async function getJAgentDeskWorktreeForCwd(
  cwd: string,
  options: JAgentDeskWorktreeLookupOptions = {},
): Promise<JAgentDeskWorktreeForCwd> {
  // Fast-path reject: non-worktree paths do not need expensive ownership checks.
  if (!/[\\/]worktrees[\\/]/.test(cwd)) {
    return { isJAgentDeskOwnedWorktree: false };
  }

  const ownership = await isJAgentDeskOwnedWorktreeCwd(cwd, {
    jagentdeskHome: options.context?.jagentdeskHome,
...
function storedBaseRefFromMetadata(metadata: JAgentDeskWorktreeMetadata | null): string | null {
  return metadata?.baseRef ?? metadata?.baseRefName ?? null;
}

function readJAgentDeskWorktreeBaseRef(worktreeRoot: string): string | null {
  return storedBaseRefFromMetadata(readJAgentDeskWorktreeMetadata(worktreeRoot));
}
...
async function getStoredBaseRefForCwd(
  ...
  const jagentdeskWorktree = await getJAgentDeskWorktreeForCwd(cwd, { context });
  if (!jagentdeskWorktree.isJAgentDeskOwnedWorktree) {
    return null;
  }

  return readJAgentDeskWorktreeBaseRef(jagentdeskWorktree.worktreeRoot);
}
...
async function inspectCheckoutContext(
  ...
  const jagentdeskWorktree = await getJAgentDeskWorktreeForCwd(cwd, {
    context,
...
function buildPullRequestLookupTargetFromMetadata(
  ...
  const target = getJAgentDeskWorktreeChangeRequestHintForBranch(metadata, currentBranch);
  if (!target) {
    return null;
  }
  return {
...
export async function getCheckoutSnapshotFacts(
  ...
  const jagentdeskWorktreeMetadata = inspected.jagentdeskWorktree.isJAgentDeskOwnedWorktree
    ? readJAgentDeskWorktreeMetadata(inspected.jagentdeskWorktree.worktreeRoot)
    : null;
  const storedBaseRef = storedBaseRefFromMetadata(jagentdeskWorktreeMetadata);
  ...
  return {
    ...
    jagentdeskWorktree: inspected.jagentdeskWorktree,
    storedBaseRef,
    resolvedBaseRef,
    mainRepoRoot,
    comparisonBaseRef,
...
// Temporary until diffs load lazily per file. The JAgentDesk relay's 32 MiB frame limit is
...
export async function getCheckoutStatus(
  ...
  const jagentdeskWorktree = facts.jagentdeskWorktree;
  ...
  if (jagentdeskWorktree.isJAgentDeskOwnedWorktree && baseRef) {
    return {
      isGit: true,
...
function getUnavailablePullRequestStatus(
  ...
  if (
    facts?.isGit === true &&
    facts.jagentdeskWorktree.isJAgentDeskOwnedWorktree &&
