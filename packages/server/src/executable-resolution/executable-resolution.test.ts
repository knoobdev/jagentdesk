...
function makeTempDir(): string {
  const dir = mkdtempSync(path.join(os.tmpdir(), "jagentdesk-executable-test-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("findExecutable", () => {
  ...
  test("returns null when the command is not on PATH", async () => {
    ...
    await expect(findExecutable("jagentdesk-definitely-missing-command")).resolves.toBeNull();
  });
