...
} from "@jagentdesk/protocol/git-remote";
...
fragment JAgentDeskPollPullRequest on PullRequest {
  number
  url
  title
  state
...
function buildBatchPullRequestStatusQuery(
  ...
  const aliases = entries.map(
    ...
    pullRequests(headRefName: ${graphqlString(
      ...
      nodes {
        ...JAgentDeskPollPullRequest
      }
    }
  }`,
  );
  return `query JAgentDeskBatchPullRequestStatus {
  rateLimit {
    limit
    remaining
    resetAt
...
function buildBatchPullRequestChecksQuery(
  ...
  return `query JAgentDeskBatchPullRequestChecks {
  rateLimit {
    limit
    remaining
    resetAt
