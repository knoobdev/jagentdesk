...
function createFakeCliBinDir(): string {
  const dir = createTempDir("jagentdesk-cli-bin-");
  writeFileSync(join(dir, "jagentdesk"), "");
  return dir;
}
...
describe("Claude terminal agent hooks", () => {
  it("installs registered provider hooks idempotently", () => {
    const configDir = createTempDir("jagentdesk-claude-config-");
    ...
    for (const event of provider.events) {
      const jagentdeskCommands = hookCommands(settings, event.event).filter((command) =>
        command.includes(install.hookMarker),
      );
      expect(jagentdeskCommands).toHaveLength(1);
      expect(jagentdeskCommands[0]).toBe(
        `if [ -n "$JAGENTDESK_TERMINAL_ID" ]; then "\${JAGENTDESK_HOOK_CLI:-jagentdesk}" hooks ${provider.id} ${event.event}; fi`,
      );
    }
  ...
  it("preserves unrelated user hooks", () => {
    const configDir = createTempDir("jagentdesk-claude-config-preserve-");
    writeFileSync(
  ...
  it("uninstalls only marker-matched hooks", () => {
    const configDir = createTempDir("jagentdesk-claude-config-uninstall-");
    installRegisteredAgentHooks({ configDir });
    const settings = readSettings(configDir);
    settings.hooks = {
      ...settings.hooks,
  ...
  it("builds a minimal gated hook command", () => {
    ...
    expect(command).toBe(
      'if [ -n "$JAGENTDESK_TERMINAL_ID" ]; then "${JAGENTDESK_HOOK_CLI:-jagentdesk}" hooks claude UserPromptSubmit; fi',
    );
  });

  it.skipIf(isPlatform("win32")).each(AGENT_HOOK_PROVIDERS.claude.events)(
    "$event hook command exits 0 when JAGENTDESK_TERMINAL_ID is unset",
    (event) => {
      const provider = AGENT_HOOK_PROVIDERS.claude;
      const command = buildAgentHookShellCommand(provider, event);

      const result = spawnSync("/bin/sh", ["-c", command], {
        env: { PATH: process.env.PATH ?? "", JAGENTDESK_HOOK_CLI: "jagentdesk" },
        stdio: "ignore",
      });
  ...
  it("prepends the jagentdesk CLI directory and injects the hook CLI path", () => {
    const cliBinDir = createFakeCliBinDir();
    const hookCliPath = join(cliBinDir, "jagentdesk");

    const env = buildTerminalEnvironment({
      shell: "/bin/sh",
      env: { PATH: ["/usr/bin", "/bin"].join(delimiter) },
      jagentdeskCliBinDir: cliBinDir,
      jagentdeskHookCliPath: hookCliPath,
    });

    expect(env.PATH?.split(delimiter)).toEqual([cliBinDir, "/usr/bin", "/bin"]);
    expect(env.JAGENTDESK_HOOK_CLI).toBe(hookCliPath);
  });
