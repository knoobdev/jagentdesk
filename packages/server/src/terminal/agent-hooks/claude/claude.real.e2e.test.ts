...
import { resolveJAgentDeskCliBinDir } from "../../terminal.js";
...
describe.skipIf(!claudeAvailability.available)(
  ...
  () => {
    it("posts running then idle through the installed Claude config hooks", async () => {
      ...
      const configDir = createTempDir("jagentdesk-real-claude-config-");
      const cwd = createTempDir("jagentdesk-real-claude-cwd-");
      const jagentdeskCliBinDir = resolveJAgentDeskCliBinDir();
      if (!jagentdeskCliBinDir) {
        throw new Error("Could not resolve jagentdesk CLI bin directory");
      }
      ...
      try {
        const claude = pty.spawn(
          ...
          ["--settings", join(configDir, "settings.json"), "-p", "reply with: done"],
          {
            ...
            env: {
              ...process.env,
              JAGENTDESK_TERMINAL_ID: terminalId,
              JAGENTDESK_ACTIVITY_TOKEN: token,
              JAGENTDESK_TERMINAL_ACTIVITY_URL: recorder.url,
              PATH: [jagentdeskCliBinDir, process.env.PATH].filter(isString).join(delimiter),
            },
          },
        );
