...
describe("Codex terminal agent hooks", () => {
  it("installs POSIX and Windows hook commands idempotently", () => {
    const configDir = createTempDir("jagentdesk-codex-config-");
    ...
    for (const event of codexAgentHookProvider.events) {
      expect(commandHooks(config, event.event)).toEqual([
        {
          command: `if [ -n "$JAGENTDESK_TERMINAL_ID" ]; then "\${JAGENTDESK_HOOK_CLI:-jagentdesk}" hooks codex ${event.event}; fi`,
          commandWindows: `if defined JAGENTDESK_TERMINAL_ID (if defined JAGENTDESK_HOOK_CLI ("%JAGENTDESK_HOOK_CLI%" hooks codex ${event.event}) else (jagentdesk hooks codex ${event.event})) else (exit /b 0)`,
        },
      ]);
    }
  ...
  it("preserves unrelated user hooks", () => {
    const configDir = createTempDir("jagentdesk-codex-config-preserve-");
    writeFileSync(
    ...
    expect(stopCommands).toEqual([
      "say codex done",
      'if [ -n "$JAGENTDESK_TERMINAL_ID" ]; then "${JAGENTDESK_HOOK_CLI:-jagentdesk}" hooks codex Stop; fi',
    ]);
  });

  it("uninstalls only marker-matched hooks", () => {
    const configDir = createTempDir("jagentdesk-codex-config-uninstall-");
    installAgentHooks(codexAgentHookProvider, { configDir });
    const config = readHooksFile(configDir);
    config.hooks = {
      ...config.hooks,
