...
describe("OpenCode terminal agent hooks", () => {
  it("installs a self-contained OpenCode plugin idempotently", () => {
    const configDir = createTempDir("jagentdesk-opencode-config-");
    ...
    expect(firstInstall.configPath).toBe(join(configDir, "plugins", "jagentdesk-terminal-activity.js"));
  ...
  it("writes the plugin that maps OpenCode bus events to jagentdesk hook events", () => {
    const configDir = createTempDir("jagentdesk-opencode-config-source-");
    ...
    expect(source).toContain('Bun.spawn(["jagentdesk", "hooks", "opencode", event]');
    expect(source).toContain("JAGENTDESK_TERMINAL_ID");
  });

  it("uninstalls the OpenCode plugin file", () => {
    const configDir = createTempDir("jagentdesk-opencode-config-uninstall-");
  ...
  it("prefers OPENCODE_CONFIG_DIR over the XDG config home", () => {
    const homeDir = createTempDir("jagentdesk-home-");
    const configDir = createTempDir("jagentdesk-opencode-override-");
    const xdgConfigHome = createTempDir("jagentdesk-xdg-config-");
    ...
    expect(configPath).toBe(join(configDir, "plugins", "jagentdesk-terminal-activity.js"));
  });
  ...
  it("uses the XDG config home for the default OpenCode config dir", () => {
    ...
    expect(configPath).toBe(
      join(xdgConfigHome, "opencode", "plugins", "jagentdesk-terminal-activity.js"),
    );
  });
  ...
  it("falls back to the home .config OpenCode dir without an XDG config home", () => {
    ...
    expect(configPath).toBe(
      join(homeDir, ".config", "opencode", "plugins", "jagentdesk-terminal-activity.js"),
    );
  });
