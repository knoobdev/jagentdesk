...
function hookPaths(root: string) {
  return {
    ...
    opencode: join(root, "opencode", "plugins", "jagentdesk-terminal-activity.js"),
  };
}

function createStore(jagentdeskHome: string, enableTerminalAgentHooks: boolean): DaemonConfigStore {
...
describe("applyTerminalAgentHookSetting", () => {
  it("leaves agent configs untouched when the setting is disabled", () => {
    const root = createTempDir("jagentdesk-hook-setting-");
    const store = createStore(createTempDir("jagentdesk-hook-setting-home-"), false);
  ...
  it("installs agent hooks when the setting is enabled", () => {
    const root = createTempDir("jagentdesk-hook-setting-");
    const store = createStore(createTempDir("jagentdesk-hook-setting-home-"), true);
