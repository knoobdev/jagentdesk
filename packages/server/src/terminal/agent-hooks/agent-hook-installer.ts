...
export function buildAgentHookShellCommand<TConfig>(
  ...
  const hookCommand = `"\${JAGENTDESK_HOOK_CLI:-jagentdesk}" hooks ${shellToken(provider.id)} ${shellToken(event.event)}`;
  return `if [ -n "$JAGENTDESK_TERMINAL_ID" ]; then ${hookCommand}; fi`;
}
...
export function buildAgentHookWindowsCommand<TConfig>(
  ...
  return `if defined JAGENTDESK_TERMINAL_ID (if defined JAGENTDESK_HOOK_CLI ("%JAGENTDESK_HOOK_CLI%" ${hookArgs}) else (jagentdesk ${hookArgs})) else (exit /b 0)`;
}
