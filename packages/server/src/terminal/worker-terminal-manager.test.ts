...
import {
  resolveJAgentDeskCliBinDir,
  resolveJAgentDeskCliExecutablePath,
  type TerminalSession,
} from "./terminal.js";
import type { TerminalState } from "@jagentdesk/protocol/messages";
...
it("delivers rapid small writes complete and in order through worker coalescing", async () => {
  ...
  const session = trackTerminal(
    await manager.createTerminal({
      workspaceId: "ws-test",
      cwd,
      env: { JAGENTDESK_TERMINAL_BURST_GATE: burstGatePath },
      ...nodeTerminalCommand(`
      const fs = require("node:fs");
      const gatePath = process.env.JAGENTDESK_TERMINAL_BURST_GATE;
...
it("keeps registered cwd env inheritance behind the worker manager interface", async () => {
  ...
  manager.registerCwdEnv({
    cwd,
    env: { JAGENTDESK_WORKER_TERMINAL_TEST: "worker-env" },
  });
  trackTerminal(
    await manager.createTerminal({
      workspaceId: "ws-test",
      ...
      require("node:fs").writeFileSync(
        ...
        process.env.JAGENTDESK_WORKER_TERMINAL_TEST ?? "",
      );
      setInterval(() => {}, 1000);
    `),
    }),
...
it("injects parent-minted terminal activity env through the worker", async () => {
    ...
    await manager.createTerminal({
      ...
      ...nodeTerminalCommand(`
        require("node:fs").writeFileSync(
          ...
          JSON.stringify({
            terminalId: process.env.JAGENTDESK_TERMINAL_ID,
            token: process.env.JAGENTDESK_ACTIVITY_TOKEN,
            url: process.env.JAGENTDESK_TERMINAL_ACTIVITY_URL,
            hookCli: process.env.JAGENTDESK_HOOK_CLI,
  ...
  const jagentdeskCliBinDir = resolveJAgentDeskCliBinDir();
  const jagentdeskCliPath = resolveJAgentDeskCliExecutablePath();
  expect(jagentdeskCliBinDir).not.toBeNull();
  expect(jagentdeskCliPath).not.toBeNull();
  ...
  expect(env.hookCli).toBe(jagentdeskCliPath);
  ...
  expect(env.path?.split(delimiter)[0]).toBe(jagentdeskCliBinDir);
});
