# AGENTS.md — JAgentDesk Server Development Guide

For AI coding agents working in `packages/server`. Supplements [CLAUDE.md](../CLAUDE.md) at the repo root.

## Project Overview

JAgentDesk is a mobile + CLI app for monitoring and controlling local AI coding agents (Claude Code, Codex, GitHub Copilot, OpenCode, Pi). The daemon runs on your machine, manages agent processes, and streams their output over WebSocket to clients.
...
npm run build:server                 # Build: highlight + protocol + client + server + cli
...
Agent state persists to `$JAGENTDESK_HOME/agents/{cwd-with-dashes}/{agent-id}.json`  
Daemon logs: `$JAGENTDESK_HOME/daemon.log`
...
```bash
tail -f $JAGENTDESK_HOME/daemon.log      # Daemon logs
...
| [../SECURITY.md](../SECURITY.md)                           | Tailscale transport and E2E encryption           |
