import { getJAgentDeskToolLeafName } from "./tool-name-normalization.js";

export interface JAgentDeskToolDetailField {
  label: string;
  value: string;
}

export type JAgentDeskToolDetailSection =
  | {
      kind: "prose";
      title: string;
      text: string;
  ...
  | {
      kind: "fields";
      title: string;
      fields: JAgentDeskToolDetailField[];
    };
...
export function buildJAgentDeskToolDetailSections(
  toolName: string,
  input: unknown,
  output: unknown,
): JAgentDeskToolDetailSection[] | null {
  const leafName = getJAgentDeskToolLeafName(toolName);
  if (!leafName) return null;

  const spec = TOOL_SPECS[leafName] ?? {};
  const sections: JAgentDeskToolDetailSection[] = [];
