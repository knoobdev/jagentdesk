...
export function isJAgentDeskToolName(name: string): boolean {
  const normalized = normalizeToolName(name);
  ...
  if (normalized.includes("__")) {
    ...
    return (
      segments.length >= 3 &&
      segments[0] === "mcp" &&
      (segments[1] === "jagentdesk" || segments[1].startsWith("jagentdesk_"))
    );
  }
  if (normalized.includes(".")) {
    const firstSegment = normalized.split(".")[0];
    return firstSegment === "jagentdesk" || firstSegment.startsWith("jagentdesk_");
  }
  return false;
}

export function getJAgentDeskToolLeafName(name: string): string | null {
  const normalized = normalizeToolName(name);
  if (normalized.includes("__")) {
    const segments = normalized.split("__").filter((s) => s.length > 0);
    if (
  ...
  if (normalized.includes(".")) {
    const firstSegment = normalized.split(".")[0];
    if (firstSegment === "jagentdesk" || firstSegment.startsWith("jagentdesk_")) {
