...
import { JAgentDeskConfigRawSchema, JAgentDeskConfigSchema } from "@jagentdesk/protocol/jagentdesk-config-schema";

describe("jagentdesk config schema", () => {
  it("parses an empty config without metadata generation", () => {
    const parsed = JAgentDeskConfigSchema.parse({});
  ...
  it("parses old-style worktree and scripts config unchanged", () => {
    ...
    expect(JAgentDeskConfigSchema.parse(config)).toEqual({
      worktree: {
        setup: ["npm install"],
        teardown: ["npm run clean"],
      },
  ...
  it("rejects invalid service port ranges", () => {
    expect(() =>
      JAgentDeskConfigRawSchema.parse({ worktree: { servicePorts: { range: "4000-3000" } } }),
