...
import {
  ...
  getJAgentDeskBrowserWebviewRegistry,
  listRegisteredJAgentDeskBrowserIds,
  isJAgentDeskBrowserWebviewAttach,
  prepareJAgentDeskBrowserWebContents,
  PendingBrowserWindowOpenRequests,
  registerBrowserWebviewNavigationGuards,
  unregisterJAgentDeskBrowserFromHost,
  registerAttachedJAgentDeskBrowser,
  setWorkspaceActiveJAgentDeskBrowserId,
  unregisterJAgentDeskBrowserHost,
} from "./features/browser-webviews/index.js";
import {
  clearJAgentDeskBrowserProfile,
  getLegacyJAgentDeskBrowserProfileSession,
  JAGENTDESK_BROWSER_PROFILE_PARTITION,
  getJAgentDeskBrowserProfileSession,
...
// The desktop app must never discover or attach to a daemon owned by another
// installation (notably JAgentDesk). Keep the managed daemon's config, PID lock,
// Tailscale state, and status file in an app-owned home. The renderer settings
// continue to live in Electron's userData directory; this home is only for the
// daemon runtime and is intentionally isolated from legacy installs.
if (!process.env.JAGENTDESK_HOME) {
  process.env.JAGENTDESK_HOME = path.join(app.getPath("appData"), "JAgentDesk", "daemon");
}
// Keep the managed JAgentDesk daemon off JAgentDesk's legacy default port. This is
// also the port exposed by the embedded tsnet bridge, so every desktop status
// probe and every mobile tailnet connection refers to the same endpoint.
if (!process.env.JAGENTDESK_LISTEN) {
  process.env.JAGENTDESK_LISTEN = "127.0.0.1:6768";
}
// Packaged desktop builds keep the Go/tsnet bridge in Electron's resources
// directory. Point the daemon at that real filesystem path; an executable
// inside app.asar cannot be spawned on macOS (ENOTDIR).
const packagedTsnetBridge = path.join(process.resourcesPath, "bin", "tailnet-bridge");
if (!process.env.JAGENTDESK_TSNET_BRIDGE_BIN && existsSync(packagedTsnetBridge)) {
  process.env.JAGENTDESK_TSNET_BRIDGE_BIN = packagedTsnetBridge;
}
...
const browserKeyboard = new BrowserKeyboard(getJAgentDeskBrowserWebviewRegistry());
          ...
          { type: "separator" as const },
          {
            ...
            click: () => {
              log.info("[browser-devtools] inspect-element.request", {
                ...
                browserId: getJAgentDeskBrowserIdForWebContents(contents),
                x: params.x,
                y: params.y,
...
function getBrowserPopupWindowOptions(
  ...
  return {
    ...
    webPreferences: {
      partition: JAGENTDESK_BROWSER_PROFILE_PARTITION,
...
function installBrowserWindowOpenHandler(input: {
  ...
  contents.setWindowOpenHandler(({ url, disposition, frameName, features, postBody }) => {
    ...
    const sourceBrowserId = getJAgentDeskBrowserIdForWebContents(sourceContents);
    if (sourceBrowserId) {
...
const forcedUserDataDir = process.env.JAGENTDESK_ELECTRON_USER_DATA_DIR?.trim();
if (forcedUserDataDir) {
  ...
  try {
    ...
    // Main checkout (e.g. "jagentdesk") gets default userData — only worktrees diverge.
    ...
    if (isWorktree) {
      app.setPath("userData", path.join(app.getPath("appData"), `JAgentDesk-${devWorktreeName}`));
...
// Allow users to pass Chromium flags via JAGENTDESK_ELECTRON_FLAGS for debugging
// rendering issues (e.g. "--disable-gpu --ozone-platform=x11").
// Must run before app.whenReady().
const electronFlags = process.env.JAGENTDESK_ELECTRON_FLAGS?.trim();
if (electronFlags) {
...
ipcMain.handle("jagentdesk:get-pending-open-project", (event) => {
...
ipcMain.handle("jagentdesk:agent-navigation:ready", (event) => {
...
ipcMain.handle("jagentdesk:browser:register-attached", (event, rawInput: unknown) => {
  const input = readAttachedBrowserInput(rawInput);
  if (!input) {
    throw new Error("Invalid attached browser registration");
  }
  const registered = registerAttachedJAgentDeskBrowser({
    ...input,
    sender: event.sender,
    profileSession: getJAgentDeskBrowserProfileSession(session),
  ...
  log.info("[browser-webview] registered", {
    ...
    registeredBrowserIds: listRegisteredJAgentDeskBrowserIds(),
  });
...
ipcMain.handle("jagentdesk:browser:clear-profile", async (_event, rawLegacyBrowserIds: unknown) => {
  const profileSessions = getJAgentDeskBrowserProfileSessions(
    session,
    readLegacyJAgentDeskBrowserIds(rawLegacyBrowserIds),
  );
...
ipcMain.handle(
  "jagentdesk:browser:capture-element",
...
ipcMain.handle("jagentdesk:browser:copy-element", (_event, payload: unknown): boolean => {
...
async function createWindow(
  ...
  mainWindow.webContents.on("will-attach-webview", (event, webPreferences, params) => {
    if (!isJAgentDeskBrowserWebviewAttach(params)) {
...
function focusExistingWindowOnAgent(target: AgentDeepLinkTarget): void {
  ...
  if (deliverable) {
    mainWindow.webContents.send("jagentdesk:event:open-agent", deliverable);
  }
}
...
function setupSingleInstanceLock(): boolean {
  if (DISABLE_SINGLE_INSTANCE_LOCK) {
    log.info("[single-instance] disabled by JAGENTDESK_DISABLE_SINGLE_INSTANCE_LOCK");
    return true;
  }
  ...
  app.on("second-instance", (_event, commandLine) => {
    ...
    // Relaunching the app (CLI `jagentdesk [path]`, double-click, etc.) opens a new
...
async function bootstrap(): Promise<void> {
  ...
  ipcMain.handle("jagentdesk:window:openNew", async (_event, options?: unknown) => {
    const pendingPath =
...
function showDaemonShutdownDialog(): void {
  for (const win of BrowserWindow.getAllWindows()) {
    win.webContents.send("jagentdesk:event:quitting", {});
  }
}
