...
export function registerDialogHandlers(): void {
  ipcMain.handle("jagentdesk:dialog:ask", async (event, message: string, options?: AskOptions) => {
  ...
  ipcMain.handle(
    "jagentdesk:dialog:askWithCheckbox",
  ...
  ipcMain.handle("jagentdesk:dialog:open", async (event, options?: OpenOptions) => {
    const win = BrowserWindow.fromWebContents(event.sender);
