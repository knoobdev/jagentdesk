...
describe("browser webview attachment", () => {
  test("accepts only allowed URLs on the shared profile partition", () => {
    ...
    expect(
      isJAgentDeskBrowserWebviewAttach({
        src: "https://example.com",
        partition: "persist:jagentdesk-browser-tab-a",
      }),
    ).toBe(false);
    expect(
      isJAgentDeskBrowserWebviewAttach({ src: "https://example.com", partition: "persist:foreign" }),
    ).toBe(false);
  });
  ...
  test("binds explicit browser identity to the renderer that hosts the guest", () => {
    ...
    expect(getJAgentDeskBrowserIdForWebContents(guest)).toBe("browser-a");
    expect(getJAgentDeskBrowserWorkspaceId("browser-a")).toBe("workspace-a");
    unregisterJAgentDeskBrowser("browser-a");
  });
  ...
  test("rejects a guest hosted by another renderer", () => {
    ...
    expect(getJAgentDeskBrowserIdForWebContents(guest)).toBeNull();
  });
  ...
  test("concurrent windows cannot swap browser identities", () => {
    ...
    expect(getJAgentDeskBrowserIdForWebContents(firstGuest)).toBe("browser-first");
    expect(getJAgentDeskBrowserIdForWebContents(secondGuest)).toBe("browser-second");
    unregisterJAgentDeskBrowser("browser-first");
    unregisterJAgentDeskBrowser("browser-second");
  });
  ...
  test("unregisters the same browser only from its requesting host", () => {
    ...
    unregisterJAgentDeskBrowserFromHost(firstRenderer.id, "browser-shared-hosts");

    expect(getJAgentDeskBrowserIdForWebContents(firstGuest)).toBeNull();
    expect(getJAgentDeskBrowserIdForWebContents(secondGuest)).toBe("browser-shared-hosts");
    expect(getJAgentDeskBrowserWorkspaceId("browser-shared-hosts")).toBe("workspace-shared");
    unregisterJAgentDeskBrowser("browser-shared-hosts");
  });
  ...
  test("prepares throttling once and removes registration when the guest is destroyed", () => {
    ...
    prepareJAgentDeskBrowserWebContents(guest);
    registerAttachedJAgentDeskBrowser({
    ...
    expect(getJAgentDeskBrowserIdForWebContents(guest)).toBe("browser-cleanup");
