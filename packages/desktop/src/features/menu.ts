...
import { getActiveJAgentDeskBrowserWebContentsForHostWindow } from "./browser-webviews/index.js";
        ...
        { type: "separator" },
        {
          ...
          click: withBrowserWindow((win) => {
            reloadActiveBrowserOrWindow({
              win,
              getActiveBrowserContentsForHostWindow: getActiveJAgentDeskBrowserWebContentsForHostWindow,
            });
          }),
        },
        {
...
export function setupApplicationMenu(options: ApplicationMenuOptions): void {
  ...
  ipcMain.handle("jagentdesk:menu:showContextMenu", (event, input?: ShowContextMenuInput) => {
  ...
  ipcMain.handle("jagentdesk:menu:set-capturing-shortcut", (_event, capturing?: boolean) => {
