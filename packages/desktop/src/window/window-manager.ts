...
export function registerWindowManager(): void {
  ...
  ipcMain.handle("jagentdesk:window:toggleMaximize", (event) => {
  ...
  ipcMain.handle("jagentdesk:window:isFullscreen", (event) => {
    const win = BrowserWindow.fromWebContents(event.sender);
    return win?.isFullScreen() ?? false;
  });

  ipcMain.handle("jagentdesk:window:setFullscreen", (event, fullscreen: unknown) => {
    if (typeof fullscreen !== "boolean") return;
    BrowserWindow.fromWebContents(event.sender)?.setFullScreen(fullscreen);
  });

  ipcMain.handle("jagentdesk:window:setBadgeCount", (_event, count?: unknown) => {
  ...
  ipcMain.handle("jagentdesk:window:updateWindowControls", (event, update?: unknown) => {
    const win = BrowserWindow.fromWebContents(event.sender);
    if (!win) {
      return;
    }
...
export function setupWindowResizeEvents(win: BrowserWindow): void {
  ...
  const notifyResized = () => {
    ...
    win.webContents.send("jagentdesk:window:resized", {});
  };
