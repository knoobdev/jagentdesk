...
const PREDECESSOR_SKILL_NAMES = [
  "jagentdesk",
  "jagentdesk-loop",
  "jagentdesk-advisor",
  "jagentdesk-committee",
  "jagentdesk-handoff",
] as const;
...
export const LEGACY_SKILL_NAMES = [
  "jagentdesk-chat",
  "jagentdesk-epic",
  "jagentdesk-orchestrate",
  "jagentdesk-orchestrator",
  // COMPAT(predecessorSkills): old bundled skill directories are still scanned
  // so an upgrade can report and clean them without exposing them as part of
  // the JAgentDesk catalog.
  ...PREDECESSOR_SKILL_NAMES,
] as const;
...
/** Every name JAgentDesk owns on disk: what it ships now plus what it used to ship. */
...
function hasInstalledJAgentDeskSkill(disks: readonly TargetSkills[]): boolean {
...
function installedSkillNames(disks: readonly TargetSkills[], names: readonly string[]): string[] {
  return names.filter(
    (name) =>
      !PREDECESSOR_SKILL_NAMES.includes(name as (typeof PREDECESSOR_SKILL_NAMES)[number]) &&
      disks.some((disk) => disk.has(name)),
  );
}
...
export async function getSkillsStatus(
  ...
  if (!hasInstalledJAgentDeskSkill(disks))
