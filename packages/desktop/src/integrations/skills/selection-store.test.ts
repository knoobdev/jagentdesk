...
async function createTempUserDataDir(): Promise<string> {
  const dir = await mkdtemp(path.join(os.tmpdir(), "jagentdesk-skill-selection-"));
  directories.add(dir);
  return dir;
}
...
describe("createSkillSelectionStore", () => {
  ...
  it("round-trips a custom selection through a fresh store", async () => {
    ...
    expect(saved).toEqual({ mode: "custom", skills: ["jagentdesk", "jagentdesk-loop"] });
    expect(reloaded).toEqual({ mode: "custom", skills: ["jagentdesk", "jagentdesk-loop"] });
  ...
  it("sorts, de-duplicates, and drops non-string names before persisting", async () => {
    ...
    const saved = await store.set({
      mode: "custom",
      skills: ["jagentdesk-loop", "jagentdesk", "jagentdesk-loop", "  ", 7, null],
    });
  ...
  it("switches back to all and forgets the previous custom names", async () => {
    const userDataPath = await createTempUserDataDir();
    const store = createSkillSelectionStore({ userDataPath });
    await store.set({ mode: "custom", skills: ["jagentdesk"] });
  ...
  it("falls back to all when the saved mode is unknown", async () => {
    ...
    await seedSelectionFile(
      ...
      JSON.stringify({
        version: 1,
        selection: { mode: "some-future-mode", skills: ["jagentdesk"] },
      }),
    );
  ...
  it("normalizes a custom selection that was hand-edited on disk", async () => {
    ...
    await seedSelectionFile(
      ...
      JSON.stringify({
        version: 1,
        selection: { mode: "custom", skills: ["jagentdesk-loop", 3, "jagentdesk", "jagentdesk"] },
      }),
    );
  ...
  it("leaves no temp files behind when saves overlap", async () => {
    ...
    await Promise.all([
      store.set({ mode: "custom", skills: ["jagentdesk"] }),
      store.set({ mode: "all" }),
      store.set({ mode: "custom", skills: ["jagentdesk-loop"] }),
    ]);
