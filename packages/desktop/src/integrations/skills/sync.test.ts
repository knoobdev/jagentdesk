...
async function makeSandbox(): Promise<Sandbox> {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "jagentdesk-skill-sync-"));
...
describe("syncSkills", () => {
  ...
  it("overwrites on-disk skill content when the bundle differs", async () => {
    await writeBundleSkill(sandbox.sourceDir, "jagentdesk", {
      "SKILL.md": "new jagentdesk content",
    });
    const onDiskSkill = path.join(sandbox.agentsDir, "jagentdesk");
    await fs.mkdir(onDiskSkill, { recursive: true });
    await fs.writeFile(path.join(onDiskSkill, "SKILL.md"), "old jagentdesk content");
    ...
    const result = await syncSkills({
      ...
      skillNames: ["jagentdesk"],
    });

    expect(result.processedSkills).toBe(1);
    expect(result.changedFiles).toBeGreaterThan(0);
    const agentsContent = await fs.readFile(
      path.join(sandbox.agentsDir, "jagentdesk", "SKILL.md"),
      "utf-8",
    );
    expect(agentsContent).toBe("new jagentdesk content");
    const claudeContent = await fs.readFile(
      path.join(sandbox.claudeDir, "jagentdesk", "SKILL.md"),
      "utf-8",
    );
    expect(claudeContent).toBe("new jagentdesk content");
    const codexContent = await fs.readFile(
      path.join(sandbox.codexDir, "jagentdesk", "SKILL.md"),
      "utf-8",
    );
    expect(codexContent).toBe("new jagentdesk content");
  });

  it("installs new bundled skills, including references/, when not present on disk", async () => {
    await writeBundleSkill(sandbox.sourceDir, "jagentdesk-committee", {
    ...
    await syncSkills({
      ...
      claudeDir: sandbox.claudeDir,
      codexDir: sandbox.codexDir,
      skillNames: ["jagentdesk-committee"],
    });

    expect(
      await fs.readFile(path.join(sandbox.agentsDir, "jagentdesk-committee", "SKILL.md"), "utf-8"),
    ).toBe("committee content");
    expect(
      await fs.readFile(
        path.join(sandbox.agentsDir, "jagentdesk-committee", "references", "roles.md"),
        "utf-8",
      ),
    ).toBe("roles content");
    expect(
      await fs.readFile(
        path.join(sandbox.codexDir, "jagentdesk-committee", "references", "roles.md"),
        "utf-8",
      ),
    ).toBe("roles content");

    const claudeSkillDir = path.join(sandbox.claudeDir, "jagentdesk-committee");
  ...
  it("leaves on-disk skills not in the bundle untouched", async () => {
    await writeBundleSkill(sandbox.sourceDir, "jagentdesk", { "SKILL.md": "new" });
  ...
  it("removes stale files previously written to managed skill dirs", async () => {
    ...
    await fs.rm(path.join(sandbox.sourceDir, "jagentdesk"), { recursive: true, force: true });
    await writeBundleSkill(sandbox.sourceDir, "jagentdesk", { "SKILL.md": "new" });
  ...
  it("skips skills listed in skillNames that are missing from the bundle without raising", async () => {
    await writeBundleSkill(sandbox.sourceDir, "jagentdesk", { "SKILL.md": "content" });
    // "jagentdesk-removed" is in skillNames but not in the bundle on disk.
    ...
    const result = await syncSkills({
      ...
      claudeDir: sandbox.claudeDir,
      codexDir: sandbox.codexDir,
      skillNames: ["jagentdesk", "jagentdesk-removed"],
      onSkillError: (name, error) => errors.push({ name, error }),
    });

    expect(errors).toEqual([]);
    expect(result.processedSkills).toBe(1);
    expect(await fs.readFile(path.join(sandbox.agentsDir, "jagentdesk", "SKILL.md"), "utf-8")).toBe(
      "content",
    );
    await expect(fs.access(path.join(sandbox.agentsDir, "jagentdesk-removed"))).rejects.toThrow();
  });

  it("leaves on-disk skill content alone when the skill has been removed from the bundle", async () => {
    await writeBundleSkill(sandbox.sourceDir, "jagentdesk", { "SKILL.md": "current" });
    const deprecatedDir = path.join(sandbox.agentsDir, "jagentdesk-deprecated");
    ...
    await syncSkills({
      ...
      claudeDir: sandbox.claudeDir,
      codexDir: sandbox.codexDir,
      skillNames: ["jagentdesk", "jagentdesk-deprecated"],
    });
  ...
  it("reports per-skill errors via onSkillError without throwing", async () => {
    ...
    await fs.writeFile(path.join(sandbox.agentsDir, "jagentdesk"), "blocking file");
    ...
    expect(errors).toContain("jagentdesk");
...
describe("removeSkill", () => {
  ...
  it("removes the skill from all three targets when present", async () => {
    ...
    await removeSkill("jagentdesk", {
      agentsDir: sandbox.agentsDir,
      claudeDir: sandbox.claudeDir,
      codexDir: sandbox.codexDir,
    });

    await expect(fs.access(path.join(sandbox.agentsDir, "jagentdesk"))).rejects.toThrow();
    await expect(fs.access(path.join(sandbox.claudeDir, "jagentdesk"))).rejects.toThrow();
    await expect(fs.access(path.join(sandbox.codexDir, "jagentdesk"))).rejects.toThrow();
  });
