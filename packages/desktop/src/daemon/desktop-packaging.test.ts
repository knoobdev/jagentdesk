...
function createFakeMacBundle(options: { includeHelper: boolean }): {
  ...
  const root = mkdtempSync(join(tmpdir(), "jagentdesk-cli-shim-test-"));
  const appPath = join(root, "JAgentDesk.app");
  const contentsPath = join(appPath, "Contents");
  const resourcesPath = join(contentsPath, "Resources");
  const shimPath = join(resourcesPath, "bin", "jagentdesk");
  const mainPath = join(contentsPath, "MacOS", "JAgentDesk");
  const helperPath = join(
    contentsPath,
    "Frameworks",
    "JAgentDesk Helper.app",
    "Contents",
    "MacOS",
    "JAgentDesk Helper",
  );
  ...
  copyFileSync(join(packageRoot, "bin", "jagentdesk"), shimPath);
  ...
  if (options.includeHelper) {
    ...
    writeExecutable(
      ...
      [
        "#!/bin/sh",
        'printf "helper env=%s/%s cli=%s\\n" "$ELECTRON_RUN_AS_NODE" "$JAGENTDESK_NODE_ENV" "$JAGENTDESK_CLI"',
        'printf "args=%s\\n" "$*"',
        "",
      ].join("\n"),
    );
...
describe("desktop packaging", () => {
  it("unpacks server zsh shell integration files for external shells", () => {
    ...
    expect(config).toContain(
      "node_modules/@jagentdesk/server/dist/server/terminal/shell-integration/**/*",
    );
    expect(config).not.toContain(
      "node_modules/@jagentdesk/server/dist/src/terminal/shell-integration/**/*",
    );
  });
  ...
  it("excludes package debug/source files from the packaged app", () => {
    ...
    expect(config).toContain("!node_modules/@jagentdesk/*/src/**");
    expect(config).toContain("!node_modules/@jagentdesk/**/*.test.*");
    expect(config).toContain("!node_modules/@jagentdesk/**/*.spec.*");
  });

  it("excludes the bundled daemon web UI from the packaged app", () => {
    const config = readFileSync(join(packageRoot, "electron-builder.yml"), "utf8");

    expect(config).toContain("!node_modules/@jagentdesk/server/dist/server/web-ui/**");
  });

  it("registers JAgentDesk agent links with the operating system", () => {
    const config = readFileSync(join(packageRoot, "electron-builder.yml"), "utf8");

    expect(config).toContain("name: JAgentDesk agent link");
    expect(config).toContain("- jagentdesk");
  });

  // electron-builder packs production dependencies declared in package.json into
  // app.asar. Runtime code in runtime-paths.ts and bin/jagentdesk dynamically resolves
  ...
  it("declares all workspace packages required at runtime", () => {
    ...
    for (const required of ["@jagentdesk/cli", "@jagentdesk/server"]) {
  ...
  it("launches the packaged macOS CLI through Helper instead of the main app executable", () => {
    ...
    try {
      ...
      expect(result.stdout).toContain("@jagentdesk/cli/dist/index.js");
  ...
  it("fails packaged macOS CLI startup when Helper is missing", () => {
    ...
    try {
      ...
      expect(result.stderr).toContain("Bundled JAgentDesk Helper executable not found");
      expect(result.stdout).not.toContain("main-executable");
    } finally {
      rmSync(bundle.root, { recursive: true, force: true });
    }
