...
import {
  ...
  validatePort,
  validateSshHost,
} from "@jagentdesk/protocol/ssh-transport";
...
function emitTransportEvent(payload: TransportEventPayload): void {
  for (const win of BrowserWindow.getAllWindows()) {
    win.webContents.send("jagentdesk:event:local-daemon-transport-event", payload);
  }
}
 ...
 *   ws+unix://./pipe/jagentdesk:/ws        (Windows named pipe)
 *
