...
const CLI_ENTRYPOINT: NodeEntrypointSpec = {
  entryPath: "/tmp/jagentdesk-cli.js",
  execArgv: ["--import", "tsx"],
};
...
describe("node-entrypoint-launcher", () => {
  describe("createNodeEntrypointInvocation", () => {
    it("uses the packaged runner when the desktop app is packaged", () => {
      expect(
        createNodeEntrypointInvocation({
          execPath: "/Applications/JAgentDesk.app/Contents/MacOS/JAgentDesk",
          isPackaged: true,
          packagedRunnerPath:
            "/Applications/JAgentDesk.app/Contents/Resources/app.asar/dist/daemon/node-entrypoint-runner.js",
        ...
        command: "/Applications/JAgentDesk.app/Contents/MacOS/JAgentDesk",
        args: [
          "--disable-warning=DEP0040",
          "/Applications/JAgentDesk.app/Contents/Resources/app.asar/dist/daemon/node-entrypoint-runner.js",
          "node-script",
          "/tmp/jagentdesk-cli.js",
          "ls",
          "--json",
        ],
        env: {
    ...
    it("uses the entrypoint directly in development", () => {
      expect(
        createNodeEntrypointInvocation({
          ...
          argvMode: "node-script",
          args: ["ls"],
          baseEnv: { PATH: "/usr/bin", JAGENTDESK_NODE_ENV: "production" },
        }),
      ).toEqual({
        command: "/opt/homebrew/bin/electron",
        args: ["--import", "tsx", "/tmp/jagentdesk-cli.js", "ls"],
        env: {
          PATH: "/usr/bin",
          ELECTRON_RUN_AS_NODE: "1",
