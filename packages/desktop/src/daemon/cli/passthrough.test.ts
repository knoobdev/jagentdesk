...
const originalDesktopCli = process.env.JAGENTDESK_DESKTOP_CLI;
...
describe("passthrough CLI", () => {
  afterEach(() => {
    ...
    if (originalDesktopCli === undefined) {
      delete process.env.JAGENTDESK_DESKTOP_CLI;
    } else {
      process.env.JAGENTDESK_DESKTOP_CLI = originalDesktopCli;
    }
  });
  ...
  it("returns null when no CLI args are provided", () => {
    expect(
      parsePassthroughCliArgs({
        argv: ["/Applications/JAgentDesk.app/Contents/MacOS/JAgentDesk"],
        isDefaultApp: false,
        forceCli: false,
      }),
    ).toBeNull();
  ...
  it("ignores macOS GUI launch arguments", () => {
    expect(
      parsePassthroughCliArgs({
        argv: ["/Applications/JAgentDesk.app/Contents/MacOS/JAgentDesk", "-psn_0_12345"],
        isDefaultApp: false,
        forceCli: false,
      }),
    ).toBeNull();
  ...
  it("ignores --no-sandbox injected by Linux wrapper", () => {
    expect(
      parsePassthroughCliArgs({
        argv: ["/usr/bin/JAgentDesk", "--no-sandbox", "status"],
        isDefaultApp: false,
        forceCli: false,
      }),
    ).toEqual(["status"]);
  ...
  it("returns null when only --no-sandbox is present", () => {
    expect(
      parsePassthroughCliArgs({
        argv: ["/usr/bin/JAgentDesk", "--no-sandbox"],
        isDefaultApp: false,
        forceCli: false,
      }),
    ).toBeNull();
    ...
    expect(
      parsePassthroughCliArgs({
        argv: [
          ...
          "/nix/store/jagentdesk-desktop/share/jagentdesk-desktop/electron-app",
          "--no-sandbox",
          "--class=jagentdesk-desktop",
          "daemon",
          "status",
        ],
        isDefaultApp: true,
  ...
  it("ignores Electron remote debugging switches", () => {
    expect(
      parsePassthroughCliArgs({
        argv: ["/usr/bin/JAgentDesk", "--remote-debugging-port=9233"],
        isDefaultApp: false,
        forceCli: false,
      }),
    ).toBeNull();
  ...
  it("passes --open-project through as a normal CLI arg", () => {
    expect(
      parsePassthroughCliArgs({
        argv: ["/Applications/JAgentDesk.app/Contents/MacOS/JAgentDesk", "--open-project", "/tmp/project"],
        isDefaultApp: false,
        forceCli: false,
      }),
