...
const CDP_PORT = process.env.JAGENTDESK_ELECTRON_REMOTE_DEBUGGING_PORT ?? "9223";
...
async function readBridgeFullscreen(page) {
  return page.evaluate(
    async () =>
      (await window.jagentdeskDesktop?.window?.getCurrentWindow?.()?.isFullscreen?.()) === true,
  );
}

async function setNativeFullscreen(page, fullscreen) {
  await page.evaluate(async (nextFullscreen) => {
    const win = window.jagentdeskDesktop?.window?.getCurrentWindow?.();
...
async function inspectFullscreenWindowChrome(page, platform) {
  ...
  try {
    ...
    const details = await page.evaluate(async () => {
      const bridge = window.jagentdeskDesktop?.window?.getCurrentWindow?.();
...
async function detectDesktopBridge(page) {
  return page.evaluate(() => {
    const bridge = window.jagentdeskDesktop;
