...
export function resolveDevRuntime(env = process.env) {
  const remoteDebuggingPort = env.JAGENTDESK_ELECTRON_REMOTE_DEBUGGING_PORT
    ? parsePort(env.JAGENTDESK_ELECTRON_REMOTE_DEBUGGING_PORT, "JAGENTDESK_ELECTRON_REMOTE_DEBUGGING_PORT")
    : 0;
  return {
    electronFlags: buildElectronFlags(env.JAGENTDESK_ELECTRON_FLAGS, remoteDebuggingPort),
    userDataDir: resolveDevUserDataDir({
      devRoot: env.JAGENTDESK_DEV_ROOT,
      inheritedUserDataDir: env.JAGENTDESK_ELECTRON_USER_DATA_DIR,
      fallbackRoot: env.JAGENTDESK_DEV_RUNTIME_FALLBACK_ROOT,
    }),
  };
}
