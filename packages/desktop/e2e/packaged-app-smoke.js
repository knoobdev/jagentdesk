...
const EXECUTABLE_NAME = "JAgentDesk";
...
function getCliShimPath(appPath) {
  if (process.platform === "darwin") {
    return path.join(appPath, "Contents", "Resources", "bin", "jagentdesk");
  }

  if (process.platform === "win32") {
    return path.join(appPath, "resources", "bin", "jagentdesk.cmd");
  }

  return path.join(appPath, "resources", "bin", "jagentdesk");
}
...
function getTerminalHookSmokeCommand(marker) {
  if (process.platform === "win32") {
    const script = [
      "& $env:JAGENTDESK_HOOK_CLI hooks codex Stop",
  ...
  return `"$JAGENTDESK_HOOK_CLI" hooks codex Stop && echo ${marker}`;
}
...
function createDefaultDaemonEnv(extraEnv) {
  ...
  delete env.JAGENTDESK_HOME;
  delete env.JAGENTDESK_LISTEN;
  return env;
}
...
async function writeFailureArtifacts({ page, stdout, stderr, userData, daemonHome, error }) {
  const artifactDir = process.env.JAGENTDESK_DESKTOP_SMOKE_ARTIFACT_DIR?.trim();
  if (!artifactDir) {
    return;
  }
  ...
  if (page) {
    const renderer = await page
      .evaluate(() => ({
        ...
        bridgeKeys:
          typeof window.jagentdeskDesktop === "object" && window.jagentdeskDesktop !== null
            ? Object.keys(window.jagentdeskDesktop)
            : [],
      }))
...
async function waitForPackagedAppPage(browser, deadline) {
  while (Date.now() < deadline) {
    const page = browser
      ...
      .find((candidate) => candidate.url().startsWith("jagentdesk://app/"));
    if (page) {
      return page;
    }
    await delay(250);
  }
  throw new Error("Timed out waiting for the packaged jagentdesk://app/ renderer");
}
...
async function smokeColdCliDaemonStart({ appPath }) {
  const home = createTempDir("jagentdesk-smoke-cli-daemon-home-");
  const pidPath = path.join(home, "jagentdesk.pid");
...
async function smokeCliTerminal({ appPath, env }) {
  const cwd = createTempDir("jagentdesk-smoke-terminal-cwd-");
  const marker = `jagentdesk-packaged-terminal-smoke-${Date.now()}`;
...
async function smokePackagedDesktopApp({ appPath }) {
  ...
  const userData = createTempDir("jagentdesk-smoke-user-data-");
  const daemonHome = createTempDir("jagentdesk-smoke-daemon-home-");
  ...
  try {
    ...
    if (artifactDir) {
      ...
      await page.screenshot({ path: path.join(artifactDir, "renderer.png"), fullPage: true });
    }
...
if (require.main === module) {
  ...
  if (!appPath) {
    process.stderr.write("Usage: node smoke-packaged-desktop-app.js --app <JAgentDesk.app>\n");
    process.exit(2);
  }
