...
declare global {
  interface Window {
    __jagentdeskStartupPresentationTrace?: StartupPresentation[];
  }
}
...
async function observeStartupPresentation(page: Page): Promise<void> {
  await page.addInitScript(() => {
    ...
    window.__jagentdeskStartupPresentationTrace = trace;
...
async function getStartupPresentation(page: Page): Promise<StartupPresentation[]> {
  return page.evaluate(() => window.__jagentdeskStartupPresentationTrace?.slice() ?? []);
}
