 ...
 * status endpoint, PID from the jagentdesk.pid lock file, log path from the
 * E2E_JAGENTDESK_HOME directory. Call this in Node test code (not in the browser).
 */
export async function loadRealDaemonState(): Promise<RealDaemonState> {
  ...
  try {
    const raw = readFileSync(`${jagentdeskHome}/jagentdesk.pid`, "utf8");
...
export async function installDesktopRuntime(
  ...
  await page.addInitScript((cfg) => {
    ...
    (window as unknown as { jagentdeskDesktop: unknown }).jagentdeskDesktop = desktopBridge;
  }, config);
}
