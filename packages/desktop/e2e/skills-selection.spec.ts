...
test.describe("Choosing installed skills", () => {
  test("installs only the skills selected in Settings", async ({ page, startSkills }) => {
    ...
    await chooseCustomSkills(page, ["jagentdesk", "jagentdesk-loop"]);
    await saveSkillSelection(page);

    await expectSkillsInstalled(skills, ["jagentdesk", "jagentdesk-loop"]);
    await expectSelectedSkills(page, ["jagentdesk", "jagentdesk-loop"]);
  });
  ...
  test("all skills includes every available skill", async ({ page, startSkills }) => {
    ...
    await openSkillsIntegrations(page);
    await openSkillSelection(page);
    await chooseCustomSkills(page, ["jagentdesk"]);
    await saveSkillSelection(page);
  ...
  test("cancelling leaves the installed skills untouched", async ({ page, startSkills }) => {
    ...
    await openSkillSelection(page);
    await toggleSkill(page, "jagentdesk-loop");
    await cancelSkillSelection(page);

    await expectSkillsInstalled(skills, ["jagentdesk"]);
    await expectSelectedSkills(page, ["jagentdesk"]);
  });
...
test.describe("Removing an installed skill", () => {
  test("delete-only drift stays in Choose skills instead of Update", async ({
    ...
    await startSkills({
      installed: { mode: "custom", skills: ["jagentdesk"] },
      placeOnDisk: {
        skill: "jagentdesk-loop",
        target: "claude",
        files: { "SKILL.md": "# jagentdesk-loop\n", "notes/mine.md": "my notes" },
      },
    });
    await gotoAppShell(page);
    await openSkillsIntegrations(page);
  ...
  test("warns before deleting the skill folders", async ({ page, startSkills }) => {
    ...
    await openSkillSelection(page);
    await chooseCustomSkills(page, ["jagentdesk", "jagentdesk-advisor"]);
    await saveSkillSelection(page);

    await expectRemovalWarning(page, ["jagentdesk-loop"]);
    await expectSkillSelectionOpen(page);
    await expectSkillsInstalled(skills, skills.bundledSkills);
  ...
  test("confirming the warning removes the skill", async ({ page, startSkills }) => {
    ...
    await expectRemovalWarning(page, ["jagentdesk-loop"]);
    await expectSkillsInstalled(skills, ["jagentdesk", "jagentdesk-advisor"]);
    await expectSelectedSkills(page, ["jagentdesk", "jagentdesk-advisor"]);
  });

  test("warns for a skill installed in only one agent directory", async ({ page, startSkills }) => {
    const skills = await startSkills({
      installed: { mode: "all" },
      keepOnlyIn: { skill: "jagentdesk-loop", target: "claude" },
    ...
    await expectRemovalWarning(page, ["jagentdesk-loop"]);
    await expectSkillSelectionOpen(page);
    await expectSkillPresentIn(skills, "claude", "jagentdesk-loop");
    await expectSavedSkillSelection(skills, { mode: "all" });
  });
  ...
  test("warns for a skill on disk that the saved selection never mentioned", async ({
    ...
    // The committed preference excludes jagentdesk-loop, but the directory is back —
    ...
    await openSkillSelection(page);
    await toggleSkill(page, "jagentdesk-advisor");
    await saveSkillSelection(page);

    await expectRemovalWarning(page, ["jagentdesk-loop"]);
    await expectSkillSelectionOpen(page);
    await expectSkillFilePresent(skills, "claude", "jagentdesk-loop", "notes/mine.md");
    await expectSavedSkillSelection(skills, { mode: "custom", skills: ["jagentdesk"] });
  });
  ...
  test("warns about a directory that appears after the sheet was opened", async ({
    ...
    await skills.placeSkillOnDisk("jagentdesk-loop", "claude");
    await toggleSkill(page, "jagentdesk-advisor");
    await saveSkillSelection(page);
  ...
  test("keeps draft choices when fresher status arrives while the sheet is open", async ({
    ...
    await skills.commitSelectionExternally(
      { mode: "custom", skills: ["jagentdesk", "jagentdesk-loop"] },
      ["jagentdesk-advisor"],
    );
    await saveSkillSelection(page);
    await expectRemovalWarning(page, ["jagentdesk-loop"]);

    await expectSkillChoices(page, ["jagentdesk", "jagentdesk-advisor"]);
  });
