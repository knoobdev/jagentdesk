...
export async function runArchiveCommandWithDeps(
  ...
  if (!nameArg || nameArg.trim().length === 0) {
    const error: CommandError = {
      ...
      details: "Usage: jagentdesk worktree archive <name>",
    };
    throw error;
  }
  ...
  try {
    ...
    const listResponse = await client.getJAgentDeskWorktreeList({});
    ...
    if (!worktree) {
      const error: CommandError = {
        ...
        details: 'Use "jagentdesk worktree ls" to list available worktrees',
      };
      throw error;
    }

    // Archive the worktree. scope:"worktree" archives every active workspace on
    // the directory and then removes the directory (JAgentDesk-owned gated).
    const response = await client.archiveJAgentDeskWorktree({
