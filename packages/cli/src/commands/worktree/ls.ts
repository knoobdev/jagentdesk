...
export function resolveJAgentDeskHomePath(): string {
  return process.env.JAGENTDESK_HOME ?? join(homedir(), ".jagentdesk");
}

export function resolveJAgentDeskWorktreesDir(): string {
  return join(resolveJAgentDeskHomePath(), "worktrees");
}

function isAgentInManagedWorktree(agentCwd: string): boolean {
  const worktreesDir = resolveJAgentDeskWorktreesDir();
...
export async function runLsCommand(
  ...
  try {
    ...
    const response = await client.getJAgentDeskWorktreeList({});
