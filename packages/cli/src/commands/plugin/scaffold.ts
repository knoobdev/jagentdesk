...
const SDK_DECLARATIONS = `declare module "@jagentdesk/plugin/server" {
...
declare module "@jagentdesk/plugin" {
...
const ENTRY = `import type { PluginContext } from "@jagentdesk/plugin";
...
const CLIENT_SURFACE = `import type { PluginSurfaceProps } from "@jagentdesk/plugin";
...
export async function scaffoldPluginDirectory(
  ...
  const files = new Map<string, string>([
    ["jagentdesk-plugin.json", `${JSON.stringify({ id }, null, 2)}\n`],
    ["package.json", `${JSON.stringify(packageJson, null, 2)}\n`],
    ["tsconfig.json", `${JSON.stringify(TSCONFIG, null, 2)}\n`],
    ["jagentdesk-plugin.d.ts", SDK_DECLARATIONS],
    ["index.ts", ENTRY],
