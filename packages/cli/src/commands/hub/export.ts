...
export async function runHubExport(
  ...
  const directory = path.resolve(dependencies.cwd(), directoryInput ?? ".jagentdesk/triggers");
  ...
  addJsonOption(
    addHubResolutionHelp(
      parent
        ...
        .argument("[directory]", "Destination directory", ".jagentdesk/triggers")
        .option("--hub <origin>", "JAgentDesk Hub origin")
