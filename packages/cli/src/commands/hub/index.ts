    ...
    data: [
      {
        ...
        scopes: status.permissions.join(", "),
...
export function createHubCommand(
  ...
  const hub = new Command("hub").description("Manage this daemon's JAgentDesk Hub relationship");
...
function suggestedDisplayName(value: string): string {
  return value.trim().slice(0, 100) || "JAgentDesk daemon";
}
