...
describe("Hub device authorization", () => {
  it("opens activation URLs on Windows without a command shell", async () => {
    ...
    await browser.open("https://cloud.jagentdesk.test/activate?code=ABCD-EFGH-JKLMN");
    ...
    assert.deepEqual(launches, [
      {
        ...
        args: [
          ...
          "https://cloud.jagentdesk.test/activate?code=ABCD-EFGH-JKLMN",
        ],
      },
    ]);
  });
  ...
  it("opens the browser, follows Cloud cadence, and returns the approved enrollment token", async () => {
    ...
    const token = await authorization.approve("https://cloud.jagentdesk.test", "Studio Mac");

    assert.equal(token, "approved-enrollment-token-1234567890");
    assert.deepEqual(authorization.observed(), {
      starts: [{ hubUrl: "https://cloud.jagentdesk.test", displayName: "Studio Mac" }],
      polls: [
        {
          hubUrl: "https://cloud.jagentdesk.test",
      ...
      opened: ["https://cloud.jagentdesk.test/activate?code=ABCD-EFGH-JKLMN"],
      instructions: ["https://cloud.jagentdesk.test/activate ABCD-EFGH-JKLMN"],
    });
  });
  ...
  it("stops without an enrollment token when the browser denies the request", async () => {
    ...
    await assert.rejects(authorization.approve("https://cloud.jagentdesk.test", "Studio Mac"), {
  ...
  it("connects the real hub command with a browser-approved token", async () => {
    ...
    await createHubCommand({
      ...
      authorize: async (url, displayName) => {
        assert.equal(url, "https://cloud.jagentdesk.test");
        assert.equal(displayName, "Studio Mac");
        return "approved-enrollment-token-1234567890";
      },
      displayName: () => "Studio Mac",
    }).parseAsync(["node", "jagentdesk hub", "connect", "https://cloud.jagentdesk.test", "--json"], {
      from: "node",
    });

    assert.deepEqual(daemon.connections, [
      {
        url: "https://cloud.jagentdesk.test",
...
class FakeCloud implements CloudDeviceAuthorization {
  ...
  async start(hubUrl: string, displayName: string) {
    ...
    return {
      ...
      verificationUri: "https://cloud.jagentdesk.test/activate",
      verificationUriComplete: "https://cloud.jagentdesk.test/activate?code=ABCD-EFGH-JKLMN",
...
function hubStatus(state: string) {
  return {
    ...
    hubOrigin: state === "connected" ? "https://cloud.jagentdesk.test" : null,
