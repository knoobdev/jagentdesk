...
function daemonConnectionFailure(host: string, cause: unknown): CommandError {
  ...
  return {
    ...
    details: isSsh
      ...
      : [
          "Start a local daemon with: jagentdesk daemon start",
          "To use another daemon, pass --host <host:port> or set JAGENTDESK_HOST.",
        ].join("\n"),
  };
}
 ...
 * - `jagentdesk agent ls`    → active non-archived agents
 * - `jagentdesk agent ls -g` → global non-archived agents
 * - `jagentdesk agent ls -a` → active agents, including archived
 * - `jagentdesk agent ls -ag` → global agents, including archived
 */
