...
function buildWaitResult(args: {
  ...
  if (state.status === "timeout") {
    const timeoutMessage = timeoutLabel
      ? `Agent did not finish within ${timeoutLabel}. Run \`jagentdesk wait ${resolvedAgentId}\` again to keep waiting.`
      : `Agent wait timed out. Run \`jagentdesk wait ${resolvedAgentId}\` again to keep waiting.`;
    return {
...
export async function runWaitCommand(
  ...
  if (!agentIdArg || agentIdArg.trim().length === 0) {
    throw {
      ...
      details: "Usage: jagentdesk agent wait <id>",
    } satisfies CommandError;
  }
