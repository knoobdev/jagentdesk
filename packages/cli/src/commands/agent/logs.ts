...
import type { AgentTimelineItem } from "@jagentdesk/protocol/agent-types";
import type { AgentStreamMessage } from "@jagentdesk/protocol/messages";
import { curateAgentActivity } from "@jagentdesk/server";
...
export async function runLogsCommand(
  ...
  if (!id) {
    ...
    console.error("Usage: jagentdesk agent logs <id>");
    process.exit(1);
  }
  ...
  try {
    ...
    console.error("Start the daemon with: jagentdesk daemon start");
    process.exit(1);
  }
  ...
  try {
    ...
    if (!fetchResult) {
      ...
      console.error("Use `jagentdesk ls` to list available agents");
      await client.close();
      process.exit(1);
    }
