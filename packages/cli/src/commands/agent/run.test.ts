...
describe("runRunCommand option validation", () => {
  const originalWorkspaceId = process.env.JAGENTDESK_WORKSPACE_ID;

  beforeEach(() => {
    delete process.env.JAGENTDESK_WORKSPACE_ID;
  });
  ...
  afterEach(() => {
    if (originalWorkspaceId === undefined) {
      ...
      process.env.JAGENTDESK_WORKSPACE_ID = originalWorkspaceId;
    }
  });
