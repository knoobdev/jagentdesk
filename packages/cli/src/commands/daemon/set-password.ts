...
import { resolveLocalJAgentDeskHome } from "./local-daemon.js";
...
export async function setDaemonPasswordInConfig(
  ...
  const configPath = path.join(jagentdeskHome, CONFIG_FILENAME);
  ...
  savePersistedConfig(jagentdeskHome, nextConfig);

  return {
    action: "password_set",
    configPath,
    restartCommand: "jagentdesk daemon restart",
    message: `Password written to ${configPath}\nRestart the daemon for the change to take effect.\nRun: jagentdesk daemon restart`,
  };
}
