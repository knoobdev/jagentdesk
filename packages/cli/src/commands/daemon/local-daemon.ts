...
const DAEMON_PID_FILENAME = "jagentdesk.pid";
...
const defaultDaemonLaunchRuntime: DaemonLaunchRuntime = {
  ...
  resolveHome: resolveJAgentDeskHome,
...
function envWithHome(home?: string): NodeJS.ProcessEnv {
  ...
  return { ...process.env, JAGENTDESK_HOME: home };
}
...
function buildChildEnv(options: DaemonStartOptions): NodeJS.ProcessEnv {
  ...
  if (options.home) {
    childEnv.JAGENTDESK_HOME = options.home;
  }
  if (options.listen) {
    childEnv.JAGENTDESK_LISTEN = options.listen;
  } else if (options.port) {
    childEnv.JAGENTDESK_LISTEN = `127.0.0.1:${options.port}`;
  }
  if (options.hostnames) {
    childEnv.JAGENTDESK_HOSTNAMES = options.hostnames;
  }
  if (options.webUi === true) {
    childEnv.JAGENTDESK_WEB_UI_ENABLED = "true";
  }
  if (options.webUi === false) {
    childEnv.JAGENTDESK_WEB_UI_ENABLED = "false";
  }
  return childEnv;
}
...
function resolveServerRunnerFromDir(currentDir: string): string | null {
  ...
  try {
    ...
    if (packageJson.name !== "@jagentdesk/server") return null;
...
function resolveDaemonRunnerEntry(): string {
  const serverExportPath = require.resolve("@jagentdesk/server");
  ...
  throw new Error("Unable to resolve @jagentdesk/server package root for daemon runner");
}

function pidFilePath(jagentdeskHome: string): string {
  return path.join(jagentdeskHome, DAEMON_PID_FILENAME);
}
...
export function resolveLocalJAgentDeskHome(home?: string): string {
  return resolveJAgentDeskHome(envWithHome(home));
}
...
export function resolveLocalDaemonState(options: { home?: string } = {}): LocalDaemonState {
  const env: NodeJS.ProcessEnv = {
    ...
    JAGENTDESK_LISTEN: undefined,
    JAGENTDESK_HOSTNAMES: undefined,
    JAGENTDESK_ALLOWED_HOSTS: undefined,
  };
  const home = resolveJAgentDeskHome(env);
  ...
  return {
    home,
    listen,
    tailnetHost: env.JAGENTDESK_TAILNET_HOST?.trim() || null,
    logPath,
    pidPath,
    pidInfo,
    running,
...
export function tailDaemonLog(home?: string, lines = 30): string | null {
  const logPath = path.join(resolveLocalJAgentDeskHome(home), DAEMON_LOG_FILENAME);
...
export async function startLocalDaemonDetached(
  ...
  const jagentdeskHome = runtime.resolveHome(childEnv);
  const logPath = path.join(jagentdeskHome, DAEMON_LOG_FILENAME);
