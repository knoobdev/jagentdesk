...
import { getOrCreateServerId, findExecutable, execCommand } from "@jagentdesk/server";
...
function toStatusRows(status: DaemonStatus): StatusRow[] {
  const rows: StatusRow[] = [
    ...
    { key: "Tailnet", value: status.tailnet },
...
function describeDaemonAuthProbeFailure(host: string, failure: DaemonAuthProbeFailure): string {
  if (failure === "auth_required") {
    return `Daemon is reachable at ${host} but requires a password. Set JAGENTDESK_PASSWORD and retry.`;
  }
  return `Daemon is reachable at ${host} but the supplied password was rejected. Check JAGENTDESK_PASSWORD and retry.`;
}
...
export function selectTailnetStatus(input: { host: string | null; listen: string }): string {
  const host = input.host?.trim();
  if (!host) return "not configured";
  const tcpListen = resolveTcpHostFromListen(input.listen);
  if (!tcpListen) return host;
  const lastColon = tcpListen.lastIndexOf(":");
  const port = lastColon === -1 ? null : tcpListen.slice(lastColon + 1);
...
export async function runStatusCommand(
  ...
  const tailnet = selectTailnetStatus({ host: state.tailnetHost, listen: state.listen });
