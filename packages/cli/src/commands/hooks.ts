...
import { resolveHookActivity, type AgentHookActivityState } from "@jagentdesk/server/agent-hooks";

interface HookEnvironment {
  JAGENTDESK_TERMINAL_ID?: string;
  JAGENTDESK_ACTIVITY_TOKEN?: string;
  JAGENTDESK_TERMINAL_ACTIVITY_URL?: string;
}
...
function resolveTarget(env: HookEnvironment) {
  const terminalId = env.JAGENTDESK_TERMINAL_ID;
  const token = env.JAGENTDESK_ACTIVITY_TOKEN;
  const url = env.JAGENTDESK_TERMINAL_ACTIVITY_URL;
