...
function readPidSocketTarget(jagentdeskHome: string): string | null {
  const pidPath = path.join(jagentdeskHome, PID_FILENAME);
...
function resolveConfiguredIpcDaemonHost(
  ...
  const pidHost = normalizeDaemonHost(readPidSocketTarget(jagentdeskHome) ?? "");
  if (isIpcDaemonHost(pidHost)) {
    return pidHost;
  }

  const config = loadConfig(jagentdeskHome, { env });
...
export function resolveDefaultDaemonHosts(env: NodeJS.ProcessEnv = process.env): string[] {
  const jagentdeskHome = resolveJAgentDeskHome(env);
  const candidates: string[] = [];
  const configuredIpcHost = resolveConfiguredIpcDaemonHost(env, jagentdeskHome);
  if (configuredIpcHost) {
    candidates.push(configuredIpcHost);
  }
  const configuredTcpHost = resolveConfiguredTcpDaemonHost(env, jagentdeskHome);
...
export function resolveDaemonPassword(host: string): string | undefined {
  ...
  const fromEnv = process.env.JAGENTDESK_PASSWORD;
...
export async function connectToDaemon(options?: ConnectOptions): Promise<DaemonClient> {
  ...
  async function tryNext(index: number, lastError: unknown): Promise<DaemonClient> {
    if (index >= hosts.length) {
      ...
      throw new Error(`Unable to connect to JAgentDesk daemon via ${hosts.join(", ")}`);
    }
