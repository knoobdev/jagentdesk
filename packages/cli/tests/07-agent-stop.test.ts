...
try {
  // Test 1: stop --help shows options
  {
    ...
    const result = await $`npx jagentdesk stop --help`.nothrow();
  ...
  // Test 2: stop requires ID, --all, or --cwd
  {
    ...
    const result =
      await $`JAGENTDESK_HOST=localhost:${port} JAGENTDESK_HOME=${jagentdeskHome} npx jagentdesk stop`.nothrow();
  ...
  // Test 3: stop handles daemon not running
  {
    ...
    const result =
      await $`JAGENTDESK_HOST=localhost:${port} JAGENTDESK_HOME=${jagentdeskHome} npx jagentdesk stop abc123`.nothrow();
  ...
  // Test 4: stop --all flag is accepted
  {
    ...
    const result =
      await $`JAGENTDESK_HOST=localhost:${port} JAGENTDESK_HOME=${jagentdeskHome} npx jagentdesk stop --all`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 5: stop --cwd flag is accepted
  {
    ...
    const result =
      await $`JAGENTDESK_HOST=localhost:${port} JAGENTDESK_HOME=${jagentdeskHome} npx jagentdesk stop --cwd /tmp`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 6: stop with ID and --host flag is accepted
  {
    ...
    const result =
      await $`JAGENTDESK_HOST=localhost:${port} JAGENTDESK_HOME=${jagentdeskHome} npx jagentdesk stop abc123 --host localhost:${port}`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 7: jagentdesk --help shows stop command
  {
    console.log("Test 7: jagentdesk --help shows stop command");
    ...
    console.log("✓ jagentdesk --help shows stop command\n");
  }
  ...
  // Test 8: -q (quiet) flag is accepted with stop
  {
    ...
    const result =
      await $`JAGENTDESK_HOST=localhost:${port} JAGENTDESK_HOME=${jagentdeskHome} npx jagentdesk -q stop abc123`.nothrow();
    const output = result.stdout + result.stderr;
