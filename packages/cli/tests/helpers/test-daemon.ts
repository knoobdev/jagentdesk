 ...
 * Test Daemon Helper
 *
 * Provides utilities for launching real JAgentDesk daemons in E2E tests.
 * Each test gets an isolated daemon on an available local port with its own JAGENTDESK_HOME.
 *
...
const TEST_OUTPUT_CAPTURE_LIMIT = Number.parseInt(
  process.env.JAGENTDESK_TEST_OUTPUT_CAPTURE_BYTES ?? `${DEFAULT_OUTPUT_CAPTURE_LIMIT}`,
  10,
);
...
export async function createTempDirs(): Promise<{ jagentdeskHome: string; workDir: string }> {
  const jagentdeskHome = await mkdtemp(join(tmpdir(), "jagentdesk-e2e-home-"));
  const workDir = await mkdtemp(join(tmpdir(), "jagentdesk-e2e-work-"));

  // Create the agents directory that the daemon expects
  const agentsDir = join(jagentdeskHome, "agents");
...
/**
 * Wait for daemon to be ready by running `jagentdesk agent ls`
...
async function probeDaemonReady(port: number, env?: NodeJS.ProcessEnv): Promise<boolean> {
  try {
    const { exitCode } = await runJAgentDeskCli(
      {
        port,
 ...
 * with isolated JAGENTDESK_HOME and JAGENTDESK_LISTEN environment variables.
 */
export async function startTestDaemon(options?: {
  ...
  const { jagentdeskHome, workDir } =
    options?.jagentdeskHome && options?.workDir
      ? { jagentdeskHome: options.jagentdeskHome, workDir: options.workDir }
  ...
  const daemonProcess = spawn(
    ...
    [TSX_ENTRY, cliSrcPath, "daemon", "start", "--foreground"],
    {
      env: {
        ...
        JAGENTDESK_LISTEN: `${TEST_DAEMON_HOST}:${port}`,
...
/**
 * Run a jagentdesk CLI command against a test daemon
 *
 * This is a helper that sets the correct environment variables
 * to point at the test daemon.
 */
export async function runJAgentDeskCli(
  ...
  return new Promise((resolve, reject) => {
    const proc = spawn(process.execPath, [TSX_ENTRY, cliSrcPath, ...args], {
      env: {
        ...process.env,
        ...TEST_DAEMON_ENV_DEFAULTS,
        JAGENTDESK_HOST: `${TEST_DAEMON_HOST}:${ctx.port}`,
        JAGENTDESK_HOME: ctx.jagentdeskHome,
        ...options?.env,
      },
      cwd,
      stdio: ["ignore", "pipe", "pipe"],
    ...
    const timeoutId = setTimeout(() => {
      ...
      reject(new Error(`CLI command timed out after ${timeout}ms: jagentdesk ${args.join(" ")}`));
    }, timeout);
...
export async function createE2ETestContext(options?: {
  ...
  TestDaemonContext & {
    /** Run a jagentdesk CLI command against this daemon */
    jagentdesk: (
      args: string[],
...
> {
  ...
  const jagentdesk = (
    args: string[],
    opts?: { timeout?: number; cwd?: string; env?: NodeJS.ProcessEnv },
  ) => runJAgentDeskCli(ctx, args, opts);
