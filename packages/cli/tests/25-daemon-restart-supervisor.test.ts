...
const jagentdeskHome = await mkdtemp(join(tmpdir(), "jagentdesk-restart-supervisor-"));
...
try {
  ...
  const statusBeforeRestart = await readDaemonStatus(jagentdeskHome);
  ...
  const statusAfterRestart = await readDaemonStatus(jagentdeskHome);
  assert.strictEqual(
