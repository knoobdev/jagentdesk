...
/**
 * Regression: `jagentdesk daemon stop` must stop a reachable daemon even when the
...
const jagentdeskHome = await mkdtemp(join(tmpdir(), "jagentdesk-stop-stale-reachable-"));
...
try {
  ...
  console.log(
    "Test 2: `jagentdesk daemon stop` should stop reachable worker instead of saying not_running",
  );
  const stopResult =
