...
/**
 * Regression: `jagentdesk daemon stop` must stop supervised dev daemons
...
async function readPidLockState(jagentdeskHome: string): Promise<PidLockState> {
...
async function readDaemonStatus(jagentdeskHome: string): Promise<DaemonStatus> {
  const result =
    await $`JAGENTDESK_HOME=${jagentdeskHome} JAGENTDESK_LOCAL_SPEECH_AUTO_DOWNLOAD=${testEnv.JAGENTDESK_LOCAL_SPEECH_AUTO_DOWNLOAD} JAGENTDESK_DICTATION_ENABLED=${testEnv.JAGENTDESK_DICTATION_ENABLED} JAGENTDESK_VOICE_MODE_ENABLED=${testEnv.JAGENTDESK_VOICE_MODE_ENABLED} npx jagentdesk daemon status --home ${jagentdeskHome} --json`.nothrow();
...
async function readCapturedSupervisorLogs(jagentdeskHome: string, recentLogs: string): Promise<string> {
  const durableLogs = await readFile(join(jagentdeskHome, "daemon.log"), "utf8").catch(() => "");
  return `${recentLogs}\n${durableLogs}`;
}
...
const jagentdeskHome = await mkdtemp(join(tmpdir(), "jagentdesk-stop-supervisor-"));
...
try {
  console.log("Test 1: start supervisor-entrypoint in dev mode with isolated JAGENTDESK_HOME");
  ...
  await waitFor(
    async () => {
      const status = await readDaemonStatus(jagentdeskHome);
      return (
  ...
  const statusBeforeStop = await readDaemonStatus(jagentdeskHome);
  ...
  const pidLockBeforeStop = await readPidLockState(jagentdeskHome);
  ...
  console.log("Test 2: `jagentdesk daemon stop` should stop without respawn");
  const stopResult =
    await $`JAGENTDESK_HOME=${jagentdeskHome} JAGENTDESK_LOCAL_SPEECH_AUTO_DOWNLOAD=${testEnv.JAGENTDESK_LOCAL_SPEECH_AUTO_DOWNLOAD} JAGENTDESK_DICTATION_ENABLED=${testEnv.JAGENTDESK_DICTATION_ENABLED} JAGENTDESK_VOICE_MODE_ENABLED=${testEnv.JAGENTDESK_VOICE_MODE_ENABLED} npx jagentdesk daemon stop --home ${jagentdeskHome} --json`.nothrow();
  ...
  const pidAfterStop = await readPidLockState(jagentdeskHome);
  ...
  const statusAfterStop = await readDaemonStatus(jagentdeskHome);
  assert.strictEqual(
    statusAfterStop.localDaemon,
    "stopped",
    "daemon should remain stopped after stop command",
  );
  const capturedSupervisorLogs = await readCapturedSupervisorLogs(jagentdeskHome, recentSupervisorLogs);
  assert(
  ...
  await $`JAGENTDESK_HOME=${jagentdeskHome} JAGENTDESK_LOCAL_SPEECH_AUTO_DOWNLOAD=${testEnv.JAGENTDESK_LOCAL_SPEECH_AUTO_DOWNLOAD} JAGENTDESK_DICTATION_ENABLED=${testEnv.JAGENTDESK_DICTATION_ENABLED} JAGENTDESK_VOICE_MODE_ENABLED=${testEnv.JAGENTDESK_VOICE_MODE_ENABLED} npx jagentdesk daemon stop --home ${jagentdeskHome} --force`.nothrow();
