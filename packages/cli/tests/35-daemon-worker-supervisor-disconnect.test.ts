...
const jagentdeskHome = await mkdtemp(join(tmpdir(), "jagentdesk-worker-supervisor-disconnect-"));
...
try {
  console.log("Test 1: start supervised daemon with isolated JAGENTDESK_HOME");
  ...
  const statusBeforeKill = await readDaemonStatus(jagentdeskHome);
