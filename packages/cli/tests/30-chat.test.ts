...
try {
...

  {
    ...
    const posted = await ctx.jagentdesk(["chat", "post", "coord-room", "first message for @agent-1"], {
      env: { JAGENTDESK_AGENT_ID: "00000000-0000-4000-8000-000000000111" },
    });
    ...
    const read = await ctx.jagentdesk(["chat", "read", "coord-room", "--limit", "10"]);
    ...
    const readJson = await ctx.jagentdesk(["chat", "read", "coord-room", "--limit", "10", "--json"]);
    ...
    // always wakes it up.
    const waitPromise = ctx.jagentdesk(["chat", "wait", "coord-room", "--timeout", "30s"]);
    const waitSentinel = waitPromise.then(() => "settled" as const);
    let postedSecond = false;
    for (let attempt = 0; attempt < 60; attempt++) {
      const post = await ctx.jagentdesk(["chat", "post", "coord-room", "second message"]);
...

  {
    ...
    const duplicate = await ctx.jagentdesk(["chat", "create", "coord-room"]);
...

  {
    ...
    const deleted = await ctx.jagentdesk(["chat", "delete", "coord-room"]);
  ...
  await ctx.stop();
  await rm(ctx.jagentdeskHome, { recursive: true, force: true });
