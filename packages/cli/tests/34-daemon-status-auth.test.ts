...
const daemon = await startTestDaemon({
  env: { JAGENTDESK_PASSWORD: "shared-secret" },
});

try {
  {
    console.log("Test 1: status reports password requirement without marking daemon unreachable");
    const result = await runLocalJAgentDesk(["daemon", "status", "--json"], {
      JAGENTDESK_HOME: daemon.jagentdeskHome,
      JAGENTDESK_HOST: "",
      JAGENTDESK_PASSWORD: "",
    });
...

  {
    ...
    const result = await runLocalJAgentDesk(["daemon", "status", "--json"], {
      JAGENTDESK_HOME: daemon.jagentdeskHome,
      JAGENTDESK_HOST: "",
      JAGENTDESK_PASSWORD: "wrong-secret",
    });
...

  {
    ...
    const result = await runLocalJAgentDesk(["daemon", "status", "--json"], {
      JAGENTDESK_HOME: daemon.jagentdeskHome,
      JAGENTDESK_HOST: "",
      JAGENTDESK_PASSWORD: "shared-secret",
    });
