 ...
 * 2. Create agent with `jagentdesk run "say hello" --provider claude`
 * 3. List agents with `jagentdesk ls`
 * 4. Wait for agent with `jagentdesk wait <id>`
 * 5. Inspect agent with `jagentdesk inspect <id>`
 * 6. Stop agent with `jagentdesk stop <id>` and verify it remains inspectable
 * 7. Delete agent with `jagentdesk delete <id>`
...
async function setup(): Promise<void> {
  ...
  try {
    ...
    console.log(`JAGENTDESK_HOME: ${ctx.jagentdeskHome}`);
...
async function test_agent_ls_empty(): Promise<void> {
  ...
  const result = await ctx.jagentdesk(["ls", "--json"]);
...
async function test_agent_run_detached(): Promise<string> {
  ...
  const result = await ctx.jagentdesk(
    [
      "-q",
      "run",
      "-d",
...
async function test_agent_wait(agentId: string): Promise<void> {
  ...
  const result = await ctx.jagentdesk(["wait", "--timeout", "120s", agentId], { timeout: 130000 });
...
async function test_agent_inspect(agentId: string): Promise<void> {
  ...
  const result = await ctx.jagentdesk(["inspect", agentId]);
...
async function test_agent_logs(agentId: string): Promise<void> {
  ...
  const result = await ctx.jagentdesk(["logs", "--tail", "20", agentId]);
...
async function test_agent_stop(agentId: string): Promise<void> {
  ...
  const result = await ctx.jagentdesk(["stop", agentId]);
  ...
  const inspectResult = await ctx.jagentdesk(["inspect", agentId]);
...
async function test_agent_delete(agentId: string): Promise<void> {
  ...
  const result = await ctx.jagentdesk(["delete", agentId]);
