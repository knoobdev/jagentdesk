...
function runLocalJAgentDesk(args: string[], env: NodeJS.ProcessEnv): Promise<CommandResult> {
...
const jagentdeskHome = await mkdtemp(join(tmpdir(), "jagentdesk-windows-status-home-"));
...
try {
  const start = await runLocalJAgentDesk(["daemon", "restart", "--port", String(port)], env);
  assert.strictEqual(
    start.exitCode,
    0,
  ...
  const statusResult = await runLocalJAgentDesk(
    ["daemon", "status", "--home", jagentdeskHome, "--json"],
    env,
  );
  assert.strictEqual(
  ...
  await runLocalJAgentDesk(["daemon", "stop", "--home", jagentdeskHome, "--force"], env);
