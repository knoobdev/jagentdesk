...
/**
 * Test runner for JAgentDesk CLI E2E tests
 *
 ...
 * its own daemon (ephemeral port + tmp JAGENTDESK_HOME), so parallelism is safe.
 */
...
// packages live in the root. Prepend it so `npx jagentdesk` resolves locally.
...
const concurrencyEnv = process.env.JAGENTDESK_CLI_TEST_CONCURRENCY;
...
const shardTotal = parsePositiveInt(process.env.JAGENTDESK_CLI_TEST_SHARD_TOTAL, 1);
const shardIndexRaw = parsePositiveInt(process.env.JAGENTDESK_CLI_TEST_SHARD, 1);
if (shardIndexRaw < 1 || shardIndexRaw > shardTotal) {
  throw new Error(
    `JAGENTDESK_CLI_TEST_SHARD=${shardIndexRaw} out of range for SHARD_TOTAL=${shardTotal}`,
  );
}
...
async function writeJsonSummary({
  ...
  await writeFile(
    ...
    JSON.stringify(
      {
        suite: "cli-local",
        command: "npm run test:local --workspace=@jagentdesk/cli",
        counts: {
          passed,
          failed,
          skipped: 0,
...
console.log("🧪 JAgentDesk CLI E2E Test Runner\n");
console.log("=".repeat(50));
...
async function runSingleTest(testFile: string): Promise<TestOutcome> {
  ...
  const npmCache = await mkdtemp(join(tmpdir(), "jagentdesk-cli-test-npm-cache-"));
    ...
    return await new Promise<TestOutcome>((resolve) => {
      const proc = spawn("npx", ["tsx", testPath], {
        env: {
          ...
          JAGENTDESK_LOCAL_SPEECH_AUTO_DOWNLOAD: testEnvDefaults.JAGENTDESK_LOCAL_SPEECH_AUTO_DOWNLOAD,
          JAGENTDESK_DICTATION_ENABLED: testEnvDefaults.JAGENTDESK_DICTATION_ENABLED,
          JAGENTDESK_VOICE_MODE_ENABLED: testEnvDefaults.JAGENTDESK_VOICE_MODE_ENABLED,
        },
