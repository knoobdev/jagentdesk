...
OUT_DIR="/tmp/jagentdesk-sidebar-drag-cancellation-$(date +%s)"
...
export JAGENTDESK_MAESTRO_DAEMON_HEALTH_URL="${JAGENTDESK_MAESTRO_DAEMON_HEALTH_URL:-http://127.0.0.1:6767/api/health}"
...
if [ ! -f "$CLIENT_EXPORTS" ]; then
  echo "Missing client build artifacts." >&2
  echo "Run: npm run build:server" >&2
  exit 1
fi

if ! curl --fail --silent --show-error --max-time 3 "$JAGENTDESK_MAESTRO_DAEMON_HEALTH_URL" >/dev/null; then
  echo "JAgentDesk daemon is unavailable at $JAGENTDESK_MAESTRO_DAEMON_HEALTH_URL" >&2
  exit 1
fi

FIXTURE_ROOT="$(mktemp -d /tmp/jagentdesk-sidebar-drag-fixture-XXXXXX)"
export JAGENTDESK_MAESTRO_DRAG_A_NAME="000-jagentdesk-drag-a-$(basename "$FIXTURE_ROOT")"
export JAGENTDESK_MAESTRO_DRAG_B_NAME="001-jagentdesk-drag-b-$(basename "$FIXTURE_ROOT")"
export JAGENTDESK_MAESTRO_DRAG_Z_NAME="zzz-jagentdesk-drag-z-$(basename "$FIXTURE_ROOT")"

mkdir -p "$OUT_DIR/flows"

if [ -z "${JAGENTDESK_MAESTRO_IOS_UDID:-}" ]; then
  export JAGENTDESK_MAESTRO_IOS_UDID
  JAGENTDESK_MAESTRO_IOS_UDID="$({ xcrun simctl list devices booted -j || true; } | node -e '
    let input = "";
...
render_flow() {
  ...
  local target="$2"
  perl -0pe '
    s/^appId: sh\.jagentdesk$/appId: $ENV{JAGENTDESK_MAESTRO_APP_ID}/m;
    s/\$\{JAGENTDESK_MAESTRO_APP_ID\}/$ENV{JAGENTDESK_MAESTRO_APP_ID}/g;
    s/\$\{JAGENTDESK_MAESTRO_DIRECT_ENDPOINT\}/$ENV{JAGENTDESK_MAESTRO_DIRECT_ENDPOINT}/g;
    s/\$\{JAGENTDESK_MAESTRO_DRAG_A_NAME\}/$ENV{JAGENTDESK_MAESTRO_DRAG_A_NAME}/g;
    s/\$\{JAGENTDESK_MAESTRO_DRAG_B_NAME\}/$ENV{JAGENTDESK_MAESTRO_DRAG_B_NAME}/g;
    s/\$\{JAGENTDESK_MAESTRO_DRAG_Z_NAME\}/$ENV{JAGENTDESK_MAESTRO_DRAG_Z_NAME}/g;
  ' "$source" > "$target"
}

for project_name in \
  "$JAGENTDESK_MAESTRO_DRAG_A_NAME" \
  "$JAGENTDESK_MAESTRO_DRAG_B_NAME" \
  "$JAGENTDESK_MAESTRO_DRAG_Z_NAME"; do
  ...
  git -C "$project_path" config user.name "JAgentDesk Maestro"
  git -C "$project_path" config user.email "maestro@jagentdesk.local"
...
const client = new DaemonClient({
  url: process.env.JAGENTDESK_MAESTRO_DAEMON_WS_URL,
...
const { DaemonClient } = await import(moduleUrl);
const projectNames = [
  process.env.JAGENTDESK_MAESTRO_DRAG_A_NAME,
  process.env.JAGENTDESK_MAESTRO_DRAG_B_NAME,
  process.env.JAGENTDESK_MAESTRO_DRAG_Z_NAME,
];
const client = new DaemonClient({
  url: process.env.JAGENTDESK_MAESTRO_DAEMON_WS_URL,
...
echo "Running sidebar drag cancellation regression on $JAGENTDESK_MAESTRO_IOS_UDID"
echo "Artifacts: $OUT_DIR"
(cd "$OUT_DIR" && maestro test "$FLOW" --udid "$JAGENTDESK_MAESTRO_IOS_UDID")
