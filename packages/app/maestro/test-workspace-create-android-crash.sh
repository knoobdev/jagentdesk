...
#   JAGENTDESK_MAESTRO_APP_ID=app.jagentdesk.mobile.debug
#   JAGENTDESK_MAESTRO_DIRECT_ENDPOINT=127.0.0.1:6767
#   JAGENTDESK_MAESTRO_DAEMON_WS_URL=ws://127.0.0.1:6767/ws
#   JAGENTDESK_MAESTRO_PROJECT_PATH=/path/to/git/repo
set -euo pipefail
...
OUT_DIR="/tmp/jagentdesk-workspace-create-android-$(date +%s)"
...
render_flow() {
  ...
  perl -0pe '
    ...
    s/\$\{JAGENTDESK_MAESTRO_PROJECT_NAME\}/$ENV{JAGENTDESK_MAESTRO_PROJECT_NAME}/g;
  ' "$source" > "$target"
}
...
if [ -z "${JAGENTDESK_MAESTRO_PROJECT_PATH:-}" ]; then
  PROJECT_PARENT="$(mktemp -d /tmp/jagentdesk-maestro-project-XXXXXX)"
  PROJECT_BASENAME="aaa-workspace-create-android-$(basename "$PROJECT_PARENT")"
  export JAGENTDESK_MAESTRO_PROJECT_PATH="$PROJECT_PARENT/$PROJECT_BASENAME"
  mkdir -p "$JAGENTDESK_MAESTRO_PROJECT_PATH"
  git -C "$JAGENTDESK_MAESTRO_PROJECT_PATH" init >/dev/null
  git -C "$JAGENTDESK_MAESTRO_PROJECT_PATH" checkout -b main >/dev/null 2>&1 || true
  git -C "$JAGENTDESK_MAESTRO_PROJECT_PATH" config user.name "JAgentDesk Maestro"
  git -C "$JAGENTDESK_MAESTRO_PROJECT_PATH" config user.email "maestro@jagentdesk.local"
  printf "# Workspace create Android repro\n" > "$JAGENTDESK_MAESTRO_PROJECT_PATH/README.md"
  git -C "$JAGENTDESK_MAESTRO_PROJECT_PATH" add README.md
  git -C "$JAGENTDESK_MAESTRO_PROJECT_PATH" commit -m "Initial commit" >/dev/null
else
  PROJECT_PARENT=""
fi

export JAGENTDESK_MAESTRO_PROJECT_NAME="${JAGENTDESK_MAESTRO_PROJECT_NAME:-$(basename "$JAGENTDESK_MAESTRO_PROJECT_PATH")}"

echo "=== Workspace Create Android Crash Harness ==="
echo "Output dir: $OUT_DIR"
echo "App id: $JAGENTDESK_MAESTRO_APP_ID"
echo "Android direct endpoint: $JAGENTDESK_MAESTRO_DIRECT_ENDPOINT"
echo "Daemon websocket: $JAGENTDESK_MAESTRO_DAEMON_WS_URL"
echo "Project: $JAGENTDESK_MAESTRO_PROJECT_PATH"
echo "Project name: $JAGENTDESK_MAESTRO_PROJECT_NAME"
...
const projectPath = process.env.JAGENTDESK_MAESTRO_PROJECT_PATH;
const daemonUrl = process.env.JAGENTDESK_MAESTRO_DAEMON_WS_URL;
