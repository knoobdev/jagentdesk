...
import { CheckoutPrStatusSchema } from "@jagentdesk/protocol/messages";
...
describe("git-actions-policy", () => {
  ...
  it("keeps push available for a no-upstream JAgentDesk worktree with local commits", () => {
    const actions = buildGitActions(
      createInput({
        hasRemote: true,
  ...
  it("offers archive workspace for Git checkouts and worktrees", () => {
    ...
    const worktree = buildGitActions(
      createInput({ hasUncommittedChanges: true, isJAgentDeskOwnedWorktree: true }),
    );
  ...
  it("still promotes archive as primary for an idle JAgentDesk-owned worktree", () => {
    const actions = buildGitActions(createInput({ isJAgentDeskOwnedWorktree: true }));
