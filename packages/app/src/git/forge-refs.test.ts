...
describe("parseForgeRef", () => {
  it.each([
    [
      "GitHub pull request",
      "git@github.com:jagentdesk/jagentdesk.git",
      "https://github.com/jagentdesk/jagentdesk/pull/994/files?diff=split#discussion_r123",
      { kind: "change_request", number: 994 },
    ],
    [
      "GitHub issue",
      "https://github.com/jagentdesk/jagentdesk.git",
      "https://github.com/jagentdesk/jagentdesk/issues/456",
  ...
  it("canonicalizes a cloud SSH alias to its web host", () => {
    expect(
      parseForgeRef(
        "https://github.com/jagentdesk/jagentdesk/pull/994",
        "ssh://git@ssh.github.com/jagentdesk/jagentdesk.git",
      ),
  ...
  it("does not apply another forge's route grammar to a known cloud host", () => {
    expect(
      parseForgeRef(
        "https://github.com/jagentdesk/jagentdesk/pulls/31",
        "git@github.com:jagentdesk/jagentdesk.git",
      ),
    ).toBeNull();
    expect(
...
describe("extractForgeRefs", () => {
  ...
  it("returns no references without text or a valid remote", () => {
    expect(extractForgeRefs("", "git@github.com:jagentdesk/jagentdesk.git")).toEqual([]);
    expect(extractForgeRefs("https://github.com/jagentdesk/jagentdesk/pull/1", null)).toEqual([]);
  });
});
