...
import type { CheckoutPrMergeMethod } from "@jagentdesk/protocol/messages";
...
function deriveGitActionsState(args: DeriveGitActionsStateArgs): DerivedGitActionsState {
  ...
  const isJAgentDeskOwnedWorktree = gitStatus?.isJAgentDeskOwnedWorktree ?? false;
...
export function useGitActions({ serverId, cwd, icons }: UseGitActionsInput): UseGitActionsResult {
  ...
  const shipDefaultStorageKey = useMemo(() => {
    ...
    return `@jagentdesk:changes-ship-default:${gitStatus.repoRoot}`;
  }, [gitStatus?.repoRoot]);
...
function translateGitActionUnavailableMessage(
  ...
  const keyByMessage: Record<string, string> = {
    ...
    "Archive isn't available here because this workspace was not created as a JAgentDesk worktree":
