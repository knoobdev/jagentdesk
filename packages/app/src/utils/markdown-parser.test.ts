...
describe("createMarkdownParser", () => {
  ...
  it("linkifies bare URLs only when asked", () => {
    expect(createMarkdownParser({ linkify: true }).render("see https://jagentdesk.local now")).toContain(
      'href="https://jagentdesk.local"',
    );
    expect(
      createMarkdownParser({ linkify: false }).render("see https://jagentdesk.local now"),
    ).not.toContain("href");
  });
});
