...
const CLIENT_ID_STORAGE_KEY = "@jagentdesk:client-id-v1";
// COMPAT(predecessorClientId): read once from the old namespace so an existing
// installation keeps its device identity; all new writes use the JAgentDesk key.
const LEGACY_CLIENT_ID_STORAGE_KEY = "@jagentdesk:client-id-v1";
      ...
      inFlight = (async () => {
        const stored =
          ...
          (storageKey === CLIENT_ID_STORAGE_KEY
            ? await deps.storage.getItem(LEGACY_CLIENT_ID_STORAGE_KEY)
            : null);
