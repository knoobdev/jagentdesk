...
const JAGENTDESK_WORKTREE_PATH_PATTERN = /(^|\/)\.jagentdesk\/worktrees(\/|$)/;
...
export function collectAgentWorkingDirectorySuggestions(
  ...
  for (const source of sources) {
    ...
    if (isJAgentDeskOwnedWorktreePath(cwd)) {
      continue;
    }
...
function isJAgentDeskOwnedWorktreePath(cwd: string): boolean {
  return JAGENTDESK_WORKTREE_PATH_PATTERN.test(cwd.replace(/\\/g, "/"));
}
