...
import type { WorkspaceScriptPayload } from "@jagentdesk/protocol/messages";
...
const runningService: WorkspaceScriptPayload = {
  scriptName: "web",
  type: "service",
  hostname: "web--feature--jagentdesk.localhost",
  port: 3000,
  localProxyUrl: "http://web--feature--jagentdesk.localhost:6767",
  publicProxyUrl: null,
  proxyUrl: "http://web--feature--jagentdesk.localhost:6767",
  lifecycle: "running",
  health: "healthy",
  exitCode: null,
  terminalId: null,
...
describe("resolveWorkspaceScriptLink", () => {
  it("defaults to the memorable JAgentDesk URL locally and keeps direct as a fallback", () => {
    expect(
      resolveLink({ type: "directTcp", endpoint: "localhost:6767", display: "localhost:6767" }),
    ).toEqual({
      primary: {
        kind: "jagentdesk",
        label: "web--feature--jagentdesk.localhost:6767",
        url: "http://web--feature--jagentdesk.localhost:6767",
      },
      targets: [
        {
          kind: "jagentdesk",
  ...
  it("defaults to an explicitly configured reverse proxy", () => {
    const publicUrl = "https://web--feature--jagentdesk.services.example.com";
    expect(
      resolveLink(
        { type: "directSocket", endpoint: "/tmp/jagentdesk.sock", display: "socket" },
        { ...runningService, publicProxyUrl: publicUrl, proxyUrl: publicUrl },
      ),
    ).toEqual({
      primary: {
        kind: "public",
        label: "web--feature--jagentdesk.services.example.com",
        url: publicUrl,
      },
      targets: [
        {
  ...
  it("offers the reverse proxy and direct route over a direct network connection", () => {
    ...
    expect(
      ...
      { kind: "public", label: "web--feature--jagentdesk.services.example.com", url: publicUrl },
      {
        kind: "jagentdesk",
        label: "web--feature--jagentdesk.localhost:6767",
        url: "http://web--feature--jagentdesk.localhost:6767",
  ...
  it("keeps service routes available independently of a tailnet connection", () => {
    const tailnet: ActiveConnection = {
      type: "tailnet",
      endpoint: "tailnet.example.ts.net:6767",
      display: "tailnet.example.ts.net:6767",
    };
    expect(resolveLink(tailnet)).toEqual({
      primary: {
        kind: "jagentdesk",
        label: "web--feature--jagentdesk.localhost:6767",
        url: "http://web--feature--jagentdesk.localhost:6767",
    ...
    const publicUrl = "https://web--feature--jagentdesk.services.example.com";
    expect(
      resolveLink(tailnet, { ...runningService, publicProxyUrl: publicUrl, proxyUrl: publicUrl }),
    ).toEqual({
      primary: {
        kind: "public",
        label: "web--feature--jagentdesk.services.example.com",
  ...
  it("classifies proxyUrl from older daemons", () => {
    ...
    expect(
      resolveLink(
        { type: "tailnet", endpoint: "tailnet.example.ts.net:6767", display: "tailnet" },
