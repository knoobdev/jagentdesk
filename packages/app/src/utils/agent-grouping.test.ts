...
describe("deriveProjectDisplayName", () => {
  ...
  it("falls back to projectName for local keys", () => {
    expect(
      deriveProjectDisplayName({
        projectKey: "/Users/me/dev/jagentdesk",
        projectName: "jagentdesk",
      }),
    ).toBe("jagentdesk");
  });
...
describe("groupAgents", () => {
  it("groups active agents by remote URL when available", () => {
    const agents = [
      makeAgent({ id: "a1", cwd: "/Users/me/dev/jagentdesk" }),
      makeAgent({ id: "a2", cwd: "/Users/me/dev/jagentdesk-fix/worktree" }),
    ];

    const { activeGroups } = groupAgents(agents, {
      getRemoteUrl: () => "git@github.com:acme/example.git",
    });
