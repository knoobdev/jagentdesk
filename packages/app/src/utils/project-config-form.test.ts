...
import { JAgentDeskConfigRawSchema } from "@jagentdesk/protocol/jagentdesk-config-schema";
import type { JAgentDeskConfigRaw } from "@jagentdesk/protocol/messages";
...
describe("applyDraftToConfig", () => {
  it("preserves the original string kind when editing an existing setup field", () => {
    const base: JAgentDeskConfigRaw = { worktree: { setup: "npm install" } };
  ...
  it("preserves the original array kind when editing an existing teardown field", () => {
    const base: JAgentDeskConfigRaw = {
  ...
  it("preserves unknown top-level, worktree, and script entry fields on round-trip", () => {
    const base = JAgentDeskConfigRawSchema.parse({
      worktree: {
        setup: "npm install",
