...
describe("resolveNewAgentWorkingDir", () => {
  ...
  it("falls back to repo root when checkout metadata is unavailable", () => {
    expect(resolveNewAgentWorkingDir("/repo/.jagentdesk/worktrees/feature", null)).toBe("/repo");
  });
  ...
  it("returns the main repo root for jagentdesk-owned worktrees", () => {
    const checkout = {
      ...
      worktreeRoot: "/repo/.jagentdesk/worktrees/feature",
      mainRepoRoot: "/repo/main",
    } as unknown as CheckoutStatusPayload;

    expect(resolveNewAgentWorkingDir("/repo/.jagentdesk/worktrees/feature", checkout)).toBe(
      "/repo/main",
    );
  });
});
