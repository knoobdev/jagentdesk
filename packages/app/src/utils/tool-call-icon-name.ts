import type { ToolCallDetail, ToolCallIconName } from "@jagentdesk/protocol/agent-types";
import { isJAgentDeskToolName } from "@jagentdesk/protocol/tool-name-normalization";

export type ToolCallIcon = ToolCallIconName | "jagentdesk";
...
export function resolveToolCallIconName(toolName: string, detail?: ToolCallDetail): ToolCallIcon {
  ...
  if (isJAgentDeskToolName(lowerName)) {
    return "jagentdesk";
  }
  if (lowerName === "task") {
    return "bot";
