...
import type { TailnetConnectionOffer } from "@jagentdesk/protocol/connection-offer";
import type { SessionOutboundMessage } from "@jagentdesk/protocol/messages";
...
afterEach(() => {
  vi.useRealTimers();
  delete (globalThis as Record<string, unknown>).__JAGENTDESK_INITIAL_DAEMON_CONNECTION__;
...
function makeHost(input?: Partial<HostProfile>): HostProfile {
  ...
  const tailnet: HostConnection = {
    id: "tailnet:tailnet.localhost:8082:443",
    type: "tailnet",
    tailnetAddress: "tailnet.localhost:8082:443",
  ...
  return {
    ...
    connections: input?.connections ?? [direct, tailnet],
...
function makeOffer(input?: Partial<TailnetConnectionOffer>): TailnetConnectionOffer {
  return {
    v: 3,
    serverId: input?.serverId ?? "srv_offer",
    daemonPublicKeyB64: input?.daemonPublicKeyB64 ?? "pk_test_offer",
    tailnetAddress: input?.tailnetAddress ?? "tailnet.example.ts.net:6767",
...
function encodeOfferUrl(payload: unknown): string {
  const encoded = Buffer.from(JSON.stringify(payload), "utf8")
    ...
    .replace(/\//g, "_")
    .replace(/=+$/g, "");
  return `jagentdesk://app/#offer=${encoded}`;
}
...
describe("HostRuntimeController", () => {
  it("keeps only the active pairing request visible", () => {
    const controller = new HostRuntimeController({
      host: makeHost(),
      deps: makeDeps({ "direct:lan:6767": 5, "tailnet:tailnet.localhost:8082:443": 5 }, []),
    });
    controller.setPendingPairingRequest({
      requestId: "pair-request-a",
      pairingCode: "123456",
    ...
    expect(controller.getSnapshot().pendingPairingRequests).toHaveLength(1);
    expect(controller.getSnapshot().pendingPairingRequests[0]?.requestId).toBe("pair-request-b");
    expect(controller.getSnapshot().pendingPairingRequest?.requestId).toBe("pair-request-b");
  });

  it("replaces the active tailnet client when re-pairing changes the daemon public key", async () => {
    const oldTailnet: HostConnection = {
      id: "tailnet:wss:tailnet.localhost:8082:443",
      type: "tailnet",
      tailnetAddress: "tailnet.localhost:8082:443",
      useTls: true,
      daemonPublicKeyB64: "pk_old",
    };
    const newTailnet: HostConnection = {
      ...oldTailnet,
    ...
    const controller = new HostRuntimeController({
      host: makeHost({
        connections: [oldTailnet],
        preferredConnectionId: oldTailnet.id,
      }),
      deps: {
    ...
    await controller.activateConnection({ connectionId: oldTailnet.id });
    expect(controller.getSnapshot().client).toBe(createdClients[0]?.client);

    await controller.updateHost(
      makeHost({
        connections: [newTailnet],
        preferredConnectionId: newTailnet.id,
      }),
    );

    expect(createdClients.map((entry) => entry.connection)).toEqual([oldTailnet, newTailnet]);
  ...
  it("adopts the first successful probe on startup", async () => {
    ...
    const latencies: Record<string, number | Error> = {
      "direct:lan:6767": 82,
      "tailnet:tailnet.localhost:8082:443": 18,
    };
    const controller = new HostRuntimeController({
      host,
    ...
    const controller = new HostRuntimeController({
      ...
      deps: {
        ...
        connectToDaemon: async ({ host: hostProfile, connection }) => {
          ...
          if (connection.id === "tailnet:tailnet.localhost:8082:443") {
  ...
  it("ranks the live connection by its heartbeat RTT without pinging it again", async () => {
    ...
    const latencies: Record<string, number | Error> = {
      "direct:lan:6767": 12,
      "tailnet:tailnet.localhost:8082:443": 65,
    };
    const controller = new HostRuntimeController({
      host,
      deps: {
  ...
  it("keeps the live connection when one probe cycle looks slow", async () => {
    ...
    const latencies: Record<string, number | Error> = {
      "direct:lan:6767": 15,
      "tailnet:tailnet.localhost:8082:443": 55,
    };
    const controller = new HostRuntimeController({
      host,
      deps: makeDeps(latencies, clients),
    ...
    latencies["tailnet:tailnet.localhost:8082:443"] = 42;
  ...
  it("backs off inactive connection probes while a host is online", async () => {
    ...
    const latencies: Record<string, number | Error> = {
      "direct:lan:6767": 10,
      "tailnet:tailnet.localhost:8082:443": 50,
    };
    const controller = new HostRuntimeController({
      host,
      deps: makeDeps(latencies, clients),
    ...
    const initialTailnetProbe = controller
      .getSnapshot()
      .probeByConnectionId.get("tailnet:tailnet.localhost:8082:443");

    latencies["direct:lan:6767"] = 12;
    latencies["tailnet:tailnet.localhost:8082:443"] = 25;
    ...
    expect(snapshot.probeByConnectionId.get("tailnet:tailnet.localhost:8082:443")).toEqual(
      initialTailnetProbe,
    );
  });
  ...
  it("switches only after the faster alternative wins consecutive probes", async () => {
    ...
    const latencies: Record<string, number | Error> = {
      "direct:lan:6767": 15,
      "tailnet:tailnet.localhost:8082:443": 60,
    };
    const controller = new HostRuntimeController({
      host,
      deps: makeDeps(latencies, clients),
    ...
    latencies["tailnet:tailnet.localhost:8082:443"] = 30;
    ...
    let switched =
      controller.getSnapshot().activeConnectionId === "tailnet:tailnet.localhost:8082:443";
    for (let index = 0; index < 6 && !switched; index += 1) {
      await vi.advanceTimersByTimeAsync(120_000);
      await controller.runProbeCycleNow();
      switched = controller.getSnapshot().activeConnectionId === "tailnet:tailnet.localhost:8082:443";
    }
  ...
  it("does not switch on a transient latency spike", async () => {
    ...
    const latencies: Record<string, number | Error> = {
      "direct:lan:6767": 15,
      "tailnet:tailnet.localhost:8082:443": 80,
    };
    const controller = new HostRuntimeController({
      host,
      deps: makeDeps(latencies, clients),
    ...
    latencies["tailnet:tailnet.localhost:8082:443"] = 20;
    ...
    latencies["tailnet:tailnet.localhost:8082:443"] = 90;
  ...
  it("ignores stale switch failures after a newer connection is already online", async () => {
    ...
    const switchTailnet = controller.activateConnection({
      connectionId: "tailnet:tailnet.localhost:8082:443",
    });
    await waitUntil(() => {
      const snapshot = controller.getSnapshot();
      return (
        snapshot.activeConnectionId === "tailnet:tailnet.localhost:8082:443" &&
        snapshot.connectionStatus === "online"
      );
    });

    firstConnectGate.resolve();
    await Promise.allSettled([switchDirect, switchTailnet]);

    const snapshot = controller.getSnapshot();
    expect(snapshot.activeConnectionId).toBe("tailnet:tailnet.localhost:8082:443");
    expect(snapshot.connectionStatus).toBe("online");
...
describe("HostRuntimeStore", () => {
  it("restores the display replica before declaring the host registry loaded", async () => {
    ...
    await storage.setItem("@jagentdesk:daemon-registry", JSON.stringify([host]));
    await storage.setItem("@jagentdesk:e2e", "1");
  ...
  it("records a chosen host color and writes it through to storage", async () => {
    ...
    const persisted = await storage.getItem("@jagentdesk:daemon-registry");
  ...
  it("serializes overlapping host appearance writes", async () => {
    ...
    const persistedHosts = JSON.parse(
      (await storage.getItem("@jagentdesk:daemon-registry")) ?? "[]",
    );
      ...
      makeFetchAgentsPayload({
        entries: [
          makeFetchAgentsEntry({
            ...
            cwd: "/workspaces/jagentdesk-pr67-review",
  ...
  it("upsertDirectConnection stores SSL and password settings", async () => {
    ...
    await store.upsertDirectConnection({
      ...
      endpoint: "example.jagentdesk.test:7443",
      useTls: true,
    ...
    expect(host?.connections).toEqual([
      {
        id: "direct:example.jagentdesk.test:7443",
        type: "directTcp",
        endpoint: "example.jagentdesk.test:7443",
        useTls: true,
        password: "shared-secret",
  ...
  it("uses the advertised hostname when adding a tailnet host from a pairing offer", async () => {
    const store = new HostRuntimeStore({
      deps: {
        createClient: () => new FakeDaemonClient() as unknown as DaemonClient,
        connectToDaemon: async ({ host }) => ({
  ...
  it("stores tailnet TLS from a pairing offer", async () => {
    const store = new HostRuntimeStore({
      deps: {
        createClient: () => new FakeDaemonClient() as unknown as DaemonClient,
        connectToDaemon: async ({ host }) => ({
    ...
    await store.upsertConnectionFromOffer(
      makeOffer({
        tailnetAddress: "tailnet.example.com:443",
        useTls: true,
      }),
      "tls tailnet",
    );
    ...
    expect(pairedHost?.connections).toEqual([
      {
        id: "tailnet:wss:tailnet.example.com:443",
        type: "tailnet",
        tailnetAddress: "tailnet.example.com:443",
        useTls: true,
  ...
  it("rejects outdated pairing offers", async () => {
    const store = new HostRuntimeStore({
      deps: {
        createClient: () => new FakeDaemonClient() as unknown as DaemonClient,
        connectToDaemon: async ({ host }) => ({
    ...
    await expect(store.upsertConnectionFromOfferUrl(oldPairingUrl, "old device")).rejects.toThrow(
      "Invalid input",
    );
  ...
  it("preserves the existing host label when re-pairing an existing tailnet host", async () => {
    const store = new HostRuntimeStore({
      deps: {
        createClient: () => new FakeDaemonClient() as unknown as DaemonClient,
        connectToDaemon: async ({ host }) => ({
...
describe("readInitialDaemonConnectionHint", () => {
  ...
  it("parses a valid listen-only hint", () => {
    (globalThis as Record<string, unknown>).__JAGENTDESK_INITIAL_DAEMON_CONNECTION__ = {
  ...
  it("preserves useTls when explicitly true", () => {
    (globalThis as Record<string, unknown>).__JAGENTDESK_INITIAL_DAEMON_CONNECTION__ = {
      listen: "jagentdesk.example.com:443",
      useTls: true,
    };
    expect(readInitialDaemonConnectionHint({ isWebRuntime: true })).toEqual({
      listen: "jagentdesk.example.com:443",
...
describe("HostRuntimeStore initial connection hint bootstrap", () => {
  it("does not bootstrap an explicit hint before the user selects a connection mode", async () => {
