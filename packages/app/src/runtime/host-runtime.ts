...
import { ConnectionOfferSchema, type ConnectionOffer } from "@jagentdesk/protocol/connection-offer";
...
import { BROWSER_AUTOMATION_COMMAND_NAMES } from "@jagentdesk/protocol/browser-automation/rpc-schemas";
...
export type ActiveConnection =
  ...
  | { type: "tailnet"; endpoint: string; display: string };
...
function toActiveConnection(connection: HostConnection): ActiveConnection {
  ...
  return {
    type: "directTcp",
    endpoint: connection.tailnetAddress,
    display: connection.tailnetAddress,
  };
}
...
function buildConnectionCandidates(
  ...
  const connections =
    connectionMode === "tailscale"
      ? host.connections.filter((connection) => connection.type === "tailnet")
      : host.connections;
  ...
  return {
    createClient: ({ connection, clientId, runtimeGeneration }) => {
      ...
      return new DaemonClient({
        ...
        // Runtime reconnects use the already-held device key only. They must
        // not create a fresh pending Pair a device request; that is reserved
        // for the explicit mobile pairing flow that supplies a code provider.
    ...
    mountClientHandlers: ({ client, host }) => {
      ...
      const unmountPairingRequest = client.on("pairing.device.requested", (message) => {
        getHostRuntimeStore().setPendingPairingRequest(host.serverId, {
          ...message.payload,
          status: "pending",
        });
      });
      const unmountPairingCompleted = client.on("pairing.device.completed", (message) => {
        getHostRuntimeStore().completePendingPairingRequest(host.serverId, message.payload);
      });
      const unmountPairingCancelled = client.on("pairing.device.cancelled", (message) => {
        getHostRuntimeStore().cancelPendingPairingRequest(host.serverId, message.payload.requestId);
      });
...
export class HostRuntimeController {
  ...
  setPendingPairingRequest(request: PendingPairingRequest): void {
    this.updateSnapshot({
      pendingPairingRequests: [request],
      pendingPairingRequest: request,
    });
  }

  clearPendingPairingRequest(): void {
    this.updateSnapshot({ pendingPairingRequests: [], pendingPairingRequest: null });
  }

  completePendingPairingRequest(completion: {
    requestId: string;
    deviceId: string;
    deviceName?: string;
    connectionId?: string;
    ...
    const current = this.snapshot.pendingPairingRequests.find(
      (request) => request.requestId === completion.requestId,
    );
    if (!current || current.requestId !== completion.requestId) {
      return;
    }
    const completed = {
      ...current,
    ...
    this.updateSnapshot({
      pendingPairingRequests: this.snapshot.pendingPairingRequests.map((request) =>
        request.requestId === completion.requestId ? completed : request,
      ),
      pendingPairingRequest: completed,
    });
  }

  cancelPendingPairingRequest(requestId: string): void {
    const requests = this.snapshot.pendingPairingRequests.filter(
      (request) => request.requestId !== requestId,
    );
    this.updateSnapshot({
      pendingPairingRequests: requests,
      pendingPairingRequest: requests.at(-1) ?? null,
    });
  }
  ...
  setConnectionMode(value: ConnectionMode | null): void {
    if (this.connectionMode === value) {
      return;
    }
  ...
  private async runProbeCycle(): Promise<void> {
    ...
    const candidateConnections = buildConnectionCandidates(this.host, this.connectionMode).map(
      ({ connection }) => connection,
    );
    const candidateConnectionIds = new Set(candidateConnections.map((connection) => connection.id));

    if (
      this.snapshot.activeConnectionId !== null &&
      !candidateConnectionIds.has(this.snapshot.activeConnectionId)
    ) {
      this.switchRequestVersion += 1;
    ...
    const connectionsToProbe = candidateConnections.filter((connection) => {
    ...
    const finalizeProbeCycle = async (): Promise<void> => {
      ...
      if (!currentActiveConnectionId || !candidateConnectionIds.has(currentActiveConnectionId)) {
        const nextConnectionId = selectBestConnection({
          candidates: buildConnectionCandidates(this.host, this.connectionMode),
          probeByConnectionId,
        });
        if (nextConnectionId) {
          await this.switchToConnection({
...
const REGISTRY_STORAGE_KEY = "@jagentdesk:daemon-registry";
const LEGACY_REGISTRY_STORAGE_KEY = "@jagentdesk:daemon-registry";
const LOCALHOST_FALLBACK_ENDPOINT = `localhost:${DEFAULT_JAGENTDESK_DAEMON_PORT}`;
const DEFAULT_LOCALHOST_BOOTSTRAP_TIMEOUT_MS = 2500;
const E2E_STORAGE_KEY = "@jagentdesk:e2e";
const INITIAL_DAEMON_CONNECTION_HINT_GLOBAL_KEY = "__JAGENTDESK_INITIAL_DAEMON_CONNECTION__";
...
export class HostRuntimeStore {
  ...
  private async runBoot(): Promise<void> {
    this.connectionMode = await getConnectionMode();
    this.connectionModeUnsubscribe?.();
    ...
    // A native device must not touch localhost before the user explicitly
    // chooses Local. `null` is the first-run login gate and `tailscale` must
    // use only tailnet connections; neither state may silently fall back to
    // the legacy localhost bootstrap.
    if (shouldUseDesktopDaemon() || this.connectionMode !== "local") {
      return;
    }
  ...
  private async loadFromStorage(): Promise<void> {
    ...
    try {
      const canonicalStored = await this.storage.getItem(REGISTRY_STORAGE_KEY);
      const legacyStored = canonicalStored
        ? null
        : await this.storage.getItem(LEGACY_REGISTRY_STORAGE_KEY);
      const stored = canonicalStored ?? legacyStored;
      if (!canonicalStored && legacyStored) {
  ...
  async upsertTailnetConnection(input: {
    ...
    const tailnetAddress = normalizeHostPort(input.tailnetAddress);
    ...
    return this.upsertHostConnection({
      ...
      label: input.label,
      connection: {
        id: useTls ? `tailnet:wss:${tailnetAddress}` : `tailnet:${tailnetAddress}`,
        type: "tailnet",
        tailnetAddress,
  ...
  async upsertConnectionFromOffer(
    ...
    return this.upsertTailnetConnection({
      serverId: offer.serverId,
      tailnetAddress: offer.tailnetAddress,
      useTls: offer.useTls,
  ...
  async upsertConnectionFromOfferUrl(
    ...
    return this.upsertConnectionFromOffer(offer, label, pairingCode);
  }
  ...
  clearPendingPairingRequest(serverId: string): void {
    this.controllers.get(serverId)?.clearPendingPairingRequest();
  }

  completePendingPairingRequest(
    serverId: string,
    completion: Parameters<HostRuntimeController["completePendingPairingRequest"]>[0],
  ): void {
    this.controllers.get(serverId)?.completePendingPairingRequest(completion);
  }

  cancelPendingPairingRequest(serverId: string, requestId: string): void {
    this.controllers.get(serverId)?.cancelPendingPairingRequest(requestId);
  }
...
const HOST_RUNTIME_STORE_GLOBAL_KEY = "__jagentdeskHostRuntimeStore";
...
export function useHostMutations(): HostMutations {
  ...
  return useMemo(
    () => ({
      ...
      upsertTailnetConnection: (input) => store.upsertTailnetConnection(input),
      upsertConnectionFromOffer: (offer, label, code) =>
        store.upsertConnectionFromOffer(offer, label, code),
      upsertConnectionFromOfferUrl: (url, label, code) =>
        store.upsertConnectionFromOfferUrl(url, label, code),
