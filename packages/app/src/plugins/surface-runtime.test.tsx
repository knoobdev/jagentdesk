...
import { JAgentDeskApiProvider } from "@jagentdesk/plugin/client/host";
import { useJAgentDesk } from "@jagentdesk/plugin/client";
...
function borrowFromAppProvider(jagentdesk: JAgentDeskApi): JAgentDeskApi {
  let borrowed: JAgentDeskApi | null = null;
  function PluginSurface() {
    borrowed = useJAgentDesk();
    return null;
  }
  ...
  if (!borrowed) throw new Error("Plugin surface did not receive JAgentDesk API");
  return borrowed;
}

describe("plugin surface host runtime", () => {
  it("creates a PR worktree and agent through useJAgentDesk on the selected app host", async () => {
    ...
    const jagentdesk = borrowFromAppProvider(runtime.jagentdesk);
  ...
  it("switches all plugin calls when the selected host changes", async () => {
    ...
    await borrowFromAppProvider(second.jagentdesk).workspaces.create({
