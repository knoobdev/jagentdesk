...
describe("evaluatePluginClientBundle", () => {
  ...
  it("runs the client entry with the full runtime context", () => {
    const plugin = evaluatePluginClientBundle(
      "review",
      bundle(`
        if (!plugin.jagentdesk || !plugin.rpc || !plugin.openSurface || !plugin.openPanel || !plugin.addComposerPill) {
  ...
  it("provides the host Icon component through @jagentdesk/plugin/client/react-native", () => {
    const plugin = evaluatePluginClientBundle(
      "example",
      `(function(require) {
        const { Icon } = require("@jagentdesk/plugin/client/react-native");
        const module = { exports: {} };
  ...
  it("provides JAgentDesk UI through @jagentdesk/plugin/client/react-native", () => {
    const plugin = evaluatePluginClientBundle(
      "example",
      `(function(require) {
        const { Icon, Modal, useToast } = require("@jagentdesk/plugin/client/react-native");
        const module = { exports: {} };
        module.exports.default = function(plugin) {
  ...
  it("keeps shared and client runtime exports separate", () => {
    expect(() =>
      evaluatePluginClientBundle(
        "example",
        `(function(require) {
      const shared = require("@jagentdesk/plugin");
      const client = require("@jagentdesk/plugin/client");
      const { ExternalLink } = require("@jagentdesk/plugin/client/ui");
      if (typeof ExternalLink !== "function") throw new Error("ExternalLink");
      for (const name of ["useJAgentDesk", "useRpc", "useSettings", "useAgent", "useWorkspace", "openExternalUrl"]) {
  ...
  it.each([
    ...
    "@jagentdesk/plugin/client/host",
    "@jagentdesk/plugin/react-native",
    "@jagentdesk/plugin/ui",
    "@jagentdesk/plugin/host",
    // Upstream also rejects the retired `@paseo/plugin` scope; the fork rewrites it to the
    // valid `@jagentdesk/plugin` root at install time, so there is no fork equivalent here.
  ...
  it("resolves shared RPC helpers from @jagentdesk/plugin", () => {
    const plugin = evaluatePluginClientBundle(
      "example",
      `(function(require) {
        const { defineRpc, defineAttachmentSource } = require("@jagentdesk/plugin");
...
it("binds imported getters to each originating installation across delayed callbacks", async () => {
  ...
  const hostRuntime = (installation: string): PluginClientRuntime => ({
    ...
    hosts: {
      getSnapshot: () => [],
      subscribe: () => () => {},
      getJAgentDeskClient(serverId) {
        calls.push(`${installation}/${serverId}`);
        return runtime.jagentdesk;
      },
    },
  });
  const source = bundle(`
    const { getJAgentDeskClient } = require("@jagentdesk/plugin/client");
    getJAgentDeskClient("entry-host");
    plugin.addCommandCenterItem({
      id: "read", title: "Read", icon: "Server", context: "global",
      onSelect: async () => { await Promise.resolve(); getJAgentDeskClient("target-host"); },
    });
  `);
