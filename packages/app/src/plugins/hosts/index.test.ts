...
describe("plugin host access", () => {
  ...
  it("rejects unknown and disconnected targets without falling through to another host", () => {
    ...
    expect(() => h.runtime.getJAgentDeskClient("missing")).toThrow(
      "Unknown JAgentDesk host: missing",
    );
    expect(() => h.runtime.getJAgentDeskClient("b")).toThrow("JAgentDesk host is disconnected: b");
    h.lifetime.abort();
  });
  ...
  it("isolates installation lifetimes and releases APIs when a connection is replaced", async () => {
    ...
    const api = h.runtime.getJAgentDeskClient("b");
    expect(api).toBe(h.runtime.getJAgentDeskClient("b"));
    expect(api).not.toBe(other.getJAgentDeskClient("b"));
    ...
    h.publish();
    expect(() => h.runtime.getJAgentDeskClient("b")).toThrow("disconnected");
    expect(() => api.config.get()).toThrow("JAgentDesk host is disconnected: b");
    h.snapshots.set("b", { connectionStatus: "online", client });
    h.publish();
    expect(h.runtime.getJAgentDeskClient("b")).toBe(api);
    ...
    expect(() => api.config.get()).toThrow("JAgentDesk client is released: b");
    expect(h.runtime.getJAgentDeskClient("b")).not.toBe(api);
    h.lifetime.abort();
    expect(() => h.runtime.getJAgentDeskClient("b")).toThrow("Plugin has stopped");
    expect(() => other.getJAgentDeskClient("b").agents.subscribe(ignoreUpdate)).not.toThrow();
    otherLifetime.abort();
  });
});
...
it("reacquires a fresh API after explicit disposal without affecting a later borrower", async () => {
  ...
  const first = h.runtime.getJAgentDeskClient("b");
  await first.dispose();
  const second = h.runtime.getJAgentDeskClient("b");
  expect(second).not.toBe(first);
  expect(() => second.agents.subscribe(ignoreUpdate)).not.toThrow();
  expect(() => first.config.get()).toThrow("JAgentDesk client is released: b");
  await first.dispose();
  expect(h.runtime.getJAgentDeskClient("b")).toBe(second);
  h.lifetime.abort();
