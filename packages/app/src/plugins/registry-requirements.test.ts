...
it("checks the app version before creating a runtime or evaluating plugin code", () => {
  ...
  expect(result.getEvaluationError("host", "example")).toBe(
    'Plugin "example" requires JAgentDesk >=0.9.0. Your app is 0.8.0. Use a compatible plugin version or update the app.',
  );
});

it("starts catalogs without requirements (pre-split plugins keep loading)", () => {
  const { result, starts } = registry("0.8.0");
...
it("unloads on a requirement-only edit and recovers after correction", () => {
  ...
  const install = (jagentdesk: string) =>
    result.installCatalog("host", [{ id: "example", clientBundle, requirements: { jagentdesk } }], {
      client,
    });
  install("^0.8.0");
  ...
  expect(starts()).toBe(1);
  expect(result.getSnapshot()).toEqual([]);
  expect(result.getEvaluationError("host", "example")).toContain("requires JAgentDesk >=0.9.0");
  install(">=0.8.0");
