  ...
  installCatalog(
    ...
    const preserved = catalog.flatMap((entry) => {
      const existing = previous.find(
        (plugin) =>
          ...
          plugin.requirements?.jagentdesk === entry.requirements?.jagentdesk,
      );
    ...
    const installed = catalog.flatMap((entry) => {
      ...
      try {
        ...
        const jagentdesk = runtime.jagentdesk;
        installation.cleanup = async () => {
          const results = await Promise.allSettled([jagentdesk.dispose(), evaluated.cleanup()]);
