...
describe("startup blocking policy", () => {
  ...
  it("keeps navigation available while desktop starts the managed daemon in the background", () => {
...
describe("resolveStartupRoute", () => {
  ...
  it("does not keep a saved Tailscale-mode desktop on splash while daemon startup is pending", () => {
    expect(
      resolveStartupRoute({
        ...baseIndexInput,
  ...
  it("restores a saved host without waiting for a Tailscale health result", () => {
    expect(
      resolveStartupRoute({
        ...baseIndexInput,
        hosts: [{ serverId: "server-1" }],
