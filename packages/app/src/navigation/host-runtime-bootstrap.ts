...
export function resolveStartupNavigationReady(input: { startupBlocker: StartupBlocker }): boolean {
  ...
  // surface, so navigation itself remains mounted for retry actions.
  return true;
}
...
export function shouldRunStartupGiveUpTimer(input: {
  ...
  // A slow daemon must not suspend the give-up timer. Without this, a cold
  // desktop launch can remain on the splash forever while the login gate is
  // waiting for the user to authenticate Tailscale.
  return input.startupBlocker.kind !== "managed-daemon-error";
}
