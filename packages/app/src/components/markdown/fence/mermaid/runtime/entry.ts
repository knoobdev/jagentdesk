...
declare global {
  interface Window {
    ...
    __JAGENTDESK_MERMAID_RUNTIME_RECEIVE__?: (message: unknown) => void;
  }
}
...
async function render(message: MermaidRuntimeRenderMessage): Promise<void> {
  try {
    ...
    const { svg } = await mermaid.render(`jagentdesk-mermaid-${message.revision}`, message.source);
...
window.__JAGENTDESK_MERMAID_RUNTIME_RECEIVE__ = receiveRender;
