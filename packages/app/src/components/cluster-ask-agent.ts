...
export async function askAgentAboutResource(input: AskAgentAboutResourceInput): Promise<void> {
  ...
  const context = [
    ...
    "kubeconfig:",
    `  • kubectl_get  — action get/describe/logs/list, always clusterId="${clusterId}"`,
    `  • kubectl_apply — for changes, always clusterId="${clusterId}"`,
    "If these tools are unavailable, you may fall back to the kubectl CLI via Bash,",
    "but the dedicated tools are more reliable and target the right cluster.",
