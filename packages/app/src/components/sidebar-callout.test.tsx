...
function buildTwoActions(onDetails: () => void, onSettings: () => void): SidebarCalloutActions {
  return [
    { label: "View details", onPress: onDetails },
    { label: "Open settings", onPress: onSettings, variant: "primary" },
  ];
}
...
describe("SidebarCallout", () => {
  ...
  it("renders title and description", () => {
    act(() => {
      root?.render(
        <SidebarCallout title="Notice available" description="A new notice is ready." />,
      );
    });

    expect(container?.textContent).toContain("Notice available");
    expect(container?.textContent).toContain("A new notice is ready.");
  });

  it("renders an icon next to the title", () => {
    act(() => {
      root?.render(<SidebarCallout title="Notice available" icon={calloutTitleIcon} />);
    });
