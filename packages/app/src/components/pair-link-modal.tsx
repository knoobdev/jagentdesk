...
import { savePendingPairingOffer } from "@/pairing/pending-pairing-offer";
...
export function PairLinkModal({
  ...
  // The compact sheet emits `onDismiss` while the route is being replaced.
  // Keep close/back callbacks suppressed for the entire submit/navigation
  // transaction; otherwise the sheet's automatic dismiss can call
  // `router.back()` after `router.replace("/pair-verify")` and lose the new
  // route even though the pending offer was already persisted.
  ...
  const handleClose = useCallback(() => {
    if (isSaving || closeBlockedRef.current) return;
    resetInput();
    onClose();
  }, [isSaving, onClose, resetInput]);

  const handleCancel = useCallback(() => {
    if (isSaving || closeBlockedRef.current) return;
    resetInput();
    (onCancel ?? onClose)();
  }, [isSaving, onCancel, onClose, resetInput]);
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      await savePendingPairingOffer({ offerUrl: raw, source });
      resetInput();
      const sourceParam = source ? `&source=${source}` : "";
      router.replace(`/pair-verify?offer=${encodeURIComponent(raw)}${sourceParam}` as Href);
    } catch (error) {
  ...
  }, [isMobile, isSaving, resetInput, router, source, t]);
  ...
  const header = useMemo<SheetHeader>(
    () => ({ title: title ?? t("pairing.link.title") }),
    [t, title],
  );
    ...
    >
      ...
      <View style={styles.field}>
        ...
        <AdaptiveTextInput
          ...
          placeholder="jagentdesk://app/#offer=..."
