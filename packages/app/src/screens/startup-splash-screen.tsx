...
const WEB_SPLASH_SHIMMER_KEYFRAME_ID = "jagentdesk-splash-shimmer-keyframes";
const WEB_SPLASH_SHIMMER_ANIMATION_NAME = "jagentdesk-splash-shimmer";
...
function WebLogoShimmer({ color }: { color: string }) {
  ...
  // The previous web-only mask contained JAgentDesk's original path. Keep the
  // shimmer setup harmless, but render the canonical JAgentDesk mark here so
  // desktop/web startup cannot display the inherited JAgentDesk logo.
  return <JAgentDeskLogo size={LOGO_SIZE} color={color} />;
}
  ...
  const maskElement = useMemo(
    () => (
      <View style={styles.shimmerMask}>
        <JAgentDeskLogo size={LOGO_SIZE} color="#000000" />
      </View>
    ),
    [],
  );
    ...
    <MaskedView style={trackStyle} maskElement={maskElement}>
      <View style={trackStyle}>
        <View style={styles.nativeShimmerBase}>
          <JAgentDeskLogo size={LOGO_SIZE} color={color} />
        </View>
