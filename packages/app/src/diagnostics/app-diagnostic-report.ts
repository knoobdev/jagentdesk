import type { ServerInfoStatusPayload } from "@jagentdesk/protocol/messages";
...
export function formatAppDiagnosticHeader(input: {
  ...
  return formatDiagnosticSection("JAgentDesk app diagnostics", [
...
export function redactAppDiagnosticReport(report: string, hosts: HostProfile[]): string {
  ...
  return redacted
    .replace(/(?:jagentdesk|jagentdesk):\/\/\S+/gi, "jagentdesk://[redacted]")
    .replace(
...
function collectSensitiveHostValues(hosts: HostProfile[]): string[] {
  ...
  for (const host of hosts) {
    for (const connection of host.connections) {
      ...
      } else if (connection.type === "tailnet") {
        values.add(connection.tailnetAddress);
