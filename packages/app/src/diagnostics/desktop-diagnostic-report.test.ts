...
function makeSources(): DesktopDiagnosticSources {
  return {
    getStatus: async () => ({
      ...
      hostname: "host",
      pid: 4242,
      home: "/jagentdesk/home",
      version: "1.2.3",
      desktopManaged: true,
      error: null,
    }),
    getDaemonLogs: async () => ({
      logPath: "/jagentdesk/home/daemon.log",
      contents: "daemon line one\ndaemon line two",
    }),
    getAppLogs: async () => ({
      logPath: "/logs/JAgentDesk/main.log",
...
describe("desktop diagnostic report", () => {
  ...
  test("includes the Electron main-process log after the daemon log", async () => {
    ...
    expect(report).toContain("  Log path: /jagentdesk/home/daemon.log");
    expect(report).toContain("  App log path: /logs/JAgentDesk/main.log");
