...
// Regression: "streaming pino log, resized the JAgentDesk terminal, old logs stayed
...
function mount(width: number, height: number): Mounted {
  ...
  const terminal = window.__jagentdeskTerminal as InspectableTerminal | undefined;
  if (!terminal) {
...
describe("terminal resize reflow repro (JAgentDesk terminal)", () => {
