...
import type { ProviderSubagentDescriptorPayload } from "@jagentdesk/protocol/messages";

export interface JAgentDeskSubagentRow {
  kind: "jagentdesk";
  id: Agent["id"];
...
export type SubagentRow = JAgentDeskSubagentRow | ProviderSubagentRow;
...
export function useSubagentsForParent(params: SelectSubagentsParams): SubagentRow[] {
  ...
  const jagentdeskRows = useStoreWithEqualityFn(
    useSessionStore,
  ...
  return useMemo(() => {
    if (providerRows.length === 0) return jagentdeskRows;
    const rows = [...jagentdeskRows, ...providerRows];
    rows.sort((left, right) => left.createdAt.getTime() - right.createdAt.getTime());
    return rows;
  }, [jagentdeskRows, providerRows]);
}
