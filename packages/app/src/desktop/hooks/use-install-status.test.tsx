...
describe("useCliInstall", () => {
  ...
  it("toasts and exposes CLI install errors", async () => {
    ...
    expect(toast.error).toHaveBeenCalledWith("Unable to install the JAgentDesk CLI.");
  ...
  it("uses the active language for CLI install errors", async () => {
    ...
    expect(toast.error).toHaveBeenCalledWith("无法安装 JAgentDesk CLI。");
  });
});
...
describe("useSkillsStatus", () => {
  ...
  it("update transitions drift to up-to-date", async () => {
    desktopDaemon.getSkillsSnapshot.mockResolvedValue({
      state: "drift",
      ops: [{ kind: "update", name: "jagentdesk" }],
    });
