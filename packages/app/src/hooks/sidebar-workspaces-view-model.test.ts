...
describe("createSidebarWorkspaceEntry workspace directory label", () => {
  it("uses the daemon-provided slug for a JAgentDesk-owned worktree", () => {
    ...
    const projects = buildSidebarProjectsFromStructure({
      projects: [
        project({
          ...
          hosts: [
            {
              serverId: "tailnet:jagentdesk-host",
              iconWorkingDir: "/repo/project-1",
              worktreeSupport: "supported" as const,
            },
          ],
          workspaceKeys: ["tailnet:jagentdesk-host:ws-main"],
        }),
      ],
    });

    expect(projects[0]?.workspaces[0]).toMatchObject({
      workspaceKey: "tailnet:jagentdesk-host:ws-main",
      serverId: "tailnet:jagentdesk-host",
      workspaceId: "ws-main",
    });
  });
...
describe("shared sidebar workspace model", () => {
  it("feeds project placement and status grouping from the same cross-host workspace identities", () => {
    const model = buildSidebarWorkspacePlacementModel({
      projects: [
        project({
          ...
          iconWorkingDir: "/repo/acme/example",
          hosts: [
            {
              serverId: "host-a",
              iconWorkingDir: "/repo/acme/example",
          ...
          workspaces: new Map([
            [
              ...
              workspace({
                ...
                projectDisplayName: "acme/example",
                status: "done",
              }),
            ],
          ]),
    ...
    expect(model.projectNamesByViewKey).toEqual(new Map([["acme/example", "acme/example"]]));
  });
      ...
      projects: [
        project({
          ...
          hosts: [
            {
              serverId: "host-a",
              iconWorkingDir: "/repo/jagentdesk",
              worktreeSupport: "supported" as const,
            },
            {
              serverId: "host-b",
