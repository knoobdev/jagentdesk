...
describe("Add Project options", () => {
  ...
  it("offers manual URL and protocol-specific owner/repo clone choices", () => {
    expect(buildManualGithubRepositoryChoices("git@github.com:acme/example.git")).toEqual([
      expect.objectContaining({
        id: "manual:git@github.com:acme/example.git",
        nameWithOwner: "acme/example",
        cloneUrl: "git@github.com:acme/example.git",
      }),
    ]);
    expect(buildManualGithubRepositoryChoices("acme/example")).toEqual([
      expect.objectContaining({ cloneProtocol: "https", cloneUrl: "acme/example" }),
      expect.objectContaining({ cloneProtocol: "ssh", cloneUrl: "acme/example" }),
    ]);
    expect(buildManualGithubRepositoryChoices("jagentdesk")).toEqual([]);
  });
  ...
  it("shows final clone paths while retaining parent paths as values", () => {
    expect(
      buildCloneLocationOptions({
        ...
        repositoryName: "jagentdesk",
        existingPaths: ["~/workspace/jagentdesk"],
      }),
    ).toEqual([
      {
        id: "~/dev",
        path: "~/dev",
        displayPath: "~/dev/jagentdesk",
      ...
      },
      {
        id: "~/workspace",
        path: "~/workspace",
        displayPath: "~/workspace/jagentdesk",
