...
if (jagentdeskArchitectures.size() == 1) {
    def jagentdeskAbi = jagentdeskArchitectures[0]
    def jagentdeskAbiVersionCode = jagentdeskAbiVersionCodes[jagentdeskAbi]
    if (jagentdeskAbiVersionCode == null) {
        throw new GradleException("Unsupported JAgentDesk Android ABI: " + jagentdeskAbi)
    }
    android.defaultConfig.versionCode = android.defaultConfig.versionCode * 10 + jagentdeskAbiVersionCode
}
`;
...
function configureFdroidAppBuildGradle(contents) {
  ...
  if (!configuredContents.includes("// JAgentDesk F-Droid single-ABI version codes")) {
