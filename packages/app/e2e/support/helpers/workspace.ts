...
export const createTempGitRepo = async (
  ...
  execSync('git config user.email "e2e@jagentdesk.test"', { cwd: repoPath, stdio: "ignore" });
  execSync('git config user.name "JAgentDesk E2E"', { cwd: repoPath, stdio: "ignore" });
  execSync("git config commit.gpgsign false", { cwd: repoPath, stdio: "ignore" });
  await writeFile(path.join(repoPath, "README.md"), "# Temp Repo\n");
  if (options?.jagentdeskConfig) {
    await writeFile(
      path.join(repoPath, "jagentdesk.json"),
  ...
  if (options?.jagentdeskConfig) {
    execSync("git add jagentdesk.json", { cwd: repoPath, stdio: "ignore" });
  }
  for (const file of options?.files ?? []) {
  ...
  for (const branch of branches) {
    ...
    const markerPath = `.jagentdesk-e2e-${branch.replace(/[^a-zA-Z0-9._-]/g, "-")}.txt`;
 ...
 */
export async function createTempDirectory(prefix = "jagentdesk-e2e-dir-"): Promise<TempDirectory> {
