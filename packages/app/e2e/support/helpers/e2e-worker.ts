...
import { forkJAgentDeskHomeMetadata, resolveJAgentDeskHomePath } from "./jagentdesk-home-fork";
...
function resolveOptionalHome(value: string | undefined): string | null {
  ...
  return resolveJAgentDeskHomePath(trimmed === "current" ? "~/.jagentdesk" : trimmed);
}
...
const recordPath = process.env.JAGENTDESK_E2E_EDITOR_RECORD_PATH;
if (recordPath) {
...
const fixtureRemote = "https://github.com/jagentdesk-e2e/local-fixture.git";
...
if (origin === fixtureRemote) {
  ...
  if (command === "repo view") {
    process.stdout.write(JSON.stringify({ owner: { login: "jagentdesk-e2e" }, name: "local-fixture", parent: null }));
    process.exit(0);
  }
  ...
  if (command === "pr list" || command === "pr view") {
    const pr = {
      ...
      url: "https://github.com/jagentdesk-e2e/local-fixture/pull/1",
      state: "OPEN",
      body: null,
      labels: [],
      baseRefName: "main",
  ...
  if (command === "api graphql" && args.some((arg) => arg.includes("PullRequestCheckoutTarget"))) {
    process.stdout.write(JSON.stringify({
      data: { repository: { pullRequest: {
        ...
        headRepositoryOwner: { login: "jagentdesk-e2e" },
        headRepository: {
          sshUrl: "git@github.com:jagentdesk-e2e/local-fixture.git",
          url: fixtureRemote
        }
      } } }
    }));
...
async function applyMetadataFork(targetHome: string, providerIds: string[]): Promise<void> {
  const sourceHome = resolveOptionalHome(process.env.E2E_FORK_JAGENTDESK_HOME_FROM);
  if (!sourceHome) return;
  const result = await forkJAgentDeskHomeMetadata({ sourceHome, targetHome });
  process.env.E2E_FORK_SOURCE_JAGENTDESK_HOME = result.sourceHome;
  process.env.E2E_FORK_TARGET_JAGENTDESK_HOME = result.targetHome;
...
export async function startE2EWorker(
  ...
  const jagentdeskHome = requestedRoot
    ...
    : await mkdtemp(path.join(tmpdir(), `jagentdesk-e2e-worker-${workerIndex}-`));
  const preserveHome = Boolean(requestedRoot) || process.env.E2E_KEEP_JAGENTDESK_HOME === "1";
  const fakeEditorBin = await createFakeEditorBin();
  const editorRecordPath = path.join(jagentdeskHome, "editor-open-records.jsonl");
  ...
  try {
    ...
    const daemon = await startIsolatedHostDaemon(serverId, {
      ...
      environment: {
        ...
        JAGENTDESK_E2E_EDITOR_RECORD_PATH: editorRecordPath,
      },
    });
