...
export interface JAgentDeskHomeMetadataForkResult {
  sourceHome: string;
  targetHome: string;
  agentFiles: number;
  agentBytes: number;
...
export function resolveJAgentDeskHomePath(value: string): string {
  if (value === "~") {
    return homedir();
  }
  if (value.startsWith("~/")) {
...
export async function forkJAgentDeskHomeMetadata(input: {
  sourceHome: string;
  targetHome: string;
}): Promise<JAgentDeskHomeMetadataForkResult> {
  const sourceHome = resolveJAgentDeskHomePath(input.sourceHome);
  const targetHome = resolveJAgentDeskHomePath(input.targetHome);

  if (sourceHome === targetHome) {
    throw new Error("Refusing to fork JAgentDesk metadata onto the same JAGENTDESK_HOME.");
  }
