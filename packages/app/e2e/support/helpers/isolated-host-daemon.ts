...
export async function startIsolatedHostDaemon(
  ...
  const jagentdeskHome =
    options.jagentdeskHome ?? (await mkdtemp(path.join(tmpdir(), "jagentdesk-e2e-secondary-host-")));
  let publishedPackageRoot: string | null = null;
  if (options.publishedVersion) {
    publishedPackageRoot = await mkdtemp(path.join(tmpdir(), "jagentdesk-e2e-published-server-"));
    await writeFile(
    ...
    try {
      execFileSync(
        ...
        [
          ...
          "--no-fund",
          "--no-package-lock",
          `@jagentdesk/server@${options.publishedVersion}`,
        ],
  ...
  const serverDir = publishedPackageRoot
    ? path.join(publishedPackageRoot, "node_modules", "@jagentdesk", "server")
  ...
  const spawnDaemon = async (): Promise<ChildProcess> => {
    const spawnOptions: SpawnOptions = {
      ...
      env: withDisabledE2ESpeechEnv({
        ...
        JAGENTDESK_CORS_ORIGINS: `http://localhost:${metroPort}`,
