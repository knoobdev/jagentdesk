...
export async function corruptJAgentDeskConfig(repoPath: string): Promise<void> {
  await writeFile(path.join(repoPath, "jagentdesk.json"), "{not valid json}");
}

export async function bumpJAgentDeskConfigOnDisk(repoPath: string): Promise<void> {
  const configPath = path.join(repoPath, "jagentdesk.json");
...
export async function restoreJAgentDeskConfig(
  repoPath: string,
  config: Record<string, unknown>,
): Promise<void> {
  await writeFile(path.join(repoPath, "jagentdesk.json"), JSON.stringify(config, null, 2) + "\n");
}
...
export async function blockJAgentDeskConfigWrites(repoPath: string): Promise<void> {
  await chmod(repoPath, 0o555);
}

export async function unblockJAgentDeskConfigWrites(repoPath: string): Promise<void> {
  await chmod(repoPath, 0o755);
}

// --- WebSocket helpers ---

// Proxies all daemon WS traffic transparently, but rejects jagentdesk.json reads
