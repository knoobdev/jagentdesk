...
export async function createProjectPickerFixture(): Promise<ProjectPickerFixtureResource> {
  const root = await mkdtemp(path.join(homedir(), "jagentdesk-e2e-project-picker-"));
  const nonce = randomUUID().replaceAll("-", "").slice(0, 8);
  const projectPath = path.join(root, "client", "team", `jagentdesk-desktop-fuzzy-target-${nonce}`);
