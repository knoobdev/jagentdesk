...
import type { DaemonClient as InternalDaemonClient } from "@jagentdesk/client/internal/daemon-client";
...
export async function archiveWorkspaceFromDaemon(
  ...
  const payload = await client.archiveJAgentDeskWorktree({
...
export async function createWorktreeViaDaemon(
  ...
  const payload = await client.createJAgentDeskWorktree({
    cwd: input.cwd,
