    ...
    ({ accessibleName, errorText, tabTestId }) => {
      ...
      (
        window as unknown as {
          __jagentdeskAssistantImageObservation?: typeof record;
        }
      ).__jagentdeskAssistantImageObservation = record;
    },
    {
      accessibleName: alt,
...
async function expectNoVisibleImageInstability(page: Page): Promise<void> {
  const observation = await page.evaluate(() => {
    const owner = window as unknown as {
      __jagentdeskAssistantImageObservation?: {
        errorSeen: boolean;
        missingSeen: boolean;
        inspect(): void;
    ...
    const record = owner.__jagentdeskAssistantImageObservation;
    if (!record) throw new Error("Assistant image stability observation was not started");
    record.inspect();
    record.observer?.disconnect();
    delete owner.__jagentdeskAssistantImageObservation;
