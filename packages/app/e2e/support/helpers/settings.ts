...
const DISABLE_DEFAULT_SEED_ONCE_KEY = "@jagentdesk:e2e-disable-default-seed-once";
const SEED_NONCE_KEY = "@jagentdesk:e2e-seed-nonce";
const REGISTRY_KEY = "@jagentdesk:daemon-registry";
...
export async function selectHostConnectionType(
  ...
  const label = type === "direct" ? "Direct connection" : "Pair over Tailscale";
...
export async function expectHostInjectMcpCard(page: Page): Promise<void> {
  ...
  await expect(card).toBeVisible();
  await expect(card.getByRole("switch", { name: "Inject JAgentDesk tools" })).toBeVisible();
}
