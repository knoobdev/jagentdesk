...
async function seedPr(args: {
  ...
  git(["config", "user.email", "e2e@jagentdesk.test"], localPath);
  git(["config", "user.name", "JAgentDesk E2E"], localPath);
...
const TEMP_GITHUB_REPO_PREFIX = "jagentdesktmp-";
...
export async function createTempGithubRepo(options: {
  ...
  git(["config", "user.email", "e2e@jagentdesk.test"], basePath);
  git(["config", "user.name", "JAgentDesk E2E"], basePath);
...
export async function cloneGithubRepoDefaultBranchOnly(
  ...
  git(["config", "user.email", "e2e@jagentdesk.test"], clonePath);
  git(["config", "user.name", "JAgentDesk E2E"], clonePath);
...
export async function createLocalGithubPrFixture(): Promise<LocalGhPrFixture> {
  const fixtureRoot = await mkdtemp(path.join("/tmp", "jagentdesk-e2e-local-github-pr-"));
  ...
  const githubUrl = "https://github.com/jagentdesk-e2e/local-fixture.git";
  ...
  git(["config", "user.email", "e2e@jagentdesk.test"], checkoutPath);
  git(["config", "user.name", "JAgentDesk E2E"], checkoutPath);
