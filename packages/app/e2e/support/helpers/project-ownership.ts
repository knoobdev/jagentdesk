...
interface ProjectClient {
  ...
  close(): Promise<void>;
  createJAgentDeskWorktree?: (...args: unknown[]) => Promise<OwnedResourceResult>;
...
export function withProjectOwnership<Client extends object>(client: Client): Client {
  ...
  return new Proxy(client, {
    get(_target, property) {
      ...
      if (property === "createJAgentDeskWorktree" && target.createJAgentDeskWorktree) {
        const createJAgentDeskWorktree = target.createJAgentDeskWorktree.bind(target);
        return async (...args: unknown[]) => trackProject(await createJAgentDeskWorktree(...args));
      }
