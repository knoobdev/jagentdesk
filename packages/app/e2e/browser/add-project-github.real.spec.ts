...
test.describe("Add Project GitHub flow", () => {
  ...
  test("searches the host's repositories and clones into a clearly shown final path", async ({
    ...
    const parentDirectory = await mkdtemp(path.join(tmpdir(), "jagentdesk-e2e-github-clone-"));
    ...
    try {
      ...
      await addProjectFlowInput(page).fill("acme/example");
      await expect(addProjectFlow(page).getByText("acme/example", { exact: true })).toBeVisible({
        timeout: 30_000,
      });
