...
async function seedRestartHome(): Promise<SeededRestartHome> {
  const jagentdeskHome = mkdtempSync(path.join(tmpdir(), "jagentdesk-playwright-restart-home-"));
  const cwd = mkdtempSync(path.join(tmpdir(), "jagentdesk-playwright-restart-cwd-"));
  const projectsDir = path.join(jagentdeskHome, "projects");
  const agentDir = path.join(jagentdeskHome, "agents", projectDirNameFromCwd(cwd));
...
async function startRestartDaemon(input: {
  ...
  const child = spawn(tsxBin, ["scripts/supervisor-entrypoint.ts", "--dev"], {
    ...
    env: withDisabledE2ESpeechEnv({
      ...process.env,
      JAGENTDESK_HOME: input.jagentdeskHome,
      JAGENTDESK_SERVER_ID: SERVER_ID,
      JAGENTDESK_LISTEN: `127.0.0.1:${port}`,
      JAGENTDESK_CORS_ORIGINS: input.origin,
...
async function seedBrowserForDaemon(page: Page, input: { serverId: string; port: number }) {
  ...
  await page.evaluate(
    ({ daemon, preferences }) => {
      ...
      localStorage.setItem("@jagentdesk:daemon-registry", JSON.stringify([daemon]));
...
test.describe("Workspace model restart regressions", () => {
  test("browser-created same-cwd workspace preserves restarted agent status and migrated tab ownership", async ({
    ...
    const daemon = await startRestartDaemon({ jagentdeskHome: seeded.jagentdeskHome, origin });
