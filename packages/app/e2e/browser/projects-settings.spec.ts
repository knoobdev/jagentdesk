...
import {
  ...
  openProjects,
  removeProjectScript,
  restoreJAgentDeskConfig,
  returnToProjectsList,
  saveProjectEdits,
  unblockJAgentDeskConfigWrites,
...
const test = base.extend<ProjectsSettingsFixtures>({
  editableProject: async ({ page: _page }, provide) => {
    const workspace = await seedWorkspace({
      ...
      repo: { jagentdeskConfig: initialJAgentDeskConfig },
    });
  ...
  gitlabRemoteProject: async ({ page: _page }, provide) => {
    const workspace = await seedWorkspace({
      ...
      repo: {
        jagentdeskConfig: initialJAgentDeskConfig,
...
async function expectProjectConfigSaved(project: ProjectsSettingsProject): Promise<void> {
  await expect
    ...
    .toMatchObject({
      worktree: {
        setup: updatedSetup,
        teardown: initialJAgentDeskConfig.worktree.teardown,
        customWorktreeField: initialJAgentDeskConfig.worktree.customWorktreeField,
      },
      scripts: {
        dev: {
          command: initialJAgentDeskConfig.scripts.dev.command,
          type: initialJAgentDeskConfig.scripts.dev.type,
          port: initialJAgentDeskConfig.scripts.dev.port,
          customScriptField: initialJAgentDeskConfig.scripts.dev.customScriptField,
        },
      },
      customTopLevelField: initialJAgentDeskConfig.customTopLevelField,
    });
...
async function readProjectConfigFile(project: ProjectsSettingsProject): Promise<string> {
  return readFile(path.join(project.path, "jagentdesk.json"), "utf8");
}
...
test.describe("Projects settings — error UX", () => {
  test("stale-write callout appears on save, disables save, and reload clears it", async ({
    ...
    await bumpJAgentDeskConfigOnDisk(editableProject.path);
  ...
  test("invalid jagentdesk.json shows read-error callout, reload after fix shows form", async ({
    page,
    editableProject,
  }) => {
    await corruptJAgentDeskConfig(editableProject.path);
    ...
    await restoreJAgentDeskConfig(editableProject.path, initialJAgentDeskConfig);
  ...
  test("write_failed callout appears on save with blocked directory, retry re-attempts, reload clears it", async ({
    ...
    await blockJAgentDeskConfigWrites(editableProject.path);
    ...
    await unblockJAgentDeskConfigWrites(editableProject.path);
    await clickReloadProjectSettings(page);
