...
const PLUGIN_SOURCE = `import { useJAgentDesk } from "@jagentdesk/plugin";
import { Icon, Modal, useToast } from "@jagentdesk/plugin/react-native";
...
test("plugin modal adapts its presentation and preserves host contexts", async ({ page }) => {
  const directory = await mkdtemp(path.join(tmpdir(), "jagentdesk-plugin-host-ui-e2e-"));
  const client = await connectNewWorkspaceDaemonClient({ ownProjects: false });
  const previousConfig = await client.getDaemonConfig();
  await writeFile(path.join(directory, "jagentdesk-plugin.json"), JSON.stringify({ id: PLUGIN_ID }));
