...
test("reloading a timeline anchors near-tail assistant image growth", async ({
    ...
    ({ accessibleName }) => {
      ...
      (
        window as unknown as {
          __jagentdeskReloadImageGeometry?: typeof state;
        }
      ).__jagentdeskReloadImageGeometry = state;
    },
  ...
  const events = await page.evaluate(() => {
    const state = (
      window as unknown as {
        __jagentdeskReloadImageGeometry?: { events: Array<unknown> };
      }
    ).__jagentdeskReloadImageGeometry;
