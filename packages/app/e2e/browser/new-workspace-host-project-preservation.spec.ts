...
const SHARED_PROJECT_NAME = "JAgentDesk";
const SHARED_REMOTE_URL = "https://github.com/acme/example.git";
...
const test = base.extend<{ crossHostProject: CrossHostProjectScenario }>({
  crossHostProject: async ({ page: _page }, provide) => {
    ...
    const primarySharedRepo = await createTempGitRepo("new-workspace-jagentdesk-primary-", {
      originUrl: SHARED_REMOTE_URL,
    });
    const primaryDuplicateRepo = await createTempGitRepo("new-workspace-jagentdesk-duplicate-", {
      originUrl: SHARED_REMOTE_URL,
    });
    const secondaryFallbackRepo = await createTempGitRepo("new-workspace-fallback-");
    const secondarySharedRepo = await createTempGitRepo("new-workspace-jagentdesk-secondary-", {
      originUrl: SHARED_REMOTE_URL,
    });
    ...
    try {
      ...
      const primarySharedProject = await createProject(primaryClient, {
        ...
        workspaceName: "Primary JAgentDesk workspace",
      });
      ...
      const primaryDuplicateProject = await createProject(primaryClient, {
        ...
        projectName: "JAgentDesk duplicate",
        workspaceName: "Duplicate JAgentDesk workspace",
      });
      ...
      const secondarySharedProject = await createProject(secondaryClient, {
        ...
        projectName: SHARED_PROJECT_NAME,
        workspaceName: "Secondary JAgentDesk workspace",
      });
      ...
      if (
        ...
        throw new Error("Expected the JAgentDesk clones to have the same equivalence key");
      }
