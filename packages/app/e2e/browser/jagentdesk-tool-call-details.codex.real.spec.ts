...
test.use({ e2eInjectJAgentDeskTools: true });

function verificationPrompt(workspacePath: string): string {
  return [
    "Use JAgentDesk tools to perform this exact UI verification.",
    ...
    "Use each named JAgentDesk tool exactly once except for the matching schedule and heartbeat deletion tools.",
  ].join(" ");
}
...
test.describe("real Codex JAgentDesk tool-call presentation", () => {
  test.setTimeout(600_000);

  test("renders owned tool inputs and results as readable text", async ({ page }, testInfo) => {
    const cwd = realpathSync(mkdtempSync(path.join(tmpdir(), "jagentdesk-tool-call-presentation-")));
