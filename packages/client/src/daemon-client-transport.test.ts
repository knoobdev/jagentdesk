...
describe("daemon-client transport helpers", () => {
  ...
  test("extractTransportMessage preserves browser and Node WebSocket frame kind", () => {
    expect(extractTransportMessage({ data: "hello" })).toEqual({
      data: "hello",
      isBinary: false,
    });

    const view = new Uint8Array([1, 2, 3]);
    const binary = extractTransportMessage(view, true);
    expect(binary.isBinary).toBe(true);
    expect(Array.from(new Uint8Array(binary.data as ArrayBuffer))).toEqual([1, 2, 3]);

    const text = extractTransportMessage(view, false);
