...
Read the **jagentdesk** skill. Before choosing committee members, read `~/.jagentdesk/orchestration-preferences.json` unless the user explicitly named providers in this request. Do not create committee agents until you have read it.
...
Create both agents in parallel via JAgentDesk with `[Committee] <task>` titles and the same prompt. Wait for both — not just whichever finishes first.
