...
Read the **jagentdesk** skill. Before choosing worker or verifier providers, read `~/.jagentdesk/orchestration-preferences.json` unless the user explicitly named providers in this request. Do not start the loop until you have read it.

Loops are a CLI primitive: `jagentdesk loop run`. Manage with `jagentdesk loop ls`, `jagentdesk loop inspect <id>`, `jagentdesk loop logs <id>`, `jagentdesk loop stop <id>`.
...
8. Launch with `jagentdesk loop run`.
