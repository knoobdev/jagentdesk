---
name: jagentdesk
description: JAgentDesk reference for managing workspaces, workspace scripts, agents, schedules, and heartbeats.
---

JAgentDesk is a daemon that supervises AI coding agents on your machine. Control it through tools or a CLI.
...
Configured `jagentdesk.json` scripts use the same supervised lifecycle from tools and the CLI.
...
**`start_workspace_script`** — `{ workspaceId, scriptName }`. Starts one configured script through JAgentDesk's managed workspace-script launcher and returns its status metadata.
...
All providers shown in this document are examples, you must resolve real providers either found in the preferences or by calling JAgentDesk provider tooling to find a valid provider.
...
jagentdesk heartbeat create --cron "*/15 * * * *" "check the build"
```

Discover with `jagentdesk --help` and `jagentdesk <cmd> --help`.

**If `jagentdesk` isn't on PATH but the desktop app is installed**, the bundled CLI is at:

- macOS: `/Applications/JAgentDesk.app/Contents/Resources/bin/jagentdesk`
- Linux: `<install-dir>/resources/bin/jagentdesk`
- Windows: `C:\Program Files\JAgentDesk\resources\bin\jagentdesk.cmd`

The desktop app's first-run hook (`installCli`) symlinks this to `~/.local/bin/jagentdesk` (macOS/Linux) or drops a `.cmd` trampoline (Windows) and adds `~/.local/bin` to PATH via shell rc files. If that didn't take, offer to symlink it — don't do it silently.
...
1. `tail -n 200 ~/.jagentdesk/daemon.log`.
2. `jagentdesk daemon status` for liveness.
