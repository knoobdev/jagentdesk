...
Read the **jagentdesk** skill. Before choosing a provider, read `~/.jagentdesk/orchestration-preferences.json` unless the user explicitly named a provider in this request. Do not create the advisor until you have read it.
...
- `/jagentdesk-advisor /unslop` → advisor runs `/unslop` on the current diff.
- `/jagentdesk-advisor /unslop-risk` → advisor does an unslop-risk review.
- `/jagentdesk-advisor $diagnose this build failure` → advisor invokes `/diagnose`.
...
Create the advisor agent via JAgentDesk with a `[Advisor] <topic>` title and the briefing as the initial prompt. Wait for it to finish. Read its response. Synthesize for the user — the advisor's verdict + your recommendation.
