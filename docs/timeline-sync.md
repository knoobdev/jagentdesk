...
Large unbounded timeline responses can exceed WebSocket frame limits, so catch-up uses bounded pages. Bounded does not mean partial.
...
Provider message IDs are not guaranteed for every displayed item. JAgentDesk-generated system errors are one example. Rendered item indices are not durable either because pagination and projection can merge source rows.
