...
- **Node-only benchmark (fast iteration, server pipeline):** `npx tsx scripts/benchmark-terminal-latency.ts`. Boots an isolated daemon (fresh `JAGENTDESK_HOME`, random port — never 6767), measures echo latency percentiles, burst jitter, and snapshot counts under ramped mock-agent load. Writes JSON to `/tmp/jagentdesk-terminal-bench/`. Healthy numbers (2026-06): echo p50 ~2.3ms, p95 ~3.3ms, a 2MB burst fully streamed with `snap=0`.
- **Browser perf specs (user-perceived path):** gated behind `JAGENTDESK_TERMINAL_PERF_E2E=1` —
...
- Tailscale-connected clients use the direct WebSocket path. WireGuard provides transport
  encryption, so the daemon does not run a second relay-side tweetnacl encryption loop.
