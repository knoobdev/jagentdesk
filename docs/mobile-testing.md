...
Run the JAgentDesk mobile suite:
...
Set `JAGENTDESK_MOBILE_E2E_METRO_PORT` when this worktree already has Metro on a non-default port:

```bash
JAGENTDESK_MOBILE_E2E_METRO_PORT=62093 npm run test:e2e:mobile
```
...
- `flows/launch.yaml` — handles dev launcher, dismisses dev menu, asserts "Welcome to JAgentDesk"
...
The workspace-create shell scripts render those subflows into a temp directory before running Maestro, which keeps nested `runFlow` paths and `${JAGENTDESK_MAESTRO_*}` placeholders working together.
...
Expected result: JAgentDesk does not throw `RuntimeException: Audio focus request failed`; native audio reports an interruption and voice mode stops or pauses coherently.

### Releasing the audio session when idle

JAgentDesk must not hold the OS audio session once it is neither capturing nor playing, or the user's
...
open JAgentDesk, use dictation once, stop, and confirm the music resumes at full quality; then
...
to `production` — so a bare `npm run ios` builds `sh.jagentdesk` and collides with the App Store install instead
of the `sh.jagentdesk.debug` dev client. Ignore prebuild's `--non-interactive is not supported` warning; use
...
```bash
cd packages/app/ios
xcodebuild -workspace JAgentDeskDebug.xcworkspace -scheme ExpoTwoWayAudio \
...
Expo dev server logs are in the tmux pane running `npm run dev`. Daemon logs are at `$JAGENTDESK_HOME/daemon.log` (see [development.md](development.md)).
