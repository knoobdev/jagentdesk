...
Desktop dev launches its desktop-managed daemon with `JAGENTDESK_NODE_ENV=development`,
...
`npm run dev` is only a shorthand for `npm run dev:server`. Keep `127.0.0.1:6767` for the packaged app and production-style `~/.jagentdesk` state.
...
Linux produces the `jagentdesk-desktop` launcher and desktop entry. macOS produces
`Applications/JAgentDesk.app` plus the `jagentdesk-desktop` launcher. Both use the nixpkgs
Electron runtime and the checkout's built daemon, client, and renderer rather
than downloading a published desktop release.

### JAGENTDESK_HOME

`JAGENTDESK_HOME` is the directory that holds runtime state (agents, worktrees, workspace config, sockets, daemon log). Resolution rules:

- The **server itself** (e.g. when launched by the desktop app or `npm run start`) defaults to `~/.jagentdesk` (see `packages/server/src/server/jagentdesk-home.ts`).
- **Repo dev scripts** default to `$ROOT/.dev/jagentdesk-home`, where `$ROOT` is the current checkout or worktree root. This keeps all dev state scoped to the checkout instead of the packaged desktop app.
- **`npm run cli -- ...`** runs through the same dev-home wrapper as the dev scripts, so the in-repo CLI automatically targets the current checkout's `.dev/jagentdesk-home` and configured dev daemon endpoint.
- **JAgentDesk-created worktrees** seed `$JAGENTDESK_WORKTREE_PATH/.dev/jagentdesk-home` from `$JAGENTDESK_SOURCE_CHECKOUT_PATH/.dev/jagentdesk-home` by copying durable JSON metadata. Runtime files like pid files, sockets, and logs are not copied.
...
```bash
JAGENTDESK_HOME=~/.jagentdesk-blue npm run dev          # explicit home
JAGENTDESK_DEV_SEED_HOME=/path/to/home npm run dev # seed from a different source home
JAGENTDESK_DEV_RESET_HOME=1 npm run dev            # clear and reseed the derived worktree home
```
...
In JAgentDesk-managed worktree services, use the injected service environment rather than hardcoded root checkout ports.
...
JAgentDesk worktrees expose the native iOS dev app through the `ios-simulator` service in `jagentdesk.json`. The service URL serves the simulator preview at `/.sim`, so the preview link is `${JAGENTDESK_URL}/.sim`.
...
- **An iOS Simulator runtime with at least one iPhone device type**. Recent Xcode versions may not bundle a runtime — add one via Xcode → Settings → Components (older Xcode: "Platforms"). The service targets `iPhone 16 Pro` by default (override with `JAGENTDESK_IOS_DEVICE_TYPE`) and falls back to any iPhone; it fails with `No iPhone simulator device type is installed` when none exist.
...
**Pointing the app at a daemon.** The client resolves its local daemon from `EXPO_PUBLIC_LOCAL_DAEMON` (`packages/app/src/runtime/host-runtime.ts`); when unset it falls back to `localhost:6767`, the production `~/.jagentdesk` daemon. To target a worktree's dev daemon instead, set it on the build command:

```bash
EXPO_PUBLIC_LOCAL_DAEMON=localhost:${JAGENTDESK_SERVICE_DAEMON_PORT} npm run ios   # worktree daemon running as a JAgentDesk service
...
`JAGENTDESK_ELECTRON_REMOTE_DEBUGGING_PORT` when a QA workflow requires a validated,
fixed port.

Desktop dev also scopes Electron `userData` to the current dev root. This prevents
desktop-only environment inherited by terminals opened inside JAgentDesk from coupling
...
```bash
npm run verify:electron-cdp --workspace=@jagentdesk/desktop
```

The verifier reads the same `EXPO_PORT` and
`JAGENTDESK_ELECTRON_REMOTE_DEBUGGING_PORT` environment names as desktop dev. Set an
...
`JAGENTDESK_PORT=57928` alone starts Metro on 57928 but Electron still loads 8081.
...
Captured samples are exposed on `globalThis.__JAGENTDESK_RENDER_PROFILE__`. Call
`globalThis.__JAGENTDESK_RESET_RENDER_PROFILE__?.()` after warm-up and before the
...
reason counts are exposed on `globalThis.__JAGENTDESK_RENDER_PROFILE_REASONS__`.
...
```bash
JAGENTDESK_PROFILE_SERVER_ID=<server-id> \
JAGENTDESK_PROFILE_WORKSPACE_ID=<workspace-path> \
JAGENTDESK_PROFILE_AGENT_ID=<agent-id> \
  npm run profile:workspace-tabs --workspace=@jagentdesk/app
```
...
```bash
JAGENTDESK_PROFILE_APP_URL=http://localhost:19010 # Expo web URL
JAGENTDESK_PROFILE_SWITCH_COUNT=1                # number of agent/terminal switch pairs
JAGENTDESK_PROFILE_SWITCH_WAIT_MS=250            # delay after each click
JAGENTDESK_PROFILE_IDLE_WAIT_MS=3000             # idle baseline before switching
JAGENTDESK_PROFILE_DUMP_COMMITS=1                # include per-commit profiler samples
```
...
Check `$JAGENTDESK_HOME/daemon.log` for daemon logs. The default level is `info`; set
`JAGENTDESK_LOG_LEVEL=trace` before launching the daemon when you need full provider,
...
`$JAGENTDESK_HOME/config.json` win first. Without persisted config, the optional
`JAGENTDESK_LOG_ROTATE_SIZE` and `JAGENTDESK_LOG_ROTATE_COUNT` env vars override the
...
the daemon-global Git process limits in `$JAGENTDESK_HOME/config.json`:
...
Restart the daemon with `jagentdesk daemon restart`. If JAgentDesk Desktop manages the daemon, fully quit and
...
Measure the MCP `tools/list` payload that JAgentDesk injects into agents with:

```bash
npm run measure:agent-tools --workspace=@jagentdesk/server
```
...
## jagentdesk.json service scripts
...
Windows. They inherit the daemon environment plus JAgentDesk's lifecycle variables;
...
| Variable                    | Value                                                                                                                     |
| --------------------------- | ------------------------------------------------------------------------------------------------------------------------- |
| `JAGENTDESK_SERVICE_<NAME>_URL`  | Proxied URL for a declared peer service. Prefer this for peer discovery; it survives peer restarts.                       |
| `JAGENTDESK_SERVICE_<NAME>_PORT` | Raw ephemeral port for a declared peer service. Use only as a bypass escape hatch; it can go stale if that peer restarts. |
| `JAGENTDESK_URL`                 | Self alias for `JAGENTDESK_SERVICE_<SELF>_URL`.                                                                                |
| `JAGENTDESK_PORT`                | Self alias for `JAGENTDESK_SERVICE_<SELF>_PORT`.                                                                               |
...
```json
{
  "scripts": {
    "web": {
      "type": "service",
      "command": "PORT=$JAGENTDESK_PORT npm run dev:web"
    }
  }
}
```
...
`$JAGENTDESK_HOME/config.json`, or replace it for one project with `worktree.servicePorts` in
`jagentdesk.json`. The block accepts an inclusive `range` such as `"3000-4000"` or a `portScript`
...
values are available as `JAGENTDESK_SCRIPTNAME`, `JAGENTDESK_WORKSPACE_ID`, `JAGENTDESK_BRANCH_NAME`, and
`JAGENTDESK_WORKTREE_PATH`. The script must print one valid TCP port. JAgentDesk trusts the external allocator,
...
The desktop-managed daemon disables the bundled web UI by default (`JAGENTDESK_WEB_UI_ENABLED=false`) because the desktop app already ships the renderer as `app-dist`. Shipping the same assets again inside `@jagentdesk/server` would duplicate the ~10.8 MiB install. Desktop packaging also excludes `node_modules/@jagentdesk/server/dist/server/web-ui/**` from the packaged app.
...
- Changed `packages/server/src/*`, `packages/cli/src/*`, or `packages/highlight/src/*`: `npm run build:server`.
...
Use `npm run cli` to run the in-repo CLI from source (`npx tsx packages/cli/src/index.ts`). The script wraps the CLI with `scripts/dev-home.sh`, so it automatically uses this checkout's `.dev/jagentdesk-home` and dev daemon endpoint unless you pass an explicit override. The globally installed `jagentdesk` binary on macOS is a symlink into the installed JAgentDesk desktop app, not this checkout — use it to drive the desktop's built-in daemon, but use `npm run cli` when you want to talk to the CLI you are editing.

Canonical automation uses `jagentdesk workspace create/ls/archive`, `jagentdesk heartbeat create/update/delete`, and the full `jagentdesk schedule` group. MCP heartbeat automation is intentionally smaller: create and delete only. Detach remains an explicit user lifecycle action rather than an agent tool. `jagentdesk run --new-workspace local|worktree` composes workspace creation with agent creation. The old `jagentdesk worktree` and `jagentdesk run --worktree` forms are hidden compatibility aliases.
...
npm run cli -- agent open <id>       # Focus an existing agent in JAgentDesk Desktop
...
sending a message. Use `jagentdesk://h/<server-id>/agent/<agent-id>`, or run
`jagentdesk agent open <agent-id>`. The CLI reads the local daemon's server ID by
...
```
$JAGENTDESK_HOME/agents/{cwd-with-dashes}/{agent-id}.json
```

Find an agent by ID:

```bash
find $JAGENTDESK_HOME/agents -name "{agent-id}.json"
```

Find by content:

```bash
rg -l "some title text" $JAGENTDESK_HOME/agents/
```
...
Point Playwright MCP at the running Expo web target. For root checkout dev, `npm run dev:app` reserves `http://localhost:8081`. For JAgentDesk-managed worktree app services, use the service URL or port shown by JAgentDesk for that worktree.

Do NOT use browser history (back/forward). Always navigate by clicking UI elements or using `browser_navigate` with the full URL — the app uses client-side routing and browser history breaks state.

## Local web renderer

`packages/app` can export a local browser renderer for development and diagnostics. It is not a
hosted product surface and is served from the workspace or the daemon that owns the connection.
Keep the renderer and daemon on the same build when validating a protocol change.
