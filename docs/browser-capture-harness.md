...
```bash
npm run capture-harness --workspace=@jagentdesk/desktop
```
...
JAGENTDESK_CAPTURE_HARNESS_GROUP=automation npm run capture-harness --workspace=@jagentdesk/desktop
```

Run the shared browser profile fixture with:

```bash
JAGENTDESK_CAPTURE_HARNESS_GROUP=browser-profile npm run capture-harness --workspace=@jagentdesk/desktop
```
