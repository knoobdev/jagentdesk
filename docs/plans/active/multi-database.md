...
- **Packaged production app (done): built + ran the electron-builder `.app`** (`packages/desktop/e2e/database-acceptance-packaged.e2e.mjs` — launches the signed app isolated on a random daemon port). **PostgreSQL: 7/7** (add via the real form → connect → browse 62/ N tables → data grid → structure grid → ER diagram). Networked engines (pg/mysql/mssql/oracle/mongo/clickhouse are pure-JS drivers) work in the packaged build.
- **SQLite in the packaged app — FIXED.** Root cause: the bundled daemon runs Electron's Node 24 (NODE_MODULE_VERSION 145) but `better-sqlite3@11` shipped a Node-ABI-127 prebuild (and its C++ used V8 APIs removed in Electron 41, so it couldn't even recompile). Fix: **upgrade to `better-sqlite3@13`, which uses N-API** — a single ABI-stable `prebuilds/<platform>.node` that loads unchanged under Node 22, Node 24, and Electron, no per-ABI rebuild. Re-ran the packaged acceptance: **SQLite 6/6** (connect → browse → data grid 8 rows → structure grid → ER diagram) in the signed `.app`. Non-docker DB suite 36/36; server typecheck clean.
