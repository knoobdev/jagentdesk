...
- **Known packaged-build limitation — SQLite:** the bundled daemon runs under Electron's Node (v24 / NODE_MODULE_VERSION **145**), but `better-sqlite3` ships a Node-ABI **127** prebuilt, so SQLite connections fail in the packaged `.app` with a NODE_MODULE_VERSION mismatch (dev mode is unaffected — it runs system Node 22 = 127). The fix is to recompile `better-sqlite3` for Electron at build time (electron `npmRebuild` / `@electron/rebuild` from source with the full package unpacked); it could not be compiled in this environment (node-gyp/amalgamation toolchain gap) and needs a proper build machine/CI. Does **not** affect any networked engine.
