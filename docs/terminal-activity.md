...
JAgentDesk surfaces terminal activity as a tab indicator (the same "running" dot used by agents).
...
OpenCode uses a server plugin instead of command hooks. The plugin listens to OpenCode bus events and emits these JAgentDesk hook events:
...
removes JAgentDesk's marker-matched hooks on disable. `jagentdesk hooks` keeps working regardless — the gate
...
When enabled, JAgentDesk installs provider hooks globally:

- Claude hooks are written to `~/.claude/settings.json` (or `CLAUDE_CONFIG_DIR/settings.json` when that override is set).
- Codex hooks are written to `~/.codex/hooks.json` (or `CODEX_HOME/hooks.json` when that override is set). Codex supports a native `commandWindows`, so each JAgentDesk hook includes both POSIX and Windows commands. Non-managed Codex hooks are trust-gated by Codex; users may see Codex's hook review prompt before the hook runs.
- OpenCode gets a self-contained plugin at `$XDG_CONFIG_HOME/opencode/plugins/jagentdesk-terminal-activity.js` (or `~/.config/opencode/plugins/jagentdesk-terminal-activity.js` when XDG is unset; `OPENCODE_CONFIG_DIR` still wins when set).

Installation is marker-based/idempotent for config hooks and exact-file/idempotent for the OpenCode plugin. JAgentDesk preserves user hooks, removes only its own marker-matched command hooks, and leaves hooks installed across daemon shutdown. Outside a JAgentDesk terminal they are inert because the command or plugin is gated on `JAGENTDESK_TERMINAL_ID`.
...
```sh
[ -n "$JAGENTDESK_TERMINAL_ID" ] && "${JAGENTDESK_HOOK_CLI:-jagentdesk}" hooks claude <event>
```

Codex also receives the Windows equivalent:

```bat
if defined JAGENTDESK_TERMINAL_ID (if defined JAGENTDESK_HOOK_CLI ("%JAGENTDESK_HOOK_CLI%" hooks codex <event>) else (jagentdesk hooks codex <event>))
```

The daemon resolves the current CLI through `JAGENTDESK_CLI` when its launcher supplies one, or through the npm package shim for standalone installs. Terminal setup exposes that resolved executable to hooks as `JAGENTDESK_HOOK_CLI`; desktop and other daemon launchers do not know about the hook-specific variable. The generated command falls back to bare `jagentdesk` if the hook env is missing and no-ops outside JAgentDesk terminals because the `JAGENTDESK_TERMINAL_ID` gate remains first. JAgentDesk also prepends the resolved CLI directory to each terminal `PATH` as a secondary fallback. All other behavior lives in `jagentdesk hooks`: read the env, map the event, POST activity, and no-op/fail-open when anything is missing or unavailable.
