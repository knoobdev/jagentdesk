{
  ...
  # (`jagentdesk-desktop-<v>-npm-deps`) and refetches the entire registry. Override
  # the upstream hash via `jagentdesk.override { npmDepsHash = "..."; }`.
  jagentdesk,
}:
buildNpmPackage {
  pname = "jagentdesk-desktop";
  ...
  buildPhase = ''
    ...
    npm run build --workspace=@jagentdesk/expo-two-way-audio

    # Expo web export for the Electron renderer
    ( cd packages/app && JAGENTDESK_WEB_PLATFORM=electron npx expo export --platform web )

    # Desktop main process
    npm run build:main --workspace=@jagentdesk/desktop
  ...
  installPhase = ''
    ...
    ${lib.optionalString stdenv.hostPlatform.isLinux ''
      mkdir -p $out/share/jagentdesk-desktop
      ...
      JAGENTDESK_TRACE_DESKTOP=1 node scripts/trace-daemon.mjs > desktop-files.txt

      while IFS= read -r path; do
        [ -z "$path" ] && continue
        mkdir -p "$out/share/jagentdesk-desktop/$(dirname "$path")"
        cp -a "$path" "$out/share/jagentdesk-desktop/$path"
      done < desktop-files.txt

      # Keep the same unpackaged monorepo layout expected by main.js.
      cp package.json $out/share/jagentdesk-desktop/
      mkdir -p $out/share/jagentdesk-desktop/packages/app
      cp -a packages/app/dist $out/share/jagentdesk-desktop/packages/app/
      ...
      for runtime_path in \
        ...
        if [ ! -e "$out/share/jagentdesk-desktop/$runtime_path" ]; then
          echo "desktop runtime trace omitted $runtime_path" >&2
          exit 1
        fi
      done

      if [ -e $out/share/jagentdesk-desktop/node_modules/electron ]; then
      ...
      if [ -d skills ]; then
        cp -a skills $out/share/jagentdesk-desktop/
      fi

      # Hicolor icon for desktop environments
      install -Dm644 packages/desktop/assets/icon.png \
        $out/share/icons/hicolor/512x512/apps/jagentdesk-desktop.png

      # Electron derives Wayland's toplevel app_id from the package name in the
      # app root it launches. Point it at a one-file app named "jagentdesk-desktop"
      # so shells can match the window to the desktop entry and hicolor icon.
      mkdir -p $out/share/jagentdesk-desktop/electron-app
      printf '%s\n' "{ \"name\": \"jagentdesk-desktop\", \"version\": \"$version\", \"main\": \"index.js\" }" \
        > $out/share/jagentdesk-desktop/electron-app/package.json
      printf '%s\n' 'require("../packages/desktop/dist/main.js");' \
        > $out/share/jagentdesk-desktop/electron-app/index.js

      # Chromium's setuid sandbox cannot live in the immutable Nix store.
      makeWrapper ${electron}/bin/electron $out/bin/jagentdesk-desktop \
        --add-flags "$out/share/jagentdesk-desktop/electron-app" \
        --add-flags "--no-sandbox" \
        --add-flags "--class=jagentdesk-desktop" \
        --set EXPO_DEV_URL "jagentdesk://app/" \
        --set CHROME_DESKTOP "jagentdesk-desktop.desktop"

      copyDesktopItems
    ''}

    ${lib.optionalString stdenv.hostPlatform.isDarwin ''
      app="$(find packages/desktop/release -maxdepth 3 -type d -name JAgentDesk.app -print -quit)"
      if [ -z "$app" ]; then
        echo "electron-builder did not produce JAgentDesk.app" >&2
        exit 1
      fi
      mkdir -p "$out/Applications"
      cp -R "$app" "$out/Applications/JAgentDesk.app"
      ln -s ../Applications/JAgentDesk.app/Contents/MacOS/JAgentDesk "$out/bin/jagentdesk-desktop"
    ''}
  ...
  desktopItems = lib.optionals stdenv.hostPlatform.isLinux [
    (makeDesktopItem {
      name = "jagentdesk-desktop";
      desktopName = "JAgentDesk";
      genericName = "AI Coding Agents";
      comment = "Self-hosted daemon for AI coding agents";
      exec = "jagentdesk-desktop";
      icon = "jagentdesk-desktop";
      categories = ["Development"];
      startupWMClass = "jagentdesk-desktop";
    })
    ...
    # package.json `name` ("jagentdesk-desktop"), 38 uses the runtime app name that
    # main.ts sets ("JAgentDesk"). Ship a NoDisplay entry for the second spelling so
    ...
    (makeDesktopItem {
      ...
      icon = "jagentdesk-desktop";
      categories = [ "Development" ];
      startupWMClass = "JAgentDesk";
      noDisplay = true;
    })
  ];

  meta = {
    description = "JAgentDesk desktop app (Electron wrapper)";
    license = lib.licenses.agpl3Plus;
    mainProgram = "jagentdesk-desktop";
